...
void Lnast_visitor::handle(const slang::AssignmentExpression& expr) {
  ...
  for (auto it = verilogList.crbegin(); it != verilogList.crend(); ++it) {
    ...
    if (*it == "AND") {
      ...
      auto node_and1 = Lnast_node::create_ref(lnast->add_string("__and" + std::to_string(count)));
      tmpList.insert(tmpList.begin(), ("__and" + std::to_string(count)));  // for connection the node refs

      auto idx_op3 = lnast->add_child(idx_and, node_and1);
      (void)idx_op3;
  ...
  for (auto it = verilogList.crbegin(); it != verilogList.crend(); ++it) {
    if (inc < count) {
      ...
      for (auto it2 = tmpList.cbegin(); it2 != tmpList.cend(); ++it2) std::cout << " " << *it2;
      if (*it == "AND") {

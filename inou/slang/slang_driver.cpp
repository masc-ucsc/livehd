...
int driverMain(int argc, TArgs argv, bool suppressColorsStdout, bool suppressColorsStderr) try {
  ...
  cmdLine.add("-y,--libdir", libDirs, "Library search paths, which will be searched for missing modules", "<dir>");
  ...
  cmdLine.add("--ast-json", astJsonFile, "Dump the compiled AST in JSON format to the specified file, or '-' for stdout", "<file>");
  ...
  if (!cmdLine.parse(argc, argv)) {
    for (auto& err : cmdLine.getErrors()) Pass::error("{}\n", err);
    return 1;
  }
  ...
  for (const std::string& dir : includeDirs) {
    try {
      ...
      Pass::error("include directory '{}' does not exist\n", dir);
      anyErrors = true;
    }
  }

...
bool runPreprocessor(SourceManager& sourceManager, const Bag& options, const std::vector<SourceBuffer>& buffers,
                     bool includeComments, bool includeDirectives) {
  BumpAllocator alloc;
...
void printMacros(SourceManager& sourceManager, const Bag& options, const std::vector<SourceBuffer>& buffers) {
  BumpAllocator alloc;
  Diagnostics   diagnostics;
  Preprocessor  preprocessor(sourceManager, alloc, diagnostics, options);
...
SourceBuffer readSource(SourceManager& sourceManager, const std::string& file) {
  ...
  if (!buffer) {
    // Pass::error(fg(errorColor), "error: ");
...
bool loadAllSources(Compilation& compilation, SourceManager& sourceManager, const std::vector<SourceBuffer>& buffers,
  ...
  auto findMissingNames = [&](const std::shared_ptr<SyntaxTree>& tree, flat_hash_set<string_view>& missing) {
    auto& meta = tree->getMetadata();
...
public:
  ...
  Compiler(Compilation& _compilation) : compilation(_compilation), diagEngine(*_compilation.getSourceManager()) {
...
int driverMain(int argc, TArgs argv, Slang_tree& tree, bool suppressColorsStdout, bool suppressColorsStderr) try {
  ...
  cmdLine.add("-I,--include-directory,+incdir", includeDirs, "Additional include search paths", "<dir>", /* isFileName */ true);
  cmdLine.add("--isystem",
              includeSystemDirs,
              "Additional system include search paths",
              "<dir>",
              /* isFileName */ true);
  cmdLine.add("-y,--libdir",
              libDirs,
  ...
  cmdLine.add("-D,--define-macro,+define",
              defines,
              "Define <macro> to <value> (or 1 if <value> ommitted) in all source files",
              "<macro>=<value>");
  cmdLine.add("-U,--undefine-macro", undefines, "Undefine macro name at the start of all source files", "<macro>");
  cmdLine.add("--comments", includeComments, "Include comments in preprocessed output (with -E)");
  cmdLine.add("--directives", includeDirectives, "Include compiler directives in preprocessed output (with -E)");
  cmdLine.add("--max-include-depth", maxIncludeDepth, "Maximum depth of nested include files allowed", "<depth>");

  // Parsing
  optional<uint32_t> maxParseDepth;
  optional<uint32_t> maxLexerErrors;
  cmdLine.add("--max-parse-depth", maxParseDepth, "Maximum depth of nested language constructs allowed", "<depth>");
  cmdLine.add("--max-lexer-errors",
              maxLexerErrors,
  ...
  cmdLine.add("--max-hierarchy-depth", maxInstanceDepth, "Maximum depth of the design hierarchy", "<depth>");
  cmdLine.add("--max-generate-steps",
              maxGenerateSteps,
              "Maximum number of steps that can occur during generate block "
              "evaluation before giving up",
              "<steps>");
  cmdLine.add("--max-constexpr-depth", maxConstexprDepth, "Maximum depth of a constant evaluation call stack", "<depth>");
  cmdLine.add("--max-constexpr-steps",
              maxConstexprSteps,
              "Maximum number of steps that can occur during constant "
              "evaluation before giving up",
              "<steps>");
  cmdLine.add("--constexpr-backtrace-limit",
              maxConstexprBacktrace,
              "Maximum number of frames to show when printing a constant evaluation "
              "backtrace; the rest will be abbreviated",
              "<limit>");
  cmdLine.add("--compat", compat, "Attempt to increase compatibility with the specified tool", "vcs");
  cmdLine.add("-T,--timing", minTypMax, "Select which value to consider in min:typ:max expressions", "min|typ|max");
  cmdLine.add("--allow-use-before-declare",
              allowUseBeforeDeclare,
              "Don't issue an error for use of names before their declarations.");
  cmdLine.add("--relax-enum-conversions", relaxEnumConversions, "Allow all integral types to convert implicitly to enum types.");
  cmdLine.add("--allow-hierarchical-const", allowHierarchicalConst, "Allow hierarchical references in constant expressions.");
  cmdLine.add("--allow-dup-initial-drivers",
              allowDupInitialDrivers,
              "Allow signals driven in an always_comb or always_ff block to also be driven "
              "by initial blocks.");
  cmdLine.add("--strict-driver-checking",
  ...
  cmdLine.add("--error-limit",
              ...
              "disable the limit.",
              "<limit>");
  cmdLine.add("--ignore-unknown-modules",
  ...
  for (const std::string& dir : includeDirs) {
    try {
      ...
      // Pass::error(fg(warningColor), "warning: ");
      Pass::warn("include directory '{}' does not exist\n", dir);
    }
  }
  ...
  if (compat.has_value()) {
    if (compat == "vcs") {
      ...
      // Pass::error(fg(errorColor), "error: ");
      Pass::error("invalid value for compat option: '{}'", *compat);
      return 1;
    }
  }
  ...
  if (minTypMax.has_value()) {
    if (minTypMax == "min") {
      ...
      // Pass::error(fg(errorColor), "error: ");
      Pass::error("invalid value for timing option: '{}'", *minTypMax);
      return 1;
    }
  }
  ...
  try {
    if (onlyPreprocess == true) {
      anyErrors = !runPreprocessor(sourceManager, options, buffers, includeComments == true, includeDirectives == true);
      ...
      compiler.setDiagnosticOptions(warningOptions, ignoreUnknownModules == true, allowUseBeforeDeclare == true, compatMode);
...
int wmain(int argc, wchar_t** argv) {
  ...
  return driverMain(argc, argv, !supportsColors(STD_OUTPUT_HANDLE), !supportsColors(STD_ERROR_HANDLE));
}
#endif
...
#ifndef FUZZ_TARGET
int slang_main(int argc, char** argv, Slang_tree& tree) { return driverMain(argc, argv, tree, false, false); }
#endif

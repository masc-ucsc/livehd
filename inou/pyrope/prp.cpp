...
bool Prp::rule_code_block_int() {
  ...
  if (rule_if_statement()) {
    ...
    RULE_SUCCESS("Matched rule_code_block_int.\n", Prp_rule_code_block_int);
  } else if (rule_return_statement()) {
    RULE_SUCCESS("Matched rule_code_block_int.\n", Prp_rule_code_block_int);
  } else if (rule_compile_check_statement()) {
    RULE_SUCCESS("Matched rule_code_block_int.\n", Prp_rule_code_block_int);
  } else if (rule_assertion_statement()) {
    RULE_SUCCESS("Matched rule_code_block_int.\n", Prp_rule_code_block_int);
  }
...
bool Prp::rule_if_statement() {
  ...
  if (!(SCAN_IS_TOKEN(Pyrope_id_if, Prp_rule_if_statement) || SCAN_IS_TOKEN(Pyrope_id_unique))) {
    RULE_FAILED("Failed rule_if_statement; couldn't find an if or a unique token.\n");
  }
  if (!rule_logical_expression()) {
    RULE_FAILED("Failed rule_if_statement; couldn't find a logical_expression.\n");
  }
  if (!rule_empty_scope_colon()) {
...
bool Prp::rule_for_index() {
  ...
  if (!rule_rhs_expression_property()) {
    RULE_FAILED("Failed rule_for_index; couldn't find an rhs_expression_property.\n");
  }
...
bool Prp::rule_try_statement() {
  ...
  // if(!SCAN_IS_TOKEN(Pyrope_id_else, Prp_rule_try_statement)){ RULE_FAILED("Failed rule_try_statement; couldn't find an else
  // token.\n"); }
...
bool Prp::rule_scope_else() {
  ...
  if (!SCAN_IS_TOKEN(Token_id_cb, Prp_rule_scope_else)) RULE_SUCCESS("Matched rule_scope_else.\n", Prp_rule_scope_else);
}
...
bool Prp::rule_scope_body() {
  ...
  if (!rule_code_blocks()) {
    if (!rule_logical_expression()) {
      RULE_FAILED("Failed rule_scope_body; couldn't find either a logical_expression or a code_blocks.\n");
    }
  ...
  // option 2
  if (!rule_logical_expression()) {
    RULE_FAILED("Failed rule_scope_body\n", Prp_rule_scope_body);
  }
...
bool Prp::rule_scope_colon() {
  ...
  if (!rule_scope()) {
    RULE_FAILED("Failed rule_scope_colon; couldn't find a scope.\n");
  }
  if (!SCAN_IS_TOKEN(Token_id_ob)) {
    RULE_FAILED("Failed rule_scope_colon; couldn't find an opening brace.\n");
  }
...
bool Prp::rule_scope_declaration() {
  ...
  if (!SCAN_IS_TOKEN(Token_id_cb)) {
    RULE_FAILED("Failed rule_scope_declaration; couldn't find a closing brace.\n");
  }
  ...
  if (rule_identifier()) {
    while (next) {
      if (!SCAN_IS_TOKEN(Token_id_dot))
        ...
        if (!rule_identifier()) {
          RULE_FAILED("Failed rule_punch_rhs; couldn't find an identifier.\n");
        }
      }
    }
  }
...
bool Prp::rule_fcall_explicit() {
  ...
  if (rule_tuple_notation()) {
    if (!SCAN_IS_TOKEN(Token_id_dot)) {
      RULE_FAILED("Failed rule_fcall_explicit; couldn't find an answering dot token.\n");
    }
  }
...
bool Prp::rule_fcall_implicit() {
  ...
  if (!rule_scope_declaration()) {
    RULE_FAILED("Failed rule_fcall_implicit; couldnt find a scope_declaration.\n");
  }
...
bool Prp::rule_assignment_expression() {
  ...
  if (!(rule_lhs_expression() || rule_overload_notation())) {
    RULE_FAILED("Failed rule_assignment_expression; couldn't find either an overload_notation or an lhs_expression.\n");
  }
  if (!rule_assignment_operator()) {
    RULE_FAILED("Failed rule_assignment_expression; couldn't find an assignment_operator.\n");
  }
  if (!(rule_rhs_expression_property() || rule_logical_expression() || rule_fcall_implicit())) {
    RULE_FAILED(
        "Failed rule_assignment_expression; couldn't find an rhs_expression_property, an fcall_implicit, or a "
        "logical_expression.\n");
  }
...
bool Prp::rule_lhs_expression() {
  ...
  if (!(rule_tuple_notation() || rule_range_notation())) {
...
bool Prp::rule_rhs_expression_property() {
  ...
  if (!(rule_identifier() == 2)) {
    RULE_FAILED("Failed rule_rhs_expression_property; couldn't find an identifier that was a label.\n");
  }
...
bool Prp::rule_tuple_notation() {
  ...
  if (SCAN_IS_TOKEN(Token_id_op, Prp_rule_tuple_notation)) {
    ...
    if (!(rule_rhs_expression_property() || rule_logical_expression())) {
      RULE_FAILED("Failed rule_tuple_notation; couldn't find either an rhs_expression_property or a logical_expression.\n");
    }
...
bool Prp::rule_tuple_dot_dot() {
  ...
  while (next) {
    if (!SCAN_IS_TOKEN(Token_id_dot)) {
      ...
      if (!rule_tuple_array_notation()) {
        RULE_FAILED("Failed rule_tuple_dot_dot; couldn't find a tuple_array_notation.\n");
      }
    }
  }
...
bool Prp::rule_tuple_array_bracket() {
  ...
  while (next) {
    if (!SCAN_IS_TOKEN(Token_id_obr)) {
      ...
      if (!rule_logical_expression()) {
        RULE_FAILED("Failed rule_tuple_array_bracket; couldn't find a logical_expression.\n");
      }
      if (!SCAN_IS_TOKEN(Token_id_cbr)) {
        RULE_FAILED("Failed rule_tuple_array_bracket; couldn't find a closing bracket.\n");
      }
    }
  }
...
bool Prp::rule_constant() {
  ...
  // option 1
  if (rule_numerical_constant()) {
    RULE_SUCCESS("Matched rule_constant.\n", Prp_rule_constant);
  }

  // option 2
  if (!rule_string_constant()) {
...
bool Prp::rule_string_constant() {
  ...
  if (!SCAN_IS_TOKEN(Token_id_string, Prp_rule_string_constant)) {
    RULE_FAILED("Failed rule_string_constant; couldn't find a double string.\n");
  }
...
bool Prp::rule_assignment_operator() {
  ...
  if (SCAN_IS_TOKEN(Token_id_eq, Prp_rule_assignment_operator)) {
    RULE_SUCCESS("Matched rule_assignment_operator; found an equals.\n", Prp_rule_assignment_operator);
  }

  if (SCAN_IS_TOKEN(Pyrope_id_as, Prp_rule_assignment_operator)) {
    RULE_SUCCESS("Matched rule_assignment_operator; found an as.\n", Prp_rule_assignment_operator);
  }
  ...
  if (SCAN_IS_TOKEN(Token_id_plus, Prp_rule_assignment_operator) || SCAN_IS_TOKEN(Token_id_mult) || SCAN_IS_TOKEN(Token_id_minus)) {
    ...
    if (SCAN_IS_TOKEN(Token_id_eq, Prp_rule_assignment_operator)) {
      RULE_SUCCESS("Matched rule_assignment_operator; found an [operator] equals (two tokens or ++=).\n",
                   Prp_rule_assignment_operator);
    }
  }
...
bool Prp::rule_bit_selection_bracket() {
  ...
  while (next) {
    if (!SCAN_IS_TOKEN(Token_id_obr)) {
      ...
      if (!SCAN_IS_TOKEN(Token_id_obr)) {
        RULE_FAILED("Failed rule_bit_selection_bracket; couldn't find a second open bracket.\n");
      }
      // optional
      rule_logical_expression();
      if (!SCAN_IS_TOKEN(Token_id_cbr)) {
        RULE_FAILED("Failed rule_bit_selection_bracket; couldn't find a closing bracket.\n");
      }
      if (!SCAN_IS_TOKEN(Token_id_cbr)) {
        RULE_FAILED("Failed rule_bit_selection_bracket; couldn't find a second closing bracket.\n");
      }
    }
  }
  ...
  while (next) {
    if (SCAN_IS_TOKEN(Token_id_plus, Prp_rule_additive_expression)) {  // can be +, ++, or +*
      ...
      if (SCAN_IS_TOKEN(Token_id_lt, Prp_rule_additive_expression)) {
        unconsume_token();
        ast_add(Prp_rule_additive_expression, scan_token());
        consume_token();
        found_op = true;
      } else {
...
bool Prp::rule_unary_expression() {
  ...
  if (!(SCAN_IS_TOKEN(Token_id_bang, Prp_rule_unary_expression) || SCAN_IS_TOKEN(Pyrope_id_tilde, Prp_rule_unary_expression))) {
...
bool Prp::rule_overload_name() {
  ...
  if (!next) {
    RULE_FAILED("Failed rule_overload_notation; found an overload_exception.\n");
  }
  while (next) next = rule_overload_exception();
...
bool Prp::rule_rhs_expression() {
  ...
  if (!(rule_fcall_explicit() || rule_lhs_expression() || rule_scope_declaration())) {
...
void Prp::elaborate() {
  ...
  for (auto it = subtree_stack.begin(); it != subtree_stack.end(); ++it) {
    PRINT("Operation: {}, Rule: {}, Token: {}\n", std::get<1>(*it), rule_id_to_string(std::get<2>(*it)),
          scan_text(std::get<3>(*it)));
  }
...
void Prp::ast_up(Rule_id rid) {
  ...
  if (add_stack.empty() && subtree_stack.empty()) {
    ...
    if (!subtree_indices.empty()) {
      while (std::get<0>(subtree_stack[std::get<1>(subtree_indices[subtree_indices.size() - 1 - sub_cnt])]) >
             std::get<0>(down_stack.back())) {
        sub_cnt++;
        if (sub_cnt >= subtree_indices.size()) break;
      }
    }
    ...
    if (add_cnt > 1 && sub_cnt == 0) {
      ...
      PRINT("Pushing an up call with rule {} to the subtree stack.\n", rule_id_to_string(rid));
      ...
      while ((add_cnt + sub_cnt) > 0) {
        ...
        if (sub_cnt == 0) {
          PRINT("Sub_cnt reached zero; placing an add call with token {} at the beginning of the subtree stack.\n",
                scan_text(std::get<3>(add_stack.back())));
          subtree_stack.emplace(subtree_stack.begin() + subtree_start, add_stack.back());
          PRINT("1 add_cnt: {}, add_stack size: {}\n", add_cnt, add_stack.size());
          add_stack.pop_back();
          add_cnt--;
          PRINT("2 add_cnt: {}, add_stack size: {}\n", add_cnt, add_stack.size());
        } else {
          // if the add calls came later, put them on first
          if (add_cnt == 0) {
            PRINT("Subtree ending with call {} and rule {} at index {} comes after add call or is the last thing.\n",
                  std::get<1>(subtree_stack[std::get<1>(subtree_indices.back())]),
                  rule_id_to_string(std::get<2>(subtree_stack[std::get<1>(subtree_indices.back())])),
                  std::get<1>(subtree_indices.back()));
            subtree_indices.pop_back();
            sub_cnt--;
          } else {
            if (std::get<0>(add_stack.back()) > std::get<0>(subtree_stack[std::get<1>(subtree_indices.back())])) {
              PRINT("Placing an add call with token {} at index {} in the subtree stack; it had op {} at that position before.\n",
                    scan_text(std::get<3>(add_stack.back())), std::get<1>(subtree_indices.back()) + 1,
                    std::get<1>(subtree_stack[std::get<1>(subtree_indices.back())]));
            ...
            // otherwise, just leave the subtree where it is; it is being merged into a single new subtree
              ...
              PRINT("Subtree ending with call {} and rule {} at index {} comes after add call with token {}.\n",
                    std::get<1>(subtree_stack[std::get<1>(subtree_indices.back())]),
                    rule_id_to_string(std::get<2>(subtree_stack[std::get<1>(subtree_indices.back())])),
                    std::get<1>(subtree_indices.back()), scan_text(std::get<3>(add_stack.back())));
              subtree_indices.pop_back();
              sub_cnt--;
            }
          }
      ...
      PRINT("Down stack size should not be zero; down stack size: {}; subtree_start: {}\n", down_stack.size(), subtree_start);
    ...
    // we have a bad node, just pop the down call and move on.
      ...
      PRINT("Couldn't find 2 good children for up call to {}. Popping matching down call.\n", rule_id_to_string(rid));
      // subtree_stack[std::get<1>(subtree_indices.back())]
      down_stack.pop_back();
    }
    // print the entire subtree stack for debugging
    //           PRINT("PRINTING THE SUBTREE STACK\n");
    //           for(uint32_t j = 0; j<subtree_stack.size(); j++){
    //             PRINT("Operation: {} Rule: {} Token: {}\n", std::get<0>(std::get<1>(subtree_stack[j])),
    //             rule_id_to_string(std::get<1>(std::get<1>(subtree_stack[j]))),
    //             scan_text(std::get<2>(std::get<1>(subtree_stack[j]))));
    //           }
    //           PRINT("PRINTING THE SUBTREE INDICES STACK.\n");
    //           for(uint32_t j = 0; j<subtree_indices.size(); j++){
    //             PRINT("Subtree {} indices: ({},{})\n", j, std::get<0>(subtree_indices[j]), std::get<1>(subtree_indices[j]));
    //           }
  }
}

void Prp::ast_add(Rule_id rid, Token_entry token) {
  PRINT("Added add call with token {} from rule {}.\n", scan_text(), rule_id_to_string(rid));
...
bool Prp::chk_and_consume(Token_id tok, Rule_id rid) {
  PRINT("called chk_and_consume: trying to match token {} to token {}\n", scan_is_end() ? "no token" : scan_text(),
        tok_id_to_string(tok));
  if (scan_is_token(tok)) {
    PRINT("chk_and_consume: matched a token.\n");
    if (rid != Prp_invalid) {
      PRINT("chk_and_consume: added a token {} to the ast.\n", rule_id_to_string(rid));
      ast_add(rid, scan_token());
    }
    consume_token();
    return true;
  }
  ...
  for (auto it = subtree_stack.begin(); it != subtree_stack.end(); ++it) {
    ...
    switch (std::get<1>(ast_op)) {
      ...
      case 1:
        ...
        PRINT("Went up with rule {}.\n", rule_id_to_string(std::get<2>(ast_op)));
        break;
      case 2:
        ast->down();
        ast->add(std::get<2>(ast_op), std::get<3>(ast_op));
        PRINT("Added token {} from rule {}.\n", scan_text(std::get<3>(ast_op)), rule_id_to_string(std::get<2>(ast_op)));
        ast->up(std::get<2>(ast_op));
        break;
    }
  }

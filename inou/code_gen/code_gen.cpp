...
void Code_gen::do_assign(const mmap_lib::Tree_index& assign_node_index, std::vector<mmap_lib::str>& hier_tup_vec,
  ...
  if (is_temp_var(key)) {
    ...
    if (key_sec.ends_with(".__sbits") || key_sec.ends_with(".__ubits")) {
      param_converted = lnast_to->set_convert_parameters(key_sec, ref.to_s());  // for getting UInt<3> a from $a.___bits=3
    }
    if (!param_converted) {
      ...
      if (!ref_map_inst_res.second) {  // this means an equivalent key already exists.
        ...
        if (hier_tup_assign) {
          ...
          absl::StrAppend(&buffer_to_print
              ,indent().to_s()
              ,lnast_to->ref_name_std(key_sec)
              ," "
              ,lnast_to->debug_name_lang(assign_node_data.type).to_s()
              ," "
...
void Code_gen::do_while(const mmap_lib::Tree_index& while_node_index) {
  ...
  absl::StrAppend(&buffer_to_print, indent().to_s(), "while");
  ...
  do_stmts(curr_index);
  indendation--;
  absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->for_stmt_end());
}
//-------------------------------------------------------------------------------------
...
void Code_gen::do_for(const mmap_lib::Tree_index& for_node_index) {
  fmt::print("node:for\n");
  absl::StrAppend(&buffer_to_print, indent().to_s(), "for");
  ...
  absl::StrAppend(&buffer_to_print,
                  ...
                  lnast_to->ref_name_std(lnast->get_name(curr_index)),
  ...
  absl::StrAppend(&buffer_to_print, lnast_to->ref_name_std(ref), lnast_to->for_cond_end());
...
void Code_gen::do_func_def(const mmap_lib::Tree_index& func_def_node_index) {
  ...
  auto cond_val = resolve_func_cond(curr_index).to_s();
  ...
  do_stmts(stmt_index);
  indendation--;
  absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->func_stmt_end(), lnast_to->func_end());
}
//-------------------------------------------------------------------------------------
...
void Code_gen::do_func_call(const mmap_lib::Tree_index& func_call_node_index) {
  ...
  absl::StrAppend(&buffer_to_print, indent().to_s());
  ...
  if (is_temp_var(lhs)) {  //|| !op_is_unary) {
    ...
    absl::StrAppend(&buffer_to_print, lhs.to_s(), " = ");  // lhs and assignment op to further assign the func name and arguments to lhs
    absl::StrAppend(&buffer_to_print, funcname.to_s());  // printitng the func name(the func called)
    absl::StrAppend(&buffer_to_print, lnast_to->ref_name_std(ref), lnast_to->stmt_sep());  // parameters for the func call
  }
}
//-------------------------------------------------------------------------------------
// Process the "if" node:
  ...
  while (curr_index != lnast->invalid_index()) {
    ...
    if (node_num > 2) {
      if (curr_node_type.is_ref() || curr_node_type.is_const()) {
        absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->start_else_if());
        do_cond(curr_index);
        ...
        if (!prev_was_cond) {
          absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->start_else());
        }
        indendation++;
        do_stmts(curr_index);
        indendation--;
        if (!prev_was_cond) {
          absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->end_if_or_else());
          if_closed = true;
        }
      }
    } else {
      if (curr_node_type.is_ref() || curr_node_type.is_const()) {
        absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->start_cond());
        do_cond(curr_index);
      } else if (curr_node_type.is_stmts()) {
        indendation++;
        do_stmts(curr_index);
...
void Code_gen::do_cond(const mmap_lib::Tree_index& cond_node_index) {
  ...
  absl::StrAppend(&buffer_to_print, lnast_to->ref_name_std(ref));
...
void Code_gen::do_op(const mmap_lib::Tree_index& op_node_index, const mmap_lib::str& op_type) {
  ...
  if (is_temp_var(key)) {  //|| !op_is_unary) {
    ...
    // absl::StrAppend (&buffer_to_print, indent(), lnast_to->ref_name_str(key), " ", lnast_to->debug_name_lang(op_node_data.type), " ",
    ...
    absl::StrAppend(&buffer_to_print,
                    ...
                    "=",
                    " ",
                    lnast_to->ref_name_std(mmap_lib::str(val)),
                    lnast_to->stmt_sep());
  }
}
...
void Code_gen::do_set_mask(const mmap_lib::Tree_index& smask_node_index) {
  ...
  if (is_temp_var(key)) {
    ...
    absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->ref_name_std(mmap_lib::str(val)), "=", smask_str_vect.back().to_s(), "\n");
...
void Code_gen::do_dot(const mmap_lib::Tree_index& dot_node_index, const mmap_lib::str& select_type) {
  ...
  if (is_temp_var(key)) {
    if (select_type == "tuple_add") {
      ...
      absl::StrAppend(&buffer_to_print, indent().to_s(), lnast_to->ref_name_std(mmap_lib::str(value)), " = ", key.to_s(), "\n");
    } else {
...
void Code_gen::do_tuple(const mmap_lib::Tree_index& tuple_node_index) {
  ...
  if (is_temp_var(key)) {
    ...
    fmt::print("key: {}\n tuple_value:{}\n", key, tuple_value);
    absl::StrAppend(&buffer_to_print, key.to_s(), " saved as ", tuple_value.to_s(), "\n");

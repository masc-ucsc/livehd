...
void Inou_code_gen::Declaration::format_raw(std::ostringstream &w) const {
...
void Inou_code_gen::iterate_declarations(Node_pin &pin) {
  (void)pin;
  I(false);
  /*
...
void Inou_code_gen::setup_declarations() {
  declarations.clear();
...
void Inou_code_gen::generate_prp(LGraph *g, std::string_view filename) {
  assert(lg == nullptr);
  lg = g;

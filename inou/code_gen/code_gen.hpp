...
public:
  ...
  void        do_assign(const mmap_lib::Tree_index& assign_node_index, std::vector<std::string>& hier_tup_vec,
  ...
  bool        is_temp_var(const std::string& test_string);      // can go to private/protected section!?
  bool        has_DblUndrScor(std::string_view test_string);    // can go to private/protected section!?
  bool        has_DblUndrScor(const std::string& test_string);  // can go to private/protected section!?
  ...
  bool           is_pos_int(const std::string& test_string);
  //  void invalid_node();

...
public:
  Driver_gen(const std::string& src, const std::map<std::string, Dut>& duts, const std::string& vcd_dir,
      ...
      : src_(src), duts_(duts), vcd_dir_(vcd_dir), args_(args) {}
  ...
  std::string emit(TSNode test, const std::string& name) {
    ...
    // Test parameters (`test name(params)`): declare each as a driver local,
    // its value taken from `--arg` when provided, else the parameter's default
    // expression; a parameter with neither is a hard error. Declared before the
    // body locals and removed from `locals_` so they are not re-declared with 0.
    for (const auto& [pname, pval] : read_params(test)) {
      o << "  long " << pname << " = " << pval << ";\n";
      locals_.erase(pname);
    }
...
private:
  // The test's `(params)`: each pair is (name, value-as-C++-expr). The value is
  // the `--arg` override when present, else the parameter's default; a parameter
  // with neither defined is a hard error (fail()).
  std::vector<std::pair<std::string, std::string>> read_params(TSNode test) {
    ...
    auto        flush = [&]() {
      ...
      // (05b-statements.md): the runner MUST supply the value, a `nil` reaching
      // the body is an error, never a silent 0.
      const bool has_default = !ts_node_is_null(pdef) && text_of(src_, pdef) != "nil";
      if (auto it = args_.find(pname); it != args_.end()) {
        val = it->second;  // --arg override
      } else if (has_default) {
        val = expr(pdef);  // the parameter's default expression
      } else {
        fail("test parameter '" + pname + "' has no default; pass --arg " + pname + "=<value>");
      }
      out.emplace_back(pname, val);
      have = false;
      pdef = TSNode{};
    };
  ...
  const std::map<std::string, std::string>& args_;  // --arg overrides for test parameters
...
int generate(const std::string& file, const std::string& simdir, const std::string& test_sel, const std::string& vcd_dir,
  ...
  Driver_gen gen(src, duts, vcd_dir, args);
  int        matched = 0;

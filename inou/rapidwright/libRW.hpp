...
public:
  ...
  void costum_Route(int design_ID, int src_ID, int snk_ID);
  int create_Module(int design_ID, int top_design_ID);
  int create_ModuleInst(char* modInst_name, int module_ID, int top_design_ID);
...
void graalThread::costum_Route(int design_ID, int src_ID, int snk_ID) {
  RW_costum_Route(thread, design_ID, src_ID, snk_ID);
}

int graalThread::create_Module(int design_ID, int top_design_ID) {
  RW_create_Module(thread, design_ID, top_design_ID);
}

int graalThread::create_ModuleInst(char* modInst_name, int module_ID, int top_design_ID) {
  RW_create_ModuleInst(thread, modInst_name, module_ID, top_design_ID);
}

...
static void process_cell_drivers_intialization(RTLIL::Module* mod, hhds::Graph* g) {
  ...
  for (auto cell : mod->cells()) {
    ...
    bool is_sub_cell = (cell->type.c_str()[0] == '\\' || strncmp(cell->type.c_str(), "$paramod\\", 8) == 0);
    ...
    while (pending_chain) {
      ...
      for (auto shift : shifts) {
        ...
        for (int pos = static_cast<int>(v.size()) - 1; pos >= 0; --pos) {
          auto i = v[pos];
          if (started) {
            wr_mask = wr_mask.shl_op(*Dlop::create_integer(1));
          }
          started = true;
        ...
        Dlop rd_mask;
        if (shift < 0) {
          rd_mask = wr_mask.shl_op(*Dlop::create_integer(-shift))->adjust_bits(wire->width);
        } else {
          rd_mask = wr_mask.sra_op(*Dlop::create_integer(shift))->adjust_bits(wire->width);
        }
      ...
      if (cell->hasParam(ID::CLR)) {
        ...
        if (cell->hasParam(ID::SRST_POLARITY)) {
          if (cell->getParam(ID::SRST_POLARITY).as_bool()) {
            setup_sink_by_name(exit_node, "negreset").connect_driver(create_const(*g, *Dlop::create_integer(0)));
          } else {
            setup_sink_by_name(exit_node, "negreset").connect_driver(create_const(*g, *Dlop::create_integer(1)));
          }
        }

...
static void look_for_wire(LGraph *g, const RTLIL::Wire *wire) {
  if (wire2pin.find(wire) != wire2pin.end()) return;
...
static Node_pin &get_edge_pin(LGraph *g, const RTLIL::Wire *wire) {
  ...
  if (wire2pin.find(wire) != wire2pin.end()) {
    if (wire->width != (int)wire2pin[wire].get_bits()) {
      fmt::print("w_name:{} w_w:{} | pid:{} bits:{}\n", wire->name.c_str(), wire->width, wire2pin[wire].get_pid(),
...
static Node_pin &create_pick_operator(LGraph *g, const RTLIL::Wire *wire, int offset, int width) {
  if (wire->width == width && offset == 0) return get_edge_pin(g, wire);
...
static bool is_black_box_output(const RTLIL::Module *module, const RTLIL::Cell *cell, const RTLIL::IdString &port_name) {
  ...
  // global input
  if (wire->port_input) return false;
...
static bool is_black_box_input(const RTLIL::Module *module, const RTLIL::Cell *cell, const RTLIL::IdString &port_name) {
  ...
  // global input
  if (wire->port_input) return true;
    ...
    for (const auto &conn : cell->connections()) {
      ...
      if (sub) {
        ...
        if (isdigit(pin_name[0])) {
          ...
          if (io_pin.dir == Sub_node::Direction::Input) continue;
          ...
          if (!sub->has_pin(pin_name) || sub->is_input(pin_name)) continue;
      ...
      if (ss.chunks().size() > 0) driver_pin.set_bits(ss.size());
        ...
        if (chunk.width == wire->width) {
          if (wire2pin.find(wire) != wire2pin.end()) {
            ...
            log("io wire %s from module %s cell type %s (%s vs %s)\n", wire->name.c_str(), module->name.c_str(), cell->type.c_str(),
                driver_pin.debug_name().c_str(), pin2.debug_name().c_str());
            if (wire->port_output) {
          ...
          if (partially_assigned.find(wire) == partially_assigned.end()) {
            ...
            if (wire2pin.find(wire) != wire2pin.end()) {
              printf("partial wire %s from module %s cell type %s (switching to partial)\n", wire->name.c_str(),
                     module->name.c_str(), cell->type.c_str());
            }
...
static Node_pin create_join_operator(LGraph *g, const RTLIL::SigSpec &ss) {
  ...
  for (auto &chunk : ss.chunks()) {
    if (chunk.wire == nullptr) {
      ...
      for (size_t i = dpin.get_bits(); i < chunk.data.size(); ++i) inp_pins.push_back(g->create_node_const(0).setup_driver_pin());
    } else {
...
static LGraph *process_module(RTLIL::Module *module, const std::string &path) {
  ...
  for (auto cell : module->cells()) {
    ...
    if (std::strncmp(cell->type.c_str(), "$and", 4) == 0 || std::strncmp(cell->type.c_str(), "$logic_and", 10) == 0 ||
      ...
      if (clk_polarity.size() && clk_polarity[0] != RTLIL::S1) connect_constant(g, 0, exit_node, 5);  // POL is 5 in SFlop_Op
      ...
      if (clk_polarity.size() && clk_polarity[0] != RTLIL::S1) connect_constant(g, 0, exit_node, 2);  // POL is 3 in Latch_Op
      ...
      if (size > 1) {
        ...
        for (size_t i = 1; i < size; ++i) g->add_edge(zero_pin, exit_node.setup_sink_pin(i));
      } else {
        exit_node.set_type(Not_Op);
        g->add_edge(entry_node.setup_driver_pin(0), exit_node.setup_sink_pin());
      }
      ...
      for (int i = 0; i < rd_clkp.size(); i++) {
        if (rd_clke[i] != RTLIL::S1) continue;

        if (rd_clkp[i] == RTLIL::S1) {
          if (rd_clk_enabled) I(rd_clk_polarity);  // All the read ports are equal
          rd_clk_enabled  = 1;
          rd_clk_polarity = 1;
      ...
      for (int i = 0; i < wr_clkp.size(); i++) {
        if (wr_clke[i] != RTLIL::S1) continue;
        if (wr_clkp[i] == RTLIL::S1) {
          if (wr_clk_enabled) I(wr_clk_polarity);  // All the read ports are equal
          wr_clk_enabled  = 1;
          wr_clk_polarity = 1;
      ...
      if (rd_clk_enabled) connect_constant(g, rd_clk_polarity, exit_node, LGRAPH_MEMOP_RDCLKPOL);
      if (wr_clk_enabled) connect_constant(g, wr_clk_polarity, exit_node, LGRAPH_MEMOP_WRCLKPOL);
      ...
      if (!inst_name.empty()) entry_node.set_name(inst_name);
#ifndef NDEBUG
      else
    ...
    for (auto &conn : cell->connections()) {
      ...
      if (entry_node.is_type(SubGraph_Op)) {
        ...
        if (std::strncmp(cell->type.c_str(), "$logic_and", 10) == 0 || std::strncmp(cell->type.c_str(), "$logic_or", 9) == 0) {
          if (ss.size() > 1) {
          ...
          sink_pid = 0;
          if (cell->parameters[conn.first.str() + "_SIGNED"].as_int() == 0) sink_pid += 1;
        } else if (entry_node.is_type(GreaterThan_Op) || entry_node.is_type(LessThan_Op) ||
                   entry_node.is_type(GreaterEqualThan_Op) || entry_node.is_type(LessEqualThan_Op) || entry_node.is_type(Div_Op) ||
  ...
  for (auto wire : module->wires()) {
    if (wire->port_output && wire2pin.find(wire) != wire2pin.end()) {
      ...
      if (dpin.is_graph_output()) continue;
...
struct Yosys2lg_Pass : public Yosys::Pass {
  ...
  virtual void execute(std::vector<std::string> args, RTLIL::Design *design) {
    ...
    for (auto &it : design->modules_) {
      ...
      for (auto wire : module->wires()) {
        if (wire->port_input) driven_signals.insert(wire->hash());
      }

      for (auto it : module->connections()) {
        for (auto &chunk : it.first.chunks()) {
          const RTLIL::Wire *wire = chunk.wire;
          if (wire) driven_signals.insert(wire->hash());
        }
      }
        ...
        if (!ct_all.cell_known(cell->type)) {
          ...
          for (auto &conn : cell->connections()) {
            for (auto &chunk : conn.second.chunks()) {
              ...
              if (cell_port_inputs.count(cell_port)) continue;
              ...
              if (!is_input && !is_output) is_input = driven_signals.count(wire->hash()) > 0;
              ...
              if (!is_input && !is_output && str[0] == '\\') {
                ...
                if (str[0] == 'A' || str[0] == 'B' || str[0] == 'C' || str[0] == 'D' || str[0] == 'S') {
                  if (isdigit(str[1]) || str[1] == 0) is_input = true;
                }
                if (str[0] == 'a' || str[0] == 'b' || str[0] == 'c' || str[0] == 'd' || str[0] == 's') {
                  if ((isdigit(str[1]) && str[2] == 0) || str[1] == 0) is_input = true;
                }
                ...
                if ((strncmp(cell->type.c_str(), "\\sa", 3) == 0) && (strcmp(str, "BC2") == 0 || strncmp(str, "DFT", 3) == 0 ||
                                                                      strncmp(str, "SO_CNT", 6) == 0 || strcmp(str, "SO_D") == 0))
                  is_input = true;
                ...
                if (strncmp(str, "io_is", 5) == 0 || strncmp(str, "io_in", 5) == 0) is_input = true;
                ...
                if (strncmp(str, "IN", 2) == 0 || strncmp(str, "in", 2) == 0) is_input = true;

                if (str[1] == 0 && (str[0] == 'X' || str[0] == 'Y' || str[0] == 'Z' || str[0] == 'Q')) is_output = true;
                if (str[1] == 0 && (str[0] == 'x' || str[0] == 'y' || str[0] == 'z' || str[0] == 'q')) is_output = true;

                if (cell->type.str()[1] == 'H' && cell->type.str()[2] == 'D') {
                  if (strcmp(str, "CON") == 0 || strcmp(str, "SN") == 0) is_output = true;
                }
                ...
                if (strncmp(str, "OU", 2) == 0 || strncmp(str, "ou", 2) == 0) is_output = true;
                ...
                if (strncmp(cell->type.c_str(), "\\sa", 3) == 0 && strstr(cell->type.c_str(), "rw")) {
                  if (strcmp(str, "RSCOUT") == 0 || strcmp(str, "SO_CN") == 0 || strcmp(str, "SO_Q") == 0) is_output = true;
                  if (!is_output) is_input = true;
                }
              }
              ...
              if (is_input) cell_port_inputs.insert(cell_port);
              if (is_output) cell_port_outputs.insert(cell_port);
            }
          }
        }
      }

...
void Lgyosys_dump::create_blackbox(const LGraph &subgraph, RTLIL::Design *design) {
  ...
  for(auto &nid : subgraph.fast()) {
    if(subgraph.node_type_get(nid).op != GraphIO_Op)
      continue;
    std::string name = "\\";
...
void Lgyosys_dump::create_wires(const LGraph *g, RTLIL::Module *module) {
  ...
  for(auto idx : g->fast()) {
    ...
    if(g->node_type_get(idx).op == GraphIO_Op)
    ...
    const auto node = g->get_node(idx);
...
void Lgyosys_dump::to_yosys(const LGraph *g) {
  ...
  for(auto nid : g->fast()) {
    auto op = g->node_type_get(nid).op;
    if (op == GraphIO_Op)

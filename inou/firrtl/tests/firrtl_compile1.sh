...
firrtl_test() {
  ...
  do
    ...
    #for code_gen
    ${LGSHELL} "inou.firrtl.tolnast files:${PATTERN_PATH}/${pt}.${FIRRTL_LEVEL}.pb |> pass.compiler gviz:true top:${pt} firrtl:true path:lgdb2 odir:gvizes_firrtl"
    ret_val=$?
    if [ $ret_val -ne 0 ]; then
      echo "ERROR: could not compile with pattern: ${pt}.${FIRRTL_LEVEL}.pb!"
      exit $ret_val
    fi
  dot -Tpdf -o gvizes_firrtl/${pt}.dot.pdf gvizes_firrtl/${pt}.dot 
    ${LGSHELL} "lgraph.open name:${pt} path:lgdb2 |> pass.lnast_fromlg |> lnast.dump |> inou.code_gen.prp odir:tmp_prp_firrtl"

    ret_val=$?
    if [ $ret_val -ne 0 ]; then
      echo "ERROR: could not compile LG from lgdb2 with pattern:${pt}.${FIRRTL_LEVEL}.pb"
      exit ret_val
    fi

    ${LGSHELL} "inou.pyrope files:tmp_prp_firrtl/${pt}.prp |> pass.compiler top:${pt} path:lgdb |> lgraph.dump"
  done #end of for
  ...
  do
    ...
    rm -rf tmp_firrtl
    ${LGSHELL} "lgraph.open name:${pt} hier:true path:lgdb |> inou.cgen.verilog odir:tmp_firrtl"

...
protected:
  ...
  void add_port_to_map(std::string_view mod_id, const firrtl::FirrtlPB_Type &type, uint8_t dir, bool flipped,
...
protected:
  ...
  void        wire_init_flip_handling(Lnast &lnast, const firrtl::FirrtlPB_Type &type, std::string id, bool flipped,
                                      Lnast_nid &parent_node, const firrtl::FirrtlPB_Statement &stmt);
  void        handle_register(Lnast &lnast, const firrtl::FirrtlPB_Type &type, std::string id, Lnast_nid &parent_node,
                              const firrtl::FirrtlPB_Statement &stmt);
  static void dump_var2flip(
      const absl::flat_hash_map<std::string, absl::flat_hash_set<std::pair<std::string, bool>>> &module_table);
  ...
  void handle_mux_assign(Lnast &lnast, const firrtl::FirrtlPB_Expression &expr, Lnast_nid &parent_node, std::string_view lhs_of_asg,
                         const firrtl::FirrtlPB_Statement &stmt);
  ...
  void handle_pad_op(Lnast &lnast, const firrtl::FirrtlPB_Expression_PrimOp &op, Lnast_nid &parent_node, std::string_view lhs,
                     const firrtl::FirrtlPB_Statement &stmt);
  void handle_binary_op(Lnast &lnast, const firrtl::FirrtlPB_Expression_PrimOp &op, Lnast_nid &parent_node, std::string_view lhs,
                        const firrtl::FirrtlPB_Statement &stmt);
  ...
  void attach_expr_str2node(Lnast &lnast, std::string_view access_str, Lnast_nid &parent_node,
                            const firrtl::FirrtlPB_Statement &stmt);
  void tuple_flattened_connections(Lnast &lnast, Lnast_nid &parent_node, std::string_view lhs_head, std::string_view rhs_head,
                                   std::string_view flattened_element, bool is_flipped, const firrtl::FirrtlPB_Statement &stmt);
  void tuple_flattened_connections_instance_l(Lnast &lnast, Lnast_nid &parent_node, std::string_view tup_hier_name,
                                              std::string_view hier_name_r, bool is_flipped, bool is_input,
                                              const firrtl::FirrtlPB_Statement &stmt);
  void tuple_flattened_connections_instance_r(Lnast &lnast, Lnast_nid &parent_node, std::string_view tup_hier_name,
                                              std::string_view hier_name_r, bool is_flipped, bool is_output,
                                              const firrtl::FirrtlPB_Statement &stmt);
  void handle_lhs_instance_connections(Lnast &lnast, Lnast_nid &parent_node, std::string_view tup_head_l,
                                       std::string_view hier_name_l, std::string_view hier_name_r,
                                       const firrtl::FirrtlPB_Statement &stmt);
  void handle_rhs_instance_connections(Lnast &lnast, Lnast_nid &parent_node, std::string_view tup_head_l,
                                       std::string_view hier_name_l, std::string_view hier_name_r,
                                       const firrtl::FirrtlPB_Statement &stmt);
  void handle_normal_cases_wire_connections(Lnast &lnast, Lnast_nid &parent_node, std::string_view tup_head_l,
                                            std::string_view hier_name_l, std::string_view hier_name_r,
                                            const firrtl::FirrtlPB_Statement &stmt);
  void handle_direct_inp_out_or_node_connection(Lnast &lnast, Lnast_nid &parent_node, std::string_view hier_name_l,
                                                std::string_view hier_name_r);

  bool check_submodule_io_flipness(Lnast &lnast, std::string_view submodule_name, std::string_view tup_head,
                                   std::string_view hier_name, bool is_sub_instance = false);
  ...
  void create_tuple_get_from_str(Lnast &ln, Lnast_nid &parent_node, std::string_view flattened_str, const Lnast_node &target_node,
                                 const firrtl::FirrtlPB_Statement &stmt);
  void create_tuple_add_for_instance_itup(Lnast &ln, Lnast_nid &parent_node, std::string_view lhs_full_name,
                                          std::string rhs_full_name, const firrtl::FirrtlPB_Statement &stmt);
  void create_wires_for_instance_itup(Lnast &ln, Lnast_nid &parent_node, std::string_view lhs_full_name,
                                      std::string_view rhs_full_name);
  void create_tuple_get_for_instance_otup(Lnast &ln, Lnast_nid &parent_node, std::string_view rhs_full_name,
  ...
  void     handle_rhs_runtime_idx(Lnast &lnast, Lnast_nid &parent_node, std::string_view hier_name_l, std::string_view hier_name_r,
                                  const firrtl::FirrtlPB_Expression &rhs_expr, const firrtl::FirrtlPB_Statement &stmt);
  void     handle_lhs_runtime_idx(Lnast &lnast, Lnast_nid &parent_node, std::string_view hier_name_l, std::string_view hier_name_r,
                                  const firrtl::FirrtlPB_Expression &rhs_expr, const firrtl::FirrtlPB_Statement &stmt);

...
mmap_lib::str Inou_firrtl::get_full_name(const mmap_lib::str &term, const bool is_rhs) {
  ...
  auto term2              = term.get_str_before_first('.');

  if (reg2qpin.count(term2)) {  // check if the register exists
    I(reg2qpin[term2].substr(0, 3) == "_#_");
    if (is_rhs) {
      auto term_rest = term.get_str_after_first('.');
...
void Inou_firrtl::create_bitwidth_dot_node(Lnast &lnast, uint32_t bitwidth, Lnast_nid &parent_node, const mmap_lib::str &port_id,
                                           bool is_signed) {
...
void Inou_firrtl::setup_register_bits(Lnast &lnast, const firrtl::FirrtlPB_Type &type, const mmap_lib::str &id,
...
void Inou_firrtl::setup_register_bits_scalar(Lnast &lnast, const mmap_lib::str &id, uint32_t bitwidth, Lnast_nid &parent_node,
                                             bool is_signed) {
...
void Inou_firrtl::collect_memory_data_struct_hierarchy(const mmap_lib::str &mem_name, const firrtl::FirrtlPB_Type &type_in,
...
void Inou_firrtl::InitCMemory(Lnast &lnast, Lnast_nid &parent_node, const firrtl::FirrtlPB_Statement_CMemory &cmem) {
  ...
  if (din_type.type_case() == firrtl::FirrtlPB_Type::kBundleType || din_type.type_case() == firrtl::FirrtlPB_Type::kVectorType) {
    ...
    mem2din_fields[mmap_lib::str(cmem.id())].emplace_back(mmap_lib::str::concat(".", bits));  // encode .bits at the end of hier-fields
  } else {
    I(false);
  }
  ...
  lnast.add_child(idx_attr_get, Lnast_node::create_ref(mmap_lib::str::concat(cmem.id(), "_interface_args"_str)));
  ...
  lnast.add_child(idx_asg_mwensize, Lnast_node::create_const(mmap_lib::str(wensize_init)));
...
void Inou_firrtl::HandleMportDeclaration(Lnast &lnast, Lnast_nid &parent_node, const firrtl::FirrtlPB_Statement_MemoryPort &mport) {
  ...
  lnast.add_child(idx_ta_maddr_ini, Lnast_node::create_const(0));
  ...
  lnast.add_child(idx_ta_men_ini, Lnast_node::create_const(0));
  ...
  lnast.add_child(idx_ta_mlat_ini, Lnast_node::create_const(0));
...
void Inou_firrtl::InitMemRes(Lnast &lnast, const mmap_lib::str &mem_name, const mmap_lib::str &port_cnt_str) {
  I(mem2initial_idx.find(mem_name) != mem2initial_idx.end());
  I(mem2din_fields.find(mem_name) != mem2din_fields.end());

  const auto &idx_initialize_stmts = mem2initial_idx[mem_name];
  const auto &hier_full_names      = mem2din_fields[mem_name];
  const auto  mem_res_str          = mmap_lib::str::concat(mem_name, "_res");
  ...
  for (const auto &hier_full_name : hier_full_names) {  // hier_full_name example: foo.bar.baz.20, the last field is bit
    ...
    if (hier_sub_names.size() == 1) {  // it's a pure sclalar memory dout
      ...
      lnast.add_child(idx_ta, Lnast_node::create_const(hier_sub_names[0]));
    } else {
      auto idx_ta = lnast.add_child(idx_initialize_stmts, Lnast_node::create_tuple_add());
      lnast.add_child(idx_ta, Lnast_node::create_ref(mem_res_str));
      lnast.add_child(idx_ta, Lnast_node::create_const(port_cnt_str));
...
void Inou_firrtl::InitMemDin(Lnast &lnast, const mmap_lib::str &mem_name, const mmap_lib::str &port_cnt_str) {
  ...
  if (it->second.front() == '.') {  // din is scalar, the din_fields starts with something like .17
    ...
    lnast.add_child(idx_ta_mdin_ini, Lnast_node::create_const(0));
  } else {  // din is tuple
    auto &hier_full_names = mem2din_fields[mem_name];
    for (const auto &hier_full_name : hier_full_names) {  // hier_full_name example: foo.bar.baz.20, the last field is bit
      fmt::print("hier_name:{}\n", hier_full_name);
      std::vector<mmap_lib::str> hier_sub_names;
      auto  found = hier_full_name.rfind('.');
...
void Inou_firrtl::create_module_inst(Lnast &lnast, const firrtl::FirrtlPB_Statement_Instance &inst, Lnast_nid &parent_node) {
  ...
  auto inst_name = mmap_lib::str(inst.id());
...
void Inou_firrtl::HandleBundVecAcc(Lnast &lnast, const firrtl::FirrtlPB_Expression &expr, Lnast_nid &parent_node, const bool is_rhs,
  ...
  if (alter_full_str.front() == '$') {
    flattened_str = flattened_str.prepend('$');
  } else if (alter_full_str.front() == '%') {
    flattened_str = flattened_str.prepend('%');
...
void Inou_firrtl::split_hier_name(const mmap_lib::str &full_name, std::vector<mmap_lib::str> &hier_subnames) {
  ...
  while ((pos = full_name.rfind(".[", prev)) != std::string::npos) {
    if (pos > prev) {
...
void Inou_firrtl::CreateTupGetFromStr(Lnast &ln, Lnast_nid &parent_node, const mmap_lib::str &full_name,
...
void Inou_firrtl::CreateTupAddFromStr(Lnast &ln, Lnast_nid &parent_node, const mmap_lib::str &full_name,
                                      const Lnast_node &value_node) {
  I(full_name.contains('.'));
...
void Inou_firrtl::ListPortInfo(Lnast &lnast, const firrtl::FirrtlPB_Port &port, Lnast_nid parent_node) {
  ...
  for (auto val : port_list) {
    ...
    if (port_dir == firrtl::FirrtlPB_Port_Direction::FirrtlPB_Port_Direction_PORT_DIRECTION_IN) {
      ...
      full_port_name = port_name.prepend('$');
...
mmap_lib::str Inou_firrtl::ReturnExprString(Lnast &lnast, const firrtl::FirrtlPB_Expression &expr, Lnast_nid &parent_node,
...
void Inou_firrtl::AttachExprStrToNode(Lnast &lnast, const mmap_lib::str &access_str, Lnast_nid &parent_node) {
  ...
  auto first_char = access_str.front();
...
void Inou_firrtl::declare_register(Lnast &lnast, Lnast_nid &parent_node, const firrtl::FirrtlPB_Statement &stmt) {
  ...
  mmap_lib::str tmp_str("__create_flop");
  lnast.add_child(idx_attget, Lnast_node::create_const(tmp_str));
...
void Inou_firrtl::ListStatementInfo(Lnast &lnast, const firrtl::FirrtlPB_Statement &stmt, Lnast_nid &parent_node) {
  switch (stmt.statement_case()) {
    case firrtl::FirrtlPB_Statement::kWire: {
      wire_names.insert(mmap_lib::str(stmt.wire().id()));
      init_wire_dots(lnast, stmt.wire().type(), mmap_lib::str(stmt.wire().id()), parent_node);
      break;
    }
    ...
    case firrtl::FirrtlPB_Statement::kPrintf: {  // Printf
      // FIXME: we should convert to a __puts call
      break;
    }
...
void Inou_firrtl::PopulateAllModsIO(Eprp_var &var, const firrtl::FirrtlPB_Circuit &circuit, const mmap_lib::str &file_name) {
  for (int i = 0; i < circuit.module_size(); i++) {
    if (circuit.module(i).has_external_module()) {
      ...
      auto     sub     = AddModToLibrary(var, mmap_lib::str(circuit.module(i).external_module().id()), file_name);
      uint64_t inp_pos = 0;
      uint64_t out_pos = 0;
      for (int j = 0; j < circuit.module(i).external_module().port_size(); j++) {
        auto port = circuit.module(i).external_module().port(j);
        AddPortToMap(
            mmap_lib::str(circuit.module(i).external_module().id())
            ,port.type()
            ,port.direction()
            ,mmap_lib::str(port.id())
            ,sub
            ,inp_pos
            ,out_pos
            );
      ...
      auto     sub     = AddModToLibrary(var, mmap_lib::str(circuit.module(i).user_module().id()), file_name);
      uint64_t inp_pos = 0;
      uint64_t out_pos = 0;
      for (int j = 0; j < circuit.module(i).user_module().port_size(); j++) {
        auto port = circuit.module(i).user_module().port(j);
        AddPortToMap(mmap_lib::str(circuit.module(i).user_module().id()), port.type(), port.direction(), mmap_lib::str(port.id()), sub, inp_pos, out_pos);
      }
    } else {
      Pass::error("Module not set.");
    }

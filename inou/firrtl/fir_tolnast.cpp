...
//----------------Helper Functions--------------------------
mmap_lib::str Inou_firrtl_module::create_tmp_var() {
  return mmap_lib::str::concat(mmap_lib::str("___F"), ++tmp_var_cnt);
}

mmap_lib::str Inou_firrtl_module::create_tmp_mut_var() {
...
mmap_lib::str Inou_firrtl_module::get_full_name(const mmap_lib::str &term, const bool is_rhs) {
  auto term2     = term;
...
void Inou_firrtl_module::create_bitwidth_dot_node(Lnast& lnast, uint32_t bitwidth, Lnast_nid& parent_node, const mmap_lib::str &port_id, bool is_signed) {
  ...
  create_tuple_add_from_str(lnast, parent_node, mmap_lib::str::concat(port_id, extension), value_node);
}
...
void Inou_firrtl_module::init_wire_dots(Lnast& lnast, const firrtl::FirrtlPB_Type &type, const mmap_lib::str &id, Lnast_nid &parent_node) {
  switch (type.type_case()) {
    case firrtl::FirrtlPB_Type::kBundleType: {  // Bundle Type
...
void Inou_firrtl_module::setup_register_bits(Lnast& lnast, const firrtl::FirrtlPB_Type& type, const mmap_lib::str& id, Lnast_nid& parent_node) {
  switch (type.type_case()) {
    case firrtl::FirrtlPB_Type::kBundleType: {  // Bundle Type
      for (int i = 0; i < type.bundle_type().field_size(); i++) {
...
void Inou_firrtl_module::setup_register_bits_scalar(Lnast& lnast, const mmap_lib::str& id, uint32_t bitwidth, Lnast_nid& parent_node, bool is_signed) {
  // Specify __bits, if bitwidth is explicit
  if (bitwidth > 0) {
    auto value_node = Lnast_node::create_const(bitwidth);
    auto extension  = is_signed ? ".__sbits" : ".__ubits";
    create_tuple_add_from_str(lnast, parent_node, mmap_lib::str::concat(id, extension), value_node);
  }
}

void Inou_firrtl_module::collect_memory_data_struct_hierarchy(const mmap_lib::str& mem_name, const firrtl::FirrtlPB_Type& type_in, const mmap_lib::str& hier_fields_concats) {
...
void Inou_firrtl_module::handle_mport_declaration(Lnast& lnast, Lnast_nid& parent_node, const firrtl::FirrtlPB_Statement_MemoryPort& mport) {
  // (void) parent_node;
...
void Inou_firrtl_module::init_mem_res(Lnast& lnast, const mmap_lib::str& mem_name, const mmap_lib::str& port_cnt_str) {
  I(mem2initial_idx.find(mem_name) != mem2initial_idx.end());
  auto& idx_initialize_stmts = mem2initial_idx[mem_name];
...
void Inou_firrtl_module::init_mem_din(Lnast& lnast, const mmap_lib::str& mem_name, const mmap_lib::str& port_cnt_str) {
  auto default_val_str = 0;
  I(mem2initial_idx.find(mem_name) != mem2initial_idx.end());
  auto& idx_initialize_stmts = mem2initial_idx[mem_name];
  auto  it                   = mem2din_fields.find(mem_name);
...
void Inou_firrtl_module::handle_mux_assign(Lnast& lnast, const firrtl::FirrtlPB_Expression& expr, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
...
void Inou_firrtl_module::handle_valid_if_assign(Lnast& lnast, const firrtl::FirrtlPB_Expression& expr, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
...
// ----------------- primitive op start -------------------------------------------------------------------------------
void Inou_firrtl_module::handle_unary_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1);
...
void Inou_firrtl_module::handle_and_reduce_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1);
...
void Inou_firrtl_module::handle_or_reduce_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1);
...
void Inou_firrtl_module::handle_xor_reduce_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1);
...
void Inou_firrtl_module::handle_negate_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1);
...
void Inou_firrtl_module::handle_conv_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1);
...
void Inou_firrtl_module::handle_extract_bits_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
...
void Inou_firrtl_module::handle_head_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
...
void Inou_firrtl_module::handle_tail_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1 && op.const__size() == 1);
  auto lhs_str = lhs;
  auto e1_str  = return_expr_str(lnast, op.arg(0), parent_node, true);
...
void Inou_firrtl_module::handle_concat_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 2);
...
void Inou_firrtl_module::handle_pad_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1 && op.const__size() == 1);
...
void Inou_firrtl_module::handle_two_expr_prime_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 2);
  auto      e1_str = return_expr_str(lnast, op.arg(0), parent_node, true);
  auto      e2_str = return_expr_str(lnast, op.arg(1), parent_node, true);
...
void Inou_firrtl_module::handle_static_shift_op(Lnast& lnast, const firrtl::FirrtlPB_Expression_PrimOp& op, Lnast_nid& parent_node, const mmap_lib::str& lhs) {
  I(lnast.get_data(parent_node).type.is_stmts());
  I(op.arg_size() == 1 || op.const__size() == 1);
...
void Inou_firrtl_module::handle_bundle_vec_acc(Lnast& lnast, const firrtl::FirrtlPB_Expression& expr, Lnast_nid& parent_node, const bool is_rhs,
...
void Inou_firrtl_module::handle_rd_mport_usage(Lnast& lnast, Lnast_nid& parent_node, const mmap_lib::str& mport_name) {
...
void Inou_firrtl_module::handle_wr_mport_usage(Lnast& lnast, Lnast_nid& parent_node, const mmap_lib::str& mport_name) {
...
void Inou_firrtl_module::set_leaf_type(const mmap_lib::str &subname, const mmap_lib::str &full_name, size_t prev,
...
void Inou_firrtl_module::split_hier_name(const mmap_lib::str &full_name, std::vector<mmap_lib::str>& hier_subnames) {
  std::size_t prev = 0;
  std::size_t pos;
...
void Inou_firrtl_module::split_hier_name(const mmap_lib::str& full_name,
                                  std::vector<std::pair<mmap_lib::str, Inou_firrtl_module::Leaf_type>>& hier_subnames) {
  std::size_t prev = 0;
  std::size_t pos;
...
void Inou_firrtl_module::create_tuple_get_from_str(Lnast& ln, Lnast_nid& parent_node, const mmap_lib::str& full_name, const Lnast_node& dest_node) {
...
void Inou_firrtl_module::create_tuple_add_from_str(Lnast& ln, Lnast_nid& parent_node, const mmap_lib::str& full_name, const Lnast_node& value_node) {
  I(full_name.contains('.'));
...
mmap_lib::str Inou_firrtl_module::flatten_expression(Lnast& ln, Lnast_nid& parent_node, const firrtl::FirrtlPB_Expression& expr) {
  if (expr.has_sub_field()) {
    return mmap_lib::str::concat(flatten_expression(ln, parent_node, expr.sub_field().expression()), ".", expr.sub_field().field());

  } else if (expr.has_sub_access()) {
    auto idx_str = return_expr_str(ln, expr.sub_access().index(), parent_node, true);
    return mmap_lib::str::concat(flatten_expression(ln, parent_node, expr.sub_access().expression()), ".", idx_str);

  } else if (expr.has_sub_index()) {
    return mmap_lib::str::concat(flatten_expression(ln, parent_node, expr.sub_index().expression()), ".", expr.sub_index().index().value());
...
void Inou_firrtl_module::list_port_info(Lnast& lnast, const firrtl::FirrtlPB_Port& port, Lnast_nid parent_node) {
  ...
  Inou_firrtl::create_io_list(port.type(), port.direction(), mmap_lib::str(port.id()), port_list);
  ...
  for (auto val : port_list) {
    ...
    if (port_bits > 0) {  // Specify __bits
      ...
      create_tuple_add_from_str(lnast, parent_node, mmap_lib::str::concat(full_port_name, extension), value_node);
    }
  }
}


void Inou_firrtl_module::record_all_input_hierarchy(const mmap_lib::str &port_name) {
...
void Inou_firrtl_module::record_all_output_hierarchy(const mmap_lib::str &port_name) {
  std::size_t pos = port_name.size();
  while (pos != std::string::npos) {
...
mmap_lib::str Inou_firrtl_module::return_expr_str(Lnast& lnast, const firrtl::FirrtlPB_Expression& expr, Lnast_nid& parent_node, const bool is_rhs, const Lnast_node value_node) {
  I(lnast.get_data(parent_node).type.is_stmts());
...
void Inou_firrtl_module::attach_expr_str2node(Lnast& lnast, const mmap_lib::str &access_str, Lnast_nid& parent_node) {
  I(!lnast.get_data(parent_node).type.is_stmts());
...
void Inou_firrtl_module::setup_register_reset_init(Lnast& lnast, Lnast_nid& parent_node, const mmap_lib::str& reg_raw_name,
  ...
  if (!value_node.is_invalid())
    create_tuple_add_from_str(lnast, parent_node, mmap_lib::str::concat("#", reg_raw_name, ".__reset"), value_node);
  ...
  if (!initial_node.is_invalid())
    create_tuple_add_from_str(lnast, parent_node, mmap_lib::str::concat("#", reg_raw_name, ".__initial"), initial_node);
}
...
void Inou_firrtl_module::list_statement_info(Lnast& lnast, const firrtl::FirrtlPB_Statement& stmt, Lnast_nid& parent_node) {
  switch (stmt.statement_case()) {
    ...
    case firrtl::FirrtlPB_Statement::kNode: {  // Node -- nodes are simply named intermediates in a circuit
      init_expr_add(lnast, stmt.node().expression(), parent_node, mmap_lib::str(stmt.node().id()));
      break;
    }
...
void Inou_firrtl::user_module_to_lnast(Eprp_var& var, const firrtl::FirrtlPB_Module& fmodule, const mmap_lib::str& file_name) {
...
void Inou_firrtl::populate_all_mods_io(Eprp_var& var, const firrtl::FirrtlPB_Circuit& circuit, const mmap_lib::str& file_name) {
  for (int i = 0; i < circuit.module_size(); i++) {
    ...
    if (circuit.module(i).has_external_module()) {
      ...
      for (int j = 0; j < circuit.module(i).external_module().port_size(); j++) {
        ...
        add_port_to_map(module_i_external_module_id, port.type(), port.direction(), mmap_lib::str(port.id()), sub, inp_pos, out_pos);
      }
      continue;
      ...
      for (int j = 0; j < circuit.module(i).user_module().port_size(); j++) {
        ...
        add_port_to_map(module_i_user_module_id, port.type(), port.direction(), mmap_lib::str(port.id()), sub, inp_pos, out_pos);
      }
    } else {
...
Sub_node Inou_firrtl::add_mod_to_library(Eprp_var& var, const mmap_lib::str& mod_name, const mmap_lib::str& file_name) {
  mmap_lib::str fpath;
...
void Inou_firrtl::add_port_sub(Sub_node& sub, uint64_t& inp_pos, uint64_t& out_pos, const mmap_lib::str& port_id,
                               const uint8_t& dir) {
  if (dir == 1) {                // PORT_DIRECTION_IN
...
void Inou_firrtl::add_port_to_map(const mmap_lib::str& mod_id, const firrtl::FirrtlPB_Type& type, uint8_t dir,
  ...
  switch (type.type_case()) {
    ...
    case firrtl::FirrtlPB_Type::kBundleType: {  // Bundle type
      ...
      for (int i = 0; i < type.bundle_type().field_size(); i++) {
        if (btype.field(i).is_flipped()) {
          ...
          I(new_dir != 0);
          add_port_to_map(mod_id, btype.field(i).type(), new_dir, mmap_lib::str::concat(port_id, ".", btype.field(i).id()), sub, inp_pos, out_pos);
        } else {
          add_port_to_map(mod_id, btype.field(i).type(), dir, mmap_lib::str::concat(port_id, ".", btype.field(i).id()), sub, inp_pos, out_pos);
        }
      }
      break;
    }
    case firrtl::FirrtlPB_Type::kVectorType: {  // Vector type
      glob_info.mod_to_io_dir_map[std::make_pair(mod_id, port_id)] = dir;
      for (uint32_t i = 0; i < type.vector_type().size(); i++) {
        add_port_to_map(mod_id, type.vector_type().type(), dir, mmap_lib::str::concat(port_id, ".", i), sub, inp_pos, out_pos);
      }
      break;
    }
    case firrtl::FirrtlPB_Type::kFixedType: {  // Fixed type
...
void Inou_firrtl::grab_ext_module_info(const firrtl::FirrtlPB_Module_ExternalModule &emod) {
  ...
  for (int j = 0; j < emod.parameter_size(); j++) {
    ...
    glob_info.emod_to_param_map[mmap_lib::str(emod.defined_name())].insert({mmap_lib::str(emod.parameter(j).id()), param_str});
  }

  // Add them to the map to let us know what ports exist in this module.
  for (const auto &elem : port_list) {
    glob_info.mod_to_io_dir_map[std::make_pair(mmap_lib::str(emod.defined_name()), mmap_lib::str(std::get<0>(elem)))] = std::get<1>(elem);
...
mmap_lib::str Inou_firrtl::convert_bigint_to_str(const firrtl::FirrtlPB_BigInt &bigint) {
...
void Inou_firrtl::iterate_modules(Eprp_var& var, const firrtl::FirrtlPB_Circuit& circuit, const mmap_lib::str& file_name) {

...
Inou_json_options_pack::Inou_json_options_pack() {
	...
	Options::get_desc()->add_options()
					("json_output,o", boost::program_options::value(&json_output), "json output <filename> for graph")
					("json_input,i", boost::program_options::value(&json_input), "json input <filename> for graph");

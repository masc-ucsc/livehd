...
int Destination::sat_operator_internal_string_input(int oper_type, int a, std::string b, ezMiniSAT &sat) {
  ...
  std::cout << "Input A in sat opertaor internal function  is : " << sat.to_string(a).c_str() << "and string input2 is " << b << std::endl;
  ...
  if(child_sat == 2) {
    ...
    case Sum_Op: {
      ...
      sat_val = sat.OR(a, b);
      std::cout << "Formula is SUM: " << sat.to_string(sat_val).c_str() << std::endl;
      break;
    }

    case Mult_Op: {
      std::cout << "Formula is  MULT: " << sat.to_string(a).c_str() << std::endl;
      sat_val = sat.AND(a, b);
      break;
    }
...
void Destination::Increment_and_Add_Child_with_sat(int node_B_ID, std::string input2, ezMiniSAT &sat, int operand_type) {
  ...
  if(Get_Child_Cnt() == 1) {
    ...
    switch(operand_type) {
    case Sum_Op: {
      sat_result = sat.OR(input1, input2);
      std::cout << std::endl;
...
int main(int argc, const char **argv) {
    ...
    for(auto idx : g->backward()) {
      ...
      for(const auto &c : g->inp_edges(idx)) {
        ...
        if(IS_operator(oprator_A)) {
          ...
          if(dest_list_1[node_B_ID].Get_Child_Cnt() == 0 || dest_list_1[node_B_ID].Get_Child_sat_Cnt() == 0)
            dest_list_1[node_B_ID].set_is_operator_1st_input();
        }

        if(g->is_graph_input(node_A_ID)) {
          if(dest_list_1[node_B_ID].Get_Child_Cnt() == 1 || dest_list_1[node_B_ID].Get_Child_sat_Cnt() == 1)
        ...
        if(g->is_graph_input(node_A_ID)) {
          ...
          if(dest_list_1[node_B_ID].Get_Child_Cnt() == 2) {
            ...
            std::cout << "\n current parent child sat No for parentID:" << cur_parent << " is: " << dest_list_1[cur_parent].Get_Child_sat_Cnt() << std::endl;
            ...
            if(dest_list_1[cur_parent].Get_Child_sat_Cnt() == 1) {
              if(dest_list_1[cur_parent].Is_input_2nd_operand_sat()) {
                ...
                std::cout << "\n current parent child sat(1) for " << cur_parent << " is :" << dest_list_1[cur_parent].Get_Child_sat_Cnt() << std::endl;
              }
            }
          ...
          if(IS_operator(operand_type_B)) {
            std::cout << " Outside Loop Interation ......\n";
            int i = 1;
            std::cout << " child sat count of nodeID: " << cur_parent << " is: " << dest_list_1[cur_parent].Get_Child_sat_Cnt() << std::endl;

            while(dest_list_1[cur_parent].Get_Child_sat_Cnt() == 2) {
              //std::cout<<" Inside Loop Interation ......\n";
              std::cout << " When Current_Parent child sat count is " << cur_parent << ":" << dest_list_1[cur_parent].Get_Child_sat_Cnt() << std::endl;
              int input1 = 0;
              ...
              if(dest_list_1[cur_parent].Get_1st_Operand() == "SAT_RESULT") {
                ...
                std::cout << " PRINTING Formula for SAT_RESULT input2 for node ID" << cur_parent << sat.to_string(sat_result_formula_1).c_str() << std::endl;
...

              {
                ...
                std::cout << " Result for nodeID:*******" << cur_parent << sat.to_string(sat_result_formula_1).c_str() << std::endl;
                dest_list_1[cur_parent].Set_result(sat_result_formula_1);
              }
              ...
              if(dest_list_1[cur_parent].Get_Child_sat_Cnt() == 1) {
                ...
                if(dest_list_1[cur_parent].Is_input_2nd_operand_sat()) {
                  ...
                  dest_list_1[cur_parent].Set_2nd_Operand_sat(sat_result_formula_1);
                  dest_list_1[cur_parent].Increment_Child_Sat();
              ...
              if(dest_list_1[cur_parent].Get_Child_sat_Cnt() == 0) {
                dest_list_1[cur_parent].Set_1st_Operand_sat(sat_result_formula_1);
                std::cout << " Setting SAT result(0)for parentID: " << cur_parent << "is : " << sat.to_string(sat_result_formula_1).c_str() << std::endl;
                dest_list_1[cur_parent].Increment_Child_Sat();
                dest_list_1[cur_parent].Set_1st_Operand("SAT_RESULT");
                //std::cout<<"Inside Current-parent child_sat no is "<<cur_parent<<":"<<dest_list[cur_parent].Get_Child_sat_Cnt()<<std::endl;
              }
  ...
  std::cout << "Printing formula for Lgraph1 is:" << sat.to_string(sat_result_formula).c_str() << std::endl;
  std::cout << "Printing formula for Lgraph2 is:" << sat.to_string(sat_result_formula_1).c_str() << std::endl;
  ...
  std::cout << " XOR Fomular : " << sat.to_string(xor_formula).c_str() << std::endl;

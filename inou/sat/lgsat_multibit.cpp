...
public:
  ...
  Destination(){ parent_ID=0; child=0;child_sat=0,oprator="NULL";operandlist={"NULL","NULL"};  operandlist_sat={ };operand_bit_no={};sat_result={};Is_operator_1st_input_sat=0;Is_operator_2nd_input_sat=0;Is_input_2nd_operand=0;input2_sat_string="NULL"; SAT_RESULT=0;}
  ...
  std::vector<int> Destination::sat_operator(int oper_type, std::vector<int> a, std::vector<int> b, ezMiniSAT& sat  ) {
    ...
    case Sum_Op :{sat_val=sat.vec_or(a,b);
             	   std::cout<<" Sum Op....."<<std::endl;	
	          break;}
           
    case Mult_Op:{sat_val=sat.vec_and(a,b);
		 std::cout<<" Sum Op....."<<std::endl;
		 break;}
...
void Destination::Increment_and_Add_Child_with_sat(int node_B_ID, std::vector<int> input2,  ezMiniSAT& sat ,int operand_type ) {
  ...
  if( Get_Child_Cnt()==1) {
	...
	switch (operand_type) {
	  case Sum_Op :{sat_result=sat.vec_or(input1,input2);
	    std::cout<<std::endl;

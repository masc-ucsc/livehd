    ...
    for(auto idx : g->backward()) {
      for(const auto &c : g->inp_edges(idx)) {
        fprintf(stderr, "From node %d o_pin:%d -> node%d inp_pin:%d TSTbck\n", (int)c.get_idx(), (int)c.get_out_pin().get_pid(),
                (int)idx, (int)c.get_inp_pin().get_pid());
      }
    }
    ...
    for(auto idx : g->backward()) {
      ...
      for(const auto &c : g->inp_edges(idx)) {
        ...
        if(!g->is_graph_output(node_B_ID)) {
          ...
          if((g->node_type_get(node_B_ID).op == Pick_Op) &&
             (dest_list[node_B_ID].get_operand_no() == get_child_edges_no(g, node_B_ID))) {
            ...
            sat_result_formula =
                dest_list[node_B_ID].sat_operator_with_Pick_Op(node_B_ID, sat, Pick_Op, g, U32Const_val, pick_bits, vec_pick);
          ...
          if(dest_list[node_B_ID].get_SAT_RESULT_READY() == 1) {
            ...
            while((dest_list[cur_parent].get_operand_no() == get_child_edges_no(g, cur_parent)) &&
                  !(g->is_graph_output(cur_parent))) {

              // std::cout<<" Inside While Loop Interation ......\n";
              // std::cout<<"get_operand_cnt is "<<dest_list[cur_parent].get_operand_no()<<" and get_child_cnt is
              // :"<<get_child_edges_no(g,cur_parent)<<std::endl; std::cout << " When Current_Parent child sat count is " <<
              // cur_parent << ":" << dest_list[cur_parent].Get_Child_Cnt() << std::endl; std::cout<<" Before Entering Flop.... ";
              ...
              if(g->node_type_get(cur_parent).op == Mult_Op || g->node_type_get(cur_parent).op == Sum_Op) {
                ...
                std::cout << " Inputs for nodeID are : " << cur_parent << sat.to_string(input1[0]).c_str() << "+ "
                          << sat.to_string(input2[0]).c_str() << std::endl;
                sat_result_formula = dest_list[cur_parent].sat_operator_SUM(cur_parent, oper_type_B, sat, cur_parent);
                std::cout << " Setting SAT result for nodeID: " << cur_parent << sat.to_string(sat_result_formula[0]).c_str()
                          << std::endl;
                cur_parent = dest_list[cur_parent].get_parent_ID();
                dest_list[cur_parent].increment_operands();
              ...
              if((g->node_type_get(cur_parent).op == Pick_Op)) {
                ...
                std::cout << " Pick 1st Operand for nodeID:" << cur_parent << "is" << sat.to_string(vec_pick[0]).c_str()
                          << std::endl;
                ...
                sat_result_formula =
                    dest_list[cur_parent].sat_operator_with_Pick_Op(cur_parent, sat, Pick_Op, g, U32Const_val, pick_bits, vec_pick);
                cur_parent = dest_list[cur_parent].get_parent_ID();
                dest_list[cur_parent].increment_operands();
                std::cout << " Setting Pick operand " << sat.to_string(sat_result_formula[0]).c_str() << std::endl;
                dest_list[cur_parent].set_operand(sat_result_formula);

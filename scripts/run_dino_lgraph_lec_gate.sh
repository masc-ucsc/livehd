...
for entry in "${DESIGNS[@]}"; do
  ...
  # ref side: raw RTL concatenated into one .sv, independently read by native Slang
  cat "${sv_files[@]}" > "$ref_sv"
  ...
  verdict="$(python3 -c "import json,sys;d=json.load(open('$OUT/${name}_lec.json'));print(d.get('lec',{}).get('verdict','?'))" 2>/dev/null || echo '?')"
  by_solver="$(grep -c 'via solver' "$log" 2>/dev/null || echo 0)"
  if [[ $rc -eq 0 && "$verdict" == "proven" ]]; then
...
echo ""
echo "LEC gate: $total designs — proven=$proven refuted=$refuted inconclusive=$inconclusive"

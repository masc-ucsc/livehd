...
RUNDIR=$(bazel info output_base)
lcov --ignore-errors source --capture --rc geninfo_auto_base=1 --rc lcov_branch_coverage=0 --compat-libtool --base-directory . --directory ${RUNDIR} --exclude '/usr/*' --exclude '*external*' --output-file cov/coverage.info 2>cov_warnings.log

...
protected:
  inline Dlop current_pyrope_value() { return *Dlop::from_pyrope(current_text()); }

  inline Dlop current_prim_value() {
    // A ref operand is not a literal, so it is statically unknown here (an
    // invalid Dlop → the caller treats it as "cannot decide"). Only literal

...
void uPass_constprop::process_tuple_concat() {
  ...
  //   Bundle mode  — every operand must be a known Bundle; they are merged in
  //                  order via Bundle::concat() into a fresh result Bundle.
  //                  Convergence is driven by downstream tuple_get (no
  //                  mark_changed here, same policy as tuple_add).
  //   Scalar mode  — every operand must be a known scalar (string or integer);
  //                  they are folded left-to-right via Lconst::concat_op.
  // Bail without folding when any operand is truly unknown (not in the symbol
  // table) or the operands mix Bundle and scalar types.
  move_to_child();
  auto dst = std::string(current_text());
  ...
  // Returns false if the current node cannot be folded (bail signal).
  auto process_one = [&]() -> bool {
    if (is_type(Lnast_ntype::Lnast_ntype_ref)) {
      auto b = st.get_bundle(current_text());
      if (b) {
        if (!is_first && !bundle_mode) return false;  // mixed scalar+bundle
        if (is_first) {
      ...
      return false;  // unknown ref — bail
    }
    if (is_type(Lnast_ntype::Lnast_ntype_const)) {
      if (bundle_mode) return false;  // mixed bundle+scalar
  ...
  if (bundle_mode) {
    // Store merged bundle; no mark_changed() — downstream tuple_get drives convergence.
...
upass::Emit_decision uPass_constprop::classify_statement() {
  ...
  // Regs, inputs, and outputs carry timing / boundary semantics beyond their
  // value — always emit them even when the value is fully known.
  ...
  // Drop the statement iff the symbol table holds a concrete, fully-known
  // value for LHS.  is_known_const() encapsulates the has_trivial +
  // is_invalid + has_unknowns checks in a single call.
...
std::optional<Lconst> uPass_constprop::fold_ref(std::string_view name) {
  ...
  // Strip I/O prefixes so the lookup resolves the same entry that
  // process_assign populated (which also strips leading %/$).

# opt_loop_incr — measurement log and evidence archive

> **This is the detailed record, not the design.** The readable, high-level
> description of the incremental compile loop lives in
> [`opt_loop_incr.md`](opt_loop_incr.md); read that first. This file keeps the
> raw evidence behind it — the 2026-08-14 audit findings (F1–F22), the harness
> items (H1–H7), the lever pool (L1–L11), the iteration log (I-0 … I-7) and the
> traps (T1–T14) — so that the identifiers other documents, tests and
> `../lhdsuite/bench` scripts cite by number keep resolving.
>
> Everything below is preserved as written on the day it was measured. Two
> rulings recorded here were **superseded on 2026-08-30** and the text was not
> retro-edited: source units are compiled independently (invalidation is
> interface-driven, not import-closure-driven), and a warning no longer blocks
> reuse. See `opt_loop_incr.md` §Rulings I12 and I13.
...
| LEC / `formal verify` | `<wd>/formal_cache.json` | 128-bit 2-lane Merkle canonical digest **pair** + options | digest only | **auto** `kFormalSrcSalt` (`lhd/BUILD:17` genrule over `pass/lec` + `pass/semdiff` + the cvc5 pin) | user `--workdir` **and** `lhd.incremental` |

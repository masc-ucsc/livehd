...
* **Hierarchical traversal**: It requires "top" graph which can have several
  ...
  Explicit hierarchy information can be obtained through the LGraph `ref_htree()` method.  The obtained hierarchy can be considered global by passing the top level module to the required pass — `lhd ... --top <top level module>` from the CLI, or in EPRP pipe form:  
  `lgraph.open name:<top level module> |> a.hier.pass`

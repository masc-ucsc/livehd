...
def fast8 (i : RuntimeInput) (_s : RuntimeState) : RuntimeResult :=
  let s0 := bv_resize 8 (i[0]?.getD (mk_bv 8 0))
  let s1 := bv_resize 8 (i[1]?.getD (mk_bv 8 0))
  let n0 := randV 8 [s0, s1]
...
def fast64 (i : RuntimeInput) (_s : RuntimeState) : RuntimeResult :=
  let s0 := bv_resize 8 (i[0]?.getD (mk_bv 8 0))
  let s1 := bv_resize 8 (i[1]?.getD (mk_bv 8 0))
  let n0 := randV 8 [s0, s1]
  let n1 := randV 8 [n0, s1]
...
def fast256 (i : RuntimeInput) (_s : RuntimeState) : RuntimeResult :=
  let s0 := bv_resize 8 (i[0]?.getD (mk_bv 8 0))
  let s1 := bv_resize 8 (i[1]?.getD (mk_bv 8 0))
  let n0 := randV 8 [s0, s1]
  let n1 := randV 8 [n0, s1]
  ...
  { outputs := #[bv_resize 8 n255]
    nextState := { flops := #[], mems := #[] } }

def stz : RuntimeState := ⟨#[], #[]⟩

def benchD (name : String) (n iters : Nat) (f : RuntimeInput → RuntimeState → RuntimeResult) : IO Unit := do
  let t0 ← IO.monoMsNow
  let mut acc : Int := 0
  for k in [0:iters] do
    acc := acc + ((f #[mk_bv 8 (Int.ofNat k), mk_bv 8 1] stz).outputs[0]!.value)
  let t1 ← IO.monoMsNow
  IO.println s!"{name}\tn={n}\titers={iters}\tms={t1-t0}\tacc={acc}"

def main : IO Unit := do
  benchD "deep  " 1 20000 (denoteResidual R1)
  benchD "shallow" 1 20000 fast1
  benchD "deep  " 8 20000 (denoteResidual R8)
  benchD "shallow" 8 20000 fast8
  benchD "deep  " 64 20000 (denoteResidual R64)
  benchD "shallow" 64 20000 fast64
  benchD "deep  " 256 20000 (denoteResidual R256)
  benchD "shallow" 256 20000 fast256

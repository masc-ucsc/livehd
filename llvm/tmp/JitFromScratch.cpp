...
llvm::Expected<std::string> codegenIR(llvm::Module *module, unsigned items) {
  ...
  if (verifyFunction(*fn, &es))
    return make_error<StringError>(Twine("Function verification failed:\n", es.str()), llvm::inconvertibleErrorCode());

  if (verifyModule(*module, &es))
    return make_error<StringError>(Twine("Module verification failed:\n", es.str()), llvm::inconvertibleErrorCode());

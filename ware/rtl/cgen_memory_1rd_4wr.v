`ifndef LIVEHD_CGEN_MEMORY_1RD_4WR_V
`define LIVEHD_CGEN_MEMORY_1RD_4WR_V
...
always @(posedge clk) begin
  for(i=0;i<WENSIZE;i=i+1) begin
    ...
    if(wr_enable_3[i]) begin
        data[wr_addr_3][i*MASKSIZE +: MASKSIZE] <=
          wr_din_3[i*MASKSIZE +: MASKSIZE];
    end
  end
end
  ...
  for(fwd_j0=0;fwd_j0<WENSIZE;fwd_j0=fwd_j0+1) begin:FWD_BLOCK_CALC_0
    always_comb begin
      d0_fwd[fwd_j0*MASKSIZE +: MASKSIZE] =
        (((UNDEF >> 3) & 1) != 0 && wr_enable_3[fwd_j0] && (wr_addr_3 == rd_addr_0)) ?
        {MASKSIZE{1'bx}} :
        (((FWD >> 3) & 1) != 0 && wr_enable_3[fwd_j0] && (wr_addr_3 == rd_addr_0)) ?
        wr_din_3[fwd_j0*MASKSIZE +: MASKSIZE] :
        (((UNDEF >> 2) & 1) != 0 && wr_enable_2[fwd_j0] && (wr_addr_2 == rd_addr_0)) ?
        {MASKSIZE{1'bx}} :
...
`endif // LIVEHD_CGEN_MEMORY_1RD_4WR_V

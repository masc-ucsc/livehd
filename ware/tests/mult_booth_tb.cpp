...
int main(int argc, char **argv, char **env) {
  ...
  uint64_t val_b[NUM_TESTS] = {(uint64_t)~0, 0, 0xAAAAAAAAAAAAAAAA, (uint64_t)1 << 63, 2, 3, 8, 16, 32, 64, 1};
  ...
  for (int i = 0; i < NUM_TESTS; i++) {
    ...
    top_ans
        = ((__int128)top->ans[0]) + ((__int128)top->ans[1] << 32) + ((__int128)top->ans[2] << 64) + ((__int128)top->ans[3] << 96);

    // evaluate correctness
    printf("Test %d: ", i);
    __int128 product = ((__int128)top_a) * ((__int128)top_b);
    if (product == top_ans) {
      printf("PASSED\n");
    } else {
      printf("FAILED\n");

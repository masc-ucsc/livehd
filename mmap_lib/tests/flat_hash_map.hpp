...
struct functor_storage<Result, Result (*)(Args...)> {
  ...
  functor_storage(function_ptr _function) : function(_function) {}
...
struct sherwood_v3_entry {
  sherwood_v3_entry() {}
  sherwood_v3_entry(int8_t _distance_from_desired) : distance_from_desired(_distance_from_desired) {}
  ~sherwood_v3_entry() {}
...
public:
  ...
  struct templated_iterator {
    ...
    templated_iterator(EntryPointer _current) : current(_current) {}
  ...
  void reserve(size_t num) {
    size_t required_buckets = num_buckets_for_reserve(num);
...
private:
  ...
  size_t num_buckets_for_reserve(size_t num) const {
    return static_cast<size_t>(std::ceil(num / std::min(0.5, static_cast<double>(_max_load_factor))));
  }
  ...
  void deallocate_data(EntryPointer begin, size_t num_s, int8_t max_l) {
    if (begin != Entry::empty_default_table()) {
      AllocatorTraits::deallocate(*this, begin, num_s + max_l + 1);
    }
  }
...
struct fibonacci_hash_policy {
  ...
  void commit(int8_t _shift) { shift = _shift; }
  void reset() { shift = 63; }

...
#pragma GCC diagnostic pop
  ...
  [[nodiscard]] static str concat(const T1 a, const T2 b, const T3 c) {
    ...
    } else if constexpr (std::is_integral_v<T3>) {
      c_s  = std::to_string(c);
      c_sv = std::string_view(c_s.data(), c_s.size());
    } else {
      assert(false);  // Convert to mmap_lib::str the 1st argument (or extend the class)
  ...
  [[nodiscard]] static str concat(const T1 a, const T2 b, const T3 c, const T4 d) {
    ...
    } else if constexpr (std::is_integral_v<T4>) {
      d_s  = std::to_string(d);
      d_sv = std::string_view(d_s.data(), d_s.size());
    } else {
      assert(false);  // Convert to mmap_lib::str the 1st argument (or extend the class)
  ...
  [[nodiscard]] static str concat(const T1 a, const T2 b, const T3 c, const T4 d, const T5 e) {
    ...
    } else if constexpr (std::is_integral_v<T5>) {
      e_s  = std::to_string(e);
      e_sv = std::string_view(e_s.data(), e_s.size());
    } else {
      assert(false);  // Convert to mmap_lib::str the 1st argument (or extend the class)
  ...
  [[nodiscard]] static str concat(const T1 a, const T2 b, const T3 c, const T4 d, const T5 e, const T6 f) {
    ...
    } else if constexpr (std::is_integral_v<T6>) {
      f_s  = std::to_string(f);
      f_sv = std::string_view(f_s.data(), f_s.size());
    } else {
      assert(false);  // Convert to mmap_lib::str the 1st argument (or extend the class)
  ...
  [[nodiscard]] static str concat(const T1 a, const T2 b, const T3 c, const T4 d, const T5 e, const T6 f, const T7 g) {
    std::string_view a_sv;
    std::string      a_s;
    if constexpr (std::is_same_v<T1, std::string_view>) {
      a_sv = a;
    ...
    std::string_view g_sv;
    std::string      g_s;
    if constexpr (std::is_same_v<T7, std::string_view>) {
      g_sv = g;
    } else if constexpr (std::is_same_v<T7, std::string>) {
      g_sv = std::string_view(g.data(), g.size());
    } else if constexpr (std::is_same_v<T7, char const *>) {
      g_sv = std::string_view(g);
    } else if constexpr (std::is_same_v<T7, mmap_lib::str>) {
      g_s  = g.to_s();
      g_sv = std::string_view(g_s.data(), g_s.size());
    } else if constexpr (std::is_integral_v<T7>) {
      g_s  = std::to_string(g);
      g_sv = std::string_view(g_s.data(), g_s.size());
    } else {
      assert(false);  // Convert to mmap_lib::str the 1st argument (or extend the class)
    ...
    std::string s;
    s.reserve(a_sv.size() + b_sv.size() + c_sv.size() + d_sv.size() + e_sv.size() + f_sv.size() + g_sv.size());
    s.append(a_sv.data(), a_sv.size());
    s.append(b_sv.data(), b_sv.size());
    s.append(c_sv.data(), c_sv.size());
    s.append(d_sv.data(), d_sv.size());
    s.append(e_sv.data(), e_sv.size());
    s.append(f_sv.data(), f_sv.size());
    s.append(g_sv.data(), g_sv.size());

    return mmap_lib::str(s);
  }

  template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7, typename T8>
  [[nodiscard]] static str concat(const T1 a, const T2 b, const T3 c, const T4 d, const T5 e, const T6 f, const T7 g, const T8 h) {
    std::string_view a_sv;
    std::string      a_s;
    if constexpr (std::is_same_v<T1, std::string_view>) {
      a_sv = a;
    ...
    std::string_view h_sv;
    std::string      h_s;
    if constexpr (std::is_same_v<T8, std::string_view>) {
      h_sv = h;
    } else if constexpr (std::is_same_v<T8, std::string>) {
      h_sv = std::string_view(h.data(), h.size());
    } else if constexpr (std::is_same_v<T8, char const *>) {
      h_sv = std::string_view(h);
    } else if constexpr (std::is_same_v<T8, mmap_lib::str>) {
      h_s  = h.to_s();
      h_sv = std::string_view(h_s.data(), h_s.size());
    } else if constexpr (std::is_integral_v<T8>) {
      h_s  = std::to_string(h);
      h_sv = std::string_view(h_s.data(), h_s.size());
    } else {
      assert(false);  // Convert to mmap_lib::str the 1st argument (or extend the class)
    ...
    std::string s;
    s.reserve(a_sv.size() + b_sv.size() + c_sv.size() + d_sv.size() + e_sv.size() + f_sv.size() + g_sv.size() + h_sv.size());
    s.append(a_sv.data(), a_sv.size());
    s.append(b_sv.data(), b_sv.size());
    s.append(c_sv.data(), c_sv.size());
    s.append(d_sv.data(), d_sv.size());

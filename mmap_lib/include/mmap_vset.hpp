...
namespace mmap_lib {
	...
	class vset {
		public:
			...
			[[nodiscard]] const T & bucket_get(const Key &key) const { return visitor_set.get(key); }
			
			template <typename T_ = T, typename = std::enable_if_t<is_array_serializable<T_>::value>>
			[[nodiscard]] T & bucket_get(const Key &key) const { return visitor_set.get(key); }
			
			template <typename T_ = T, typename = std::enable_if_t<!is_array_serializable<T_>::value>>
			[[nodiscard]] const T & bucket_get(const iterator &it) const { return visitor_set.get(it); }
			
			template <typename T_ = T, typename = std::enable_if_t<is_array_serializable<T_>::value>>
			[[nodiscard]] T & bucket_get(const iterator &it) const { return visitor_set.get(it); }
			
			template <typename T_ = T, typename = std::enable_if_t<!is_array_serializable<T_>::value>>
			[[nodiscard]] T bucket_get(const iterator &it) const { return visitor_set.get(it); }
			
			template <typename T_ = T, typename = std::enable_if_t<is_array_serializable<T_>::value>>
			[[nodiscard]] T bucket_get(const const_iterator &it) const { return visitor_set.get(it); }
		  ...
		  /* Can add functions here to set a bit specifically in the bitmap at key
			...
			[[nodiscard]] void insert(T &&ele) { 
				...
				auto hold = 0; // will hold the bitmap at index p if there is one
			...
			[[nodiscard]] void erase(T &&ele) { 
				...
				const auto i = ele % (sizeof(T)*8); // i will be the bit we are interested in in the bitmap
				auto hold = 0; // will hold the bitmap at index p if there is one
				if (visitor_set.has((Key)p)) { hold = visitor_set.get((Key)p); }
				hold = hold & ~(1 << i); //modify the bit at pos
				visitor_set.set((Key)p, (T)hold); //put it back in the bitmap
			}
			...
			[[nodiscard]] bool is_end(T &&ele) { 
				auto ex = visitor_set.capacity(); 
				ex = (ex * sizeof(T)) - 1;
				if (ele == ex) { return true; }
				return false;
			}

...
namespace mmap_lib {
	...
	class vset {
		public:
			...
			explicit vset(std::string_view _set_name) 
				: visitor_set(std::string(_set_name) + "_vs") {}
			explicit vset(std::string_view _path, std::string_view _set_name) 
				: visitor_set(_path, std::string(_set_name) + "_vs") {}
  		...
  		[[nodiscard]] const T &        bucket_get_val(const Key &key) const { return visitor_set.get(key); }
      
			template<typename T_ = T, typename = std::enable_if_t<is_array_serializable<T_>::value>>
      [[nodiscard]] T                bucket_get_val(const Key &key) const { return visitor_set.get(key); }
 			
			template<typename T_ = T, typename = std::enable_if_t<!is_array_serializable<T_>::value>>
  		[[nodiscard]] const T &        bucket_get_val(const iterator &it) const { return visitor_set.get(it); }
      
			template<typename T_ = T, typename = std::enable_if_t<!is_array_serializable<T_>::value>>
      [[nodiscard]] const T &        bucket_get_val(const const_iterator &it) const { return visitor_set.get(it); }

      template<typename T_ = T, typename = std::enable_if_t<is_array_serializable<T_>::value>>
      [[nodiscard]] T                bucket_get_val(const iterator &it) const { return visitor_set.get(it); }
      
			template<typename T_ = T, typename = std::enable_if_t<is_array_serializable<T_>::value>>
      [[nodiscard]] T                bucket_get_val(const const_iterator &it) const { return visitor_set.get(it); }
		   ...
		   */
			...
			size_t bucket_erase_key(const Key &key) {
				...
				if (it == visitor_set.end()) { return 0; }
				erase(it);
				return 1;
			}
		  ...
		  /* Can add functions here to set a bit specifically in the bitmap at key
			...
			[[nodiscard]] void insert(T &&ele) {
				if (ele > max_inserted) { max_inserted = ele; }
				...
				if (visitor_set.has((Key)p)) { hold = visitor_set.get((Key)p); }//is there a bitmap at key p
			...
			[[nodiscard]] bool is_end(T &&ele) {
				if (ele == (T)max_inserted) { return true; }
				return false;
			}

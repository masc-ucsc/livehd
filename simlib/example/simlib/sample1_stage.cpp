...
#ifdef SIMLIB_VCD
Sample1_stage::Sample1_stage(uint64_t _hidx, std::string &parent_name)//const std::string &parent_name)
  : hidx(_hidx)
...
void Sample1_stage::vcd_reset_cycle() {
  tmp = UInt<32>(0);
  to2_aValid = UInt<1>(0);
    vcd_writer.change(vcd_to2_aValid, t, "1");//to2_aValid.to_binary());
  to3_cValid = UInt<1>(0);
    vcd_writer.change(vcd_to3_cValid, t, "1");//to3_cValid.to_binary());
  to3_c = UInt<32>(0);
    vcd_writer.change(vcd_to3_c, t, "11");//to3_c.to_binary());
}
 void Sample1_stage::vcd_cycle(UInt<32> s3_to1_b, UInt<1> s2_to1_aValid, UInt<32> s2_to1_a) {
    to2_b = s3_to1_b.addw(UInt<32>(1));
    vcd_writer.change(vcd_to2_b, t , "10000");//to2_b.to_binary());

    auto tmp3 = s2_to1_a.addw(s3_to1_b);
    to2_a = tmp3.addw(UInt<32>(2));
    vcd_writer.change(vcd_to2_a, t, "11");//to2_a.to_binary());
    ...
    vcd_writer.change(vcd_to3_c, t, "10011");//to3_c.to_binary());

...
bool LGraph_Node_Type::is_type_const(Index_ID nid) const {
  ...
  return (op >= U32ConstMin_Op && op <= U32ConstMax_Op) || (op >= StrConstMin_Op && op <= StrConstMax_Op);
}
...
void LGraph_Node_Type::set_type_sub(Index_ID nid, Lg_type_id subgraphid) {
  ...
  I((node_type_table[nid] >= SubGraphMin_Op && node_type_table[nid] < SubGraphMax_Op) || node_type_table[nid] == Invalid_Op);
...
void LGraph_Node_Type::set_type_const_value(Index_ID nid, uint32_t value, uint16_t bits) {
  I(nid < node_type_table.size());
  I(node_internal[nid].is_node_state());
  ...
  d<<=16;
  d|=bits;
  const_value.set(d, Node::Compact_class(nid));
...
Index_ID LGraph_Node_Type::find_type_const_sview(std::string_view value) const {
  ...
  if (it == const_sview.end()) return 0;

  return const_sview.get(it).nid;
}

Index_ID LGraph_Node_Type::find_type_const_value(uint32_t value, uint16_t bits) const {
  uint64_t d = value;
  d<<=16;
  d|=bits;
  auto it = const_value.find(d);
  if (it == const_value.end()) return 0;
...
std::string_view LGraph_Node_Type::get_constant(uint32_t const_id) const { return const_sview.get_sview(const_id); }

...
struct __attribute__ ((packed))  SEdge_Internal {      // 2 bytes total
  ...
  Index_ID get_idx(Index_ID idx) const { assert(ridx);
...
public:
  ...
  void set_graph_io_input()  { assert(!graph_io_output); graph_io_input  = true; }
  void set_graph_io_output() { assert(!graph_io_input);  graph_io_output = true; }

  void set_root()   { root = true;  }
  void clear_root() { root = false; graph_io_input = false; graph_io_output = false; }
  ...
  uint16_t get_bits() const { assert(is_root()); return bits; }

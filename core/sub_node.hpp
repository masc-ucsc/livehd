...
public:
  ...
  struct IO_pin {
    ...
    IO_pin(const mmap_lib::str _name, Port_ID ipid, Direction _dir, Port_ID _graph_io_pos)
...
public:
  ...
  [[nodiscard]] bool has_pin(const mmap_lib::str &io_name) const {
    I(lgid);
  ...
  [[nodiscard]] Port_ID get_instance_pid(const mmap_lib::str &io_name) const {
    if (io_name == "$" || io_name == "%")
      return 0;
    I(has_pin(io_name));
  ...
  [[nodiscard]] Port_ID get_io_pos(const mmap_lib::str &io_name) const {
  ...
  [[nodiscard]] const IO_pin &get_pin(const mmap_lib::str &io_name) const {
    I(has_pin(io_name));
  ...
  [[nodiscard]] const IO_pin &get_graph_output_io_pin(const mmap_lib::str &io_name) const {
    I(has_pin(io_name));
    auto instance_pid = name2id.at(io_name);
    I(io_pins[instance_pid].name == io_name);
  ...
  [[nodiscard]] const IO_pin &get_graph_input_io_pin(const mmap_lib::str &io_name) const {
    I(has_pin(io_name));
    auto instance_pid = name2id.at(io_name);
    I(io_pins[instance_pid].name == io_name);
  ...
  [[nodiscard]] Port_ID get_graph_io_pos(const mmap_lib::str &io_name) const {
    I(has_pin(io_name));
    auto instance_pid = name2id.at(io_name);
    I(io_pins[instance_pid].name == io_name);
    return io_pins[instance_pid].graph_io_pos;
  ...
  [[nodiscard]] mmap_lib::str get_name_from_instance_pid(Port_ID instance_pid) const {
    if (!has_instance_pin(instance_pid))
      return "INVALID_PID"_str;
    return io_pins[instance_pid].name;
  }

  [[nodiscard]] mmap_lib::str get_name_from_graph_pos(Port_ID graph_pos) const {
  ...
  [[nodiscard]] bool is_input(const mmap_lib::str &io_name) const {
    if (io_name == "$")
      return true;
  ...
  [[nodiscard]] bool is_output(const mmap_lib::str &io_name) const {
    if (io_name == "%")
      return true;

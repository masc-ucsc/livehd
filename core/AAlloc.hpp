...
template <typename T, const unsigned align = 16> class AlignedAllocator {
...
template <typename T, const unsigned align> std::map<ptrdiff_t, ptrdiff_t> AlignedAllocator<T, align>::_ptrmap;

#ifndef NDEBUG
template <typename T, const unsigned align> unsigned AlignedAllocator<T, align>::_reference_count = 0;
#endif

template <typename T, const unsigned align> AlignedAllocator<T, align>::AlignedAllocator() {
#ifndef NDEBUG
  if((align & (align - 1)) != 0)
...
template <typename T, const unsigned align> AlignedAllocator<T, align>::AlignedAllocator(const AlignedAllocator &) {
#ifndef NDEBUG
#ifdef OPENMP
#pragma omp critical(ptrmap_modify)
#endif // OPENMP
...
typename AlignedAllocator<T, align>::pointer AlignedAllocator<T, align>::allocate(size_type n) {
  if(n > this->max_size())
    throw std::bad_alloc();

  ptrdiff_t op = reinterpret_cast<ptrdiff_t>(::operator new(n * sizeof(T) + static_cast<size_t>(align) - static_cast<size_t>(1)));
...
template <typename T, const unsigned align> void AlignedAllocator<T, align>::erase(ptrmap_it it) {
  pointer real_p = reinterpret_cast<pointer>(it->second);
  ::      operator delete(real_p);
  _ptrmap.erase(it);
}

template <typename T, const unsigned align> void AlignedAllocator<T, align>::construct(pointer p, const_reference val) {
#ifdef OPENMP
#pragma omp critical(ptrmap_modify)
#endif                                // OPENMP
  new(static_cast<void *>(p)) T(val); // placement new
}
template <typename T, const unsigned align> void AlignedAllocator<T, align>::deallocate(pointer p, size_type n) {

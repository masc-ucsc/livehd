...
void LGraph::each_sorted_graph_io(std::function<void(const Node_pin &pin, Port_ID pos)> f1) {
  ...
  auto out = Node(this, get_hierarchy_root(), Node::Hardcoded_output_nid);
  ...
  auto inp = Node(this, get_hierarchy_root(), Node::Hardcoded_input_nid);
...
void LGraph::each_graph_input(std::function<void(const Node_pin &pin)> f1) {
  ...
  auto node = Node(this, get_hierarchy_root(), Node::Hardcoded_input_nid);
...
void LGraph::each_graph_output(std::function<void(const Node_pin &pin)> f1) {
  ...
  auto node = Node(this, get_hierarchy_root(), Node::Hardcoded_output_nid);
  for(const auto &pin:node.out_setup_pins()) {
    f1(pin);
  }
}
...
void LGraph::each_node_fast(std::function<void(const Node &node)> f1) {
  for (const auto &ni : node_internal) {
    ...
    Node node(this, get_hierarchy_root(), ni.get_nid());
    f1(node);
  }
}
...
void LGraph::each_output_edge_fast(std::function<void(XEdge &edge)> f1) {
  for (const auto &ni : node_internal) {
    ...
    auto dpin = Node_pin(this, this, get_hierarchy_root(), ni.get_nid(), ni.get_dst_pid(), false);

    const Edge_raw *edge_raw = ni.get_output_begin();
    do {
      XEdge edge(dpin, Node_pin(this, this, get_hierarchy_root(), edge_raw->get_idx(), edge_raw->get_inp_pid(), true));
...
void LGraph::each_root_fast_direct(std::function<bool(Node &)> f1) {
  ...
  for (const auto &ni : node_internal) {
    ...
    auto node = Node(this, get_hierarchy_root(), ni.get_nid());

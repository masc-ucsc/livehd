...
template<const char *Name, typename Base, typename Key, bool Unique, Node_pin_mode Mode=Node_pin_mode::Both>
class Attribute_sview {

  using Attr_data = Attr_sview_raw<Key, Unique>;
  ...
  static std::string get_filename(Lg_type_id lgid) {
    if constexpr (Mode == Node_pin_mode::Driver)
      return absl::StrCat(get_base(), std::to_string(lgid), Name, "_d", Unique?"_uni":"_dup");
    else if constexpr (Mode == Node_pin_mode::Sink)
      return absl::StrCat(get_base(), std::to_string(lgid), Name, "_s", Unique?"_uni":"_dup");
    else if constexpr (Mode == Node_pin_mode::Both)
      return absl::StrCat(get_base(), std::to_string(lgid), Name, Unique?"_uni":"_dup");
    I(false);
    return "lgraph_bogus";
  };
...
public:
  static std::string_view set(const Base &obj, std::string_view data) {
  ...
  static auto find(LGraph *lg, std::string_view name) {

    if (unlikely(lg!=last_lg))
      setup_table(lg);

    if constexpr (std::is_same<Base, Node_pin>::value) {
      I(Mode != Node_pin_mode::Both); // not supported mode for find

      Node_pin pin(lg, last_attr->find(name));

      GI(Mode == Node_pin_mode::Driver, pin.is_driver());
      GI(Mode == Node_pin_mode::Sink  , pin.is_sink());
      return pin;
    }else if constexpr (std::is_same<Base, Node>::value) {

...
public:
  ...
  template <class U>
  struct rebind { typedef AlignedAllocator<U, align>
                      other; };
};
...
typename AlignedAllocator<T, align>::pointer AlignedAllocator<T, align>::allocate(
  ...
  ptrdiff_t op = reinterpret_cast<ptrdiff_t>(
      ::operator new(n * sizeof(T) + static_cast<size_t>(align) - static_cast<size_t>(1)));
  ptrdiff_t ap = (op + static_cast<ptrdiff_t>(align) - static_cast<ptrdiff_t>(1)) & ~(static_cast<ptrdiff_t>(align) - static_cast<ptrdiff_t>(1));

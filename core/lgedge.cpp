...
void Node_Internal::assimilate_edges(Node_Internal &other) {
  ...
  for(int i=0;i<other.inp_pos;) {
    if (other.sedge[other_pos].is_snode()) {
      bool done = sedge[self_pos].set(other.sedge[other_pos].get_idx()
          ,other.sedge[other_pos].get_inp_pid()
          ,other_out_pid
          ,true // input
          );
      if (done) {
        ...
        ledge->set(other.sedge[other_pos].get_idx()
            ,other.sedge[other_pos].get_inp_pid()
            ,true // input
            );
  ...
  if (has_space(true)) {
    ...
    while(other.out_pos) {
      ...
      bool done = sedge[self_pos].set(other_out->get_idx()
          ,other_out->get_inp_pid()
          ,other_out_pid
          ,false // output
          );

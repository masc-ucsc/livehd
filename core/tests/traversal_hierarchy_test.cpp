...
protected:
  ...
  void check_lgraph_fwd(int rseed) {
    ...
    fwd_order.resize(absl_fwd_pos.size());

    for (auto &node : node_order) {
      if (node.is_type_io())
        continue;
      const auto &fwd_pos = Fwd_pos_attr::ref(lg_root)->get(node.get_compact());
      I(fwd_pos<=fwd_order.size());
      I(fwd_order[fwd_pos-1].empty());
      I(fwd_pos>0);

      std::string txt = fmt::format("pos:{} name:{} lg:{}",fwd_pos, node.debug_name(), node.get_class_lgraph()->get_name());
      fwd_order[fwd_pos-1] = txt;
      pos++;
    }
#if 0
...
#else
#endif
      std::string txt = fmt::format("pos:{} name:{} lg:{}",pos, node.debug_name(), node.get_class_lgraph()->get_name());

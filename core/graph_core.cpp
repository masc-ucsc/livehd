...
void set_type(const Index_id master_root_id){
   ...
   if(table[master_root_id].master_root == 0){
       table[master_root_id].pid_bits_or_type = type;
     }
}
...
Port_ID get_pid(const Index_id master_root_id){
   if(table[master_root_id].master_root == 0){
     return 0;
   }else{
     return table[master_root_id].edge_storage_or_pid_bits;
   }
}
...
void Graph_core::add_edge(const Index_id sink_id, const Index_id driver_id){
   ...
   if(is_master_root(sink_id) == true){
     ...
     table[driver_id].edge_storage[0] = table[sink_id].get_pid();

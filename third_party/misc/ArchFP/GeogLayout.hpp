...
// It implements two different strategies for floorplanning, one for situations where overlap is tolerated and one for where it is not.
// The first strategy makes optimistic assumptions about the aspect ratio of nodes as they are floorplanned.
// The second strategy implements simulated annealing and uses a slice tree in order to develop a floorplan with no overlap.
...
class geogLayout : public FPContainer {
  bool hardARLayoutHelper(double targetWidth, double targetHeight, double curX, double curY, FPObject** layoutStack,
                    int curDepth, FPObject** centerItems, int centerItemsCount);
public:
  geogLayout(unsigned int rsize);

  virtual bool hardARLayout(double targetAR = 1.0);

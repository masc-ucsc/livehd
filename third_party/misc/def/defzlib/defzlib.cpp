 ...
 * Private functions:
 */
size_t defGZip_read(FILE* file, char* buf, size_t len) { return (gzread((gzFile)file, buf, len)); }
...
int defrReadGZip(defGZFile file, const char* gzipFile, defiUserData uData) { return defrRead((FILE*)file, gzipFile, uData, 1); }
...
int defrGZipClose(defGZFile filePtr) { return (gzclose((gzFile)filePtr)); }

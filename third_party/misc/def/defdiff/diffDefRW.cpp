...
int compf(defrCallbackType_e c, defiComponent* co, defiUserData ud) {
  ...
  if (co->hasNets()) {
    ...
    fprintf(fout, "\n");
  if (co->isFixed())
    fprintf(fout, "COMP %s FIXED ( %d %d ) %s\n", co->id(), co->placementX(), co->placementY(), co->placementOrientStr());
  if (co->isCover())
  ...
  if (co->hasGenerate())
    fprintf(fout, "COMP %s GENERATE %s %s\n", co->id(), co->generateName(), co->macroName());
  if (co->hasHalo()) {
  ...
  if (co->hasRouteHalo()) {
    fprintf(fout, "COMP %s ROUTEHALO %d %s %s\n", co->id(), co->haloDist(), co->minLayer(), co->maxLayer());
  }
  if (co->hasForeignName())
    fprintf(fout, "COMP %s FOREIGN %s %d %d %s\n", co->id(), co->foreignName(), co->foreignX(), co->foreignY(), co->foreignOri());
  if (co->hasWeight())
  ...
  if (co->numProps()) {
    for (i = 0; i < co->numProps(); i++) {
      fprintf(fout, "COMP %s PROP %s %s ", co->id(), co->propName(i), co->propValue(i));
      switch (co->propType(i)) {
        case 'R': fprintf(fout, "REAL "); break;
        case 'I': fprintf(fout, "INT "); break;
        case 'S': fprintf(fout, "STR "); break;
        case 'Q': fprintf(fout, "QSTR "); break;
        case 'N': fprintf(fout, "NUM "); break;
      }
      fprintf(fout, "\n");
    }
  }
...
int netf(defrCallbackType_e c, defiNet* net, defiUserData ud) {
  ...
  for (i = 0; i < net->numConnections(); i++) {
    fprintf(fout, "\nNET %s ( %s %s ) ", net->name(), net->instance(i), net->pin(i));
  }
  ...
  for (i = 0; i < net->numVpins(); i++) {
    ...
    if (vpin->status() != ' ') {
      switch (vpin->status()) {
        case 'P':
        case 'p': fprintf(fout, " PLACED"); break;
        case 'F':
        case 'f': fprintf(fout, " FIXED"); break;
        case 'C':
        case 'c': fprintf(fout, " COVER"); break;
      }
          ...
          switch (path) {
            ...
            case DEFIPATH_VIA:
              if (!netSeCmp) {
                ...
                if (nline) {
                  if (!ignoreVN)
                    fprintf(fout, "\nNET %s %s ( %d %d ) %s", net->name(), wire->wireType(), px, py, p->getVia());
                  else
                    fprintf(fout, "\nNET %s %s ( %d %d )", net->name(), wire->wireType(), px, py);
                } else {
                  if (!ignoreVN)
            ...
            case DEFIPATH_POINT:
              ...
              if (!netSeCmp) {
                if (!nline) {
                  ...
                  fprintf(fout, "\nNET %s %s %s ( %d %d )", net->name(), wire->wireType(), layerName, x, y);
                }
              } else {
                if ((px == -99) && (py == -99)) {
                  px = x;
                  ...
                  if (nline)
                    fprintf(fout, "\nNET %s %s %s", net->name(), wire->wireType(), layerName);
                  if (px < x) {
            ...
            case DEFIPATH_FLUSHPOINT:
              ...
              if (!netSeCmp) {
                if (!nline) {
                  ...
                  fprintf(fout, "\nNET %s %s %s ( %d %d %d )", net->name(), wire->wireType(), layerName, x, y, z);
                }
              } else {
                if ((px == -99) && (py == -99) && (pz == -99)) {
                  ...
                  if (px < x) {
                    if (pz != -99)
                      fprintf(fout, " ( %d %d %d ) ( %d %d %d )", px, py, pz, x, y, z);
                    else
                    ...
                    if (py < y) {
                      ...
                      if (pz != -99)
                        fprintf(fout, " ( %d %d %d ) ( %d %d %d )", x, y, z, px, py, pz);
                      else
          ...
          switch (path) {
            ...
            case DEFIPATH_POINT:
              ...
              if (!nline) {
                ...
                fprintf(fout, "\nNET %s %s ( %d %d )", net->name(), layerName, x, y);
                nline = 1;
              }
              break;
            case DEFIPATH_FLUSHPOINT:
            ...
            while (elem) {
              switch (elem) {
                ...
                case DEFIPATH_VIAROTATION:
                  ...
                  nline = 1;
                  break;
                case DEFIPATH_WIDTH: fprintf(fout, " WIDTH %d", p->getWidth()); break;
                case DEFIPATH_POINT:
                  p->getPoint(&x, &y);
                  if (!nline) {
                    ...
                    fprintf(fout, "\nNET %s %s %s POINT %d %d", s->name(), wire->wireType(), layerName, x, y);
                    nline = 1;
                  }
                  break;
                  // case DEFIPATH_FLUSHPOINT:
  ...
  if (net->hasWeight() || net->hasCap() || net->hasSource() || net->hasPattern() || net->hasOriginal() || net->hasUse()) {
    fprintf(fout, "\nNET %s ", net->name());
...
int snetf(defrCallbackType_e c, defiNet* net, defiUserData ud) {
  ...
  if (net->numConnections() > 0) {
    for (i = 0; i < net->numConnections(); i++) fprintf(fout, "SNET %s ( %s %s )\n", net->name(), net->instance(i), net->pin(i));
  }
  ...
  if (net->numRectangles()) {  // 5.6
    ...
    for (i = 0; i < net->numRectangles(); i++) {
      ...
      if (net->rectMask(i)) {
        ...
        fprintf(fout, "RECT %s %d %d %d %d", net->name(), net->xl(i), net->yl(i), net->xh(i), net->yh(i));
      }
    }
  }
  ...
  if (net->numPolygons()) {
    ...
    for (i = 0; i < net->numPolygons(); i++) {
      ...
      if (net->polyMask(i)) {
        fprintf(fout, "MASK %d POLYGON % s ", net->polyMask(i), net->polygonName(i));
      } else {
  ...
  if (curVer >= 5.8 && net->numViaSpecs()) {
    for (i = 0; i < net->numViaSpecs(); i++) {
      ...
      if (net->topMaskNum(i) || net->cutMaskNum(i) || net->bottomMaskNum(i)) {
        fprintf(fout, "MASK %d%d%d VIA %s ", net->topMaskNum(i), net->cutMaskNum(i), net->bottomMaskNum(i), net->viaName(i));
      } else {
        fprintf(fout, "\n  VIA %s ", net->viaName(i));
      }
          ...
          switch (path) {
            ...
            case DEFIPATH_POINT:
              ...
              if (!nline) {
                ...
                fprintf(fout, "\nSNET %s %s %s ( %d %d )", net->name(), wire->wireType(), sNLayerName, x, y);
                nline = 1;
              }
              break;
            case DEFIPATH_FLUSHPOINT:
          ...
          for (j = 0; j < wire->numPaths(); j++) {
            ...
            while ((path = (int)p->next()) != DEFIPATH_DONE) {
              switch (path) {
                ...
                case DEFIPATH_VIAROTATION: fprintf(fout, " %d", p->getViaRotation()); break;
                case DEFIPATH_WIDTH: fprintf(fout, " %d", p->getWidth()); break;
                case DEFIPATH_POINT:
                  p->getPoint(&x, &y);
                  if (!nline) {
  ...
  if (net->numProps()) {
    for (i = 0; i < net->numProps(); i++) {
      fprintf(fout, "\nSNET %s PROP %s %s ", net->name(), net->propName(i), net->propValue(i));
          ...
          switch (path) {
            ...
            case DEFIPATH_POINT:
              ...
              if (!nline) {
                ...
                fprintf(fout, "\nSNET %s SHIELD %s %s ( %d %d )", net->name(), shield->shieldName(), sNLayerName, x, y);
              }
              break;
            case DEFIPATH_TAPER: fprintf(fout, " TAPER"); break;
          }
  ...
  if (net->hasSpacingRules()) {
    ...
    for (i = 0; i < net->numSpacingRules(); i++) {
      ...
      if (left == right)
        ...
        fprintf(fout, " SPACING %s %g RANGE %g %g ", layerName, checkDouble(dist), checkDouble(left), checkDouble(right));
    }
  }

  if (net->hasVoltage() || net->hasWeight() || net->hasCap() || net->hasSource() || net->hasPattern() || net->hasOriginal()
      || net->hasUse()) {
    fprintf(fout, "\nSNET %s", net->name());
    if (net->hasVoltage())
...
int ndr(defrCallbackType_e c, defiNonDefault* nd, defiUserData ud) {
  ...
  for (i = 0; i < nd->numViaRules(); i++) fprintf(fout, "NDR %s VIARULE %s\n", nd->name(), nd->viaRuleName(i));
  for (i = 0; i < nd->numMinCuts(); i++) fprintf(fout, "NDR %s MINCUTS %s %d\n", nd->name(), nd->cutLayerName(i), nd->numCuts(i));
  for (i = 0; i < nd->numProps(); i++) fprintf(fout, "NDR %s PROPERTY %s %s\n", nd->name(), nd->propName(i), nd->propValue(i));
  --numObjs;
  return 0;
}
...
int prop(defrCallbackType_e c, defiProp* p, defiUserData ud) {
  ...
  if (p->dataType() == 'Q')
    fprintf(fout, "STR ");
  if (p->hasRange()) {
    fprintf(fout, "RANGE %g %g ", checkDouble(p->left()), checkDouble(p->right()));
  }
  if (p->hasNumber())
...
int cls(defrCallbackType_e c, void* cl, defiUserData ud) {
  ...
  switch (c) {
    case defrSiteCbkType:
      site = (defiSite*)cl;
      fprintf(fout, "SITE %s %g %g %s ", site->name(), checkDouble(site->x_orig()), checkDouble(site->y_orig()), site->orientStr());
      fprintf(fout,
              "DO %g BY %g STEP %g %g\n",
    ...
    case defrPinCbkType:
      ...
      if (pin->hasPlacement()) {
        ...
        if (pin->isFixed())
          fprintf(fout, " FIXED ");
        fprintf(fout, "( %d %d ) %s ", pin->placementX(), pin->placementY(), pin->orientStr());
      }
      if (pin->hasSpecial())
        ...
        for (j = 0; j < pin->numPorts(); j++) {
          ...
          for (i = 0; i < port->numVias(); i++) {
            if (port->viaTopMask(i) || port->viaCutMask(i) || port->viaBottomMask(i)) {
              ...
              fprintf(fout, " VIA %s ( %d %d ) ", port->viaName(i), port->viaPtX(i), port->viaPtY(i));
            }
          }
          if (port->hasPlacement()) {
            if (port->isPlaced()) {
              fprintf(fout, " + PLACED");
              fprintf(fout, " %d %d %d ", port->placementX(), port->placementY(), port->orient());
            }
            if (port->isCover()) {
              fprintf(fout, " + COVER");
              fprintf(fout, " %d %d %d ", port->placementX(), port->placementY(), port->orient());
      ...
      if (pin->hasAPinPartialMetalArea()) {
        ...
        for (i = 0; i < pin->numAPinPartialMetalArea(); i++) {
          fprintf(fout, " ANTPINPARTIALMETALAREA %d ", pin->APinPartialMetalArea(i));
      ...
      if (pin->hasAPinPartialMetalSideArea()) {
        ...
        for (i = 0; i < pin->numAPinPartialMetalSideArea(); i++) {
          fprintf(fout, "ANTPINPARTIALMETALSIDEAREA %d", pin->APinPartialMetalSideArea(i));
      ...
      for (j = 0; j < pin->numAntennaModel(); j++) {
        ...
        if (aModel->hasAPinGateArea()) {
          fprintf(fout, "PIN %s + NET %s %s ", tmpPinName, pin->netName(), aModel->antennaOxide());
        ...
        if (aModel->hasAPinMaxSideAreaCar()) {
          ...
          for (i = 0; i < aModel->numAPinMaxSideAreaCar(); i++) {
            fprintf(fout, "ANTPINMAXSIDEAREACAR %d", aModel->APinMaxSideAreaCar(i));
    ...
    case defrRowCbkType:
      ...
      if (ignoreRN)  // PCR 716759, if flag is set don't bother with name
        fprintf(fout, "ROW %s %g %g %d", row->macro(), checkDouble(row->x()), checkDouble(row->y()), row->orient());
      else
        fprintf(fout, "ROW %s %s %g %g %d", row->name(), row->macro(), checkDouble(row->x()), checkDouble(row->y()), row->orient());
      if (row->hasDo()) {
        fprintf(fout, " DO %g BY %g", checkDouble(row->xNum()), checkDouble(row->yNum()));
        if (row->hasDoStep())
      ...
      if (row->numProps() > 0) {
        if (ignoreRN) {
          for (i = 0; i < row->numProps(); i++) fprintf(fout, "ROW PROP %s %s\n", row->propName(i), row->propValue(i));
        } else {
          for (i = 0; i < row->numProps(); i++)
            fprintf(fout, "ROW %s PROP %s %s\n", row->name(), row->propName(i), row->propValue(i));
        }
      }
      break;
    ...
    case defrGcellGridCbkType:
      ...
      fprintf(fout, "GCELLGRID %s %d DO %d STEP %g\n", gcg->macro(), gcg->x(), gcg->xNum(), checkDouble(gcg->xStep()));
      break;
    case defrViaCbkType:
      via = (defiVia*)cl;
      ...
      if (via->hasViaRule()) {
        ...
        fprintf(fout, "VIA %s VIARULE %s CUTSIZE %d %d LAYERS %s %s %s", via->name(), vrn, xs, ys, bl, cl, tl);
        fprintf(fout, " CUTSPACING %d %d ENCLOSURE %d %d %d %d", xcs, ycs, xbe, ybe, xte, yte);
        if (via->hasRowCol()) {
    ...
    case defrRegionCbkType:
      ...
      for (i = 0; i < re->numRectangles(); i++)
        fprintf(fout, "REGION %s ( %d %d ) ( %d %d )\n", re->name(), re->xl(i), re->yl(i), re->xh(i), re->yh(i));
      if (re->hasType())
    ...
    case defrScanchainCbkType:
      ...
      if (sc->hasPartition()) {
        fprintf(fout, "SCANCHAINS %s PARTITION %s", sc->name(), sc->partitionName());
    ...
    case defrIOTimingCbkType:
      ...
      if (iot->hasCapacitance())
        fprintf(fout, "IOTIMING %s CAPACITANCE %g\n", iot->inst(), checkDouble(iot->capacitance()));
      if (iot->hasDriveCell()) {
        fprintf(fout, "IOTIMING %s DRIVECELL %s ", iot->inst(), iot->driveCell());
        if (iot->hasFrom())
    ...
    case defrTimingDisableCbkType:
      ...
      if (td->hasFromTo())
        fprintf(fout, "TIMINGDISABLE FROMPIN %s %s ", td->fromInst(), td->fromPin(), td->toInst(), td->toPin());
      if (td->hasThru())
      ...
      if (td->hasMacroThru())
        fprintf(fout, " MACRO %s THRUPIN %s %s ", td->macroName(), td->fromPin());
      fprintf(fout, "\n");
      break;
    case defrPartitionCbkType:
      ...
      if (strcmp(itemT, "CLOCK") == 0) {
        ...
        if (part->hasMin())
          fprintf(fout, "MIN %g %g ", checkDouble(part->partitionMin()), checkDouble(part->partitionMax()));
        if (part->hasMax())
          fprintf(fout, "MAX %g %g ", checkDouble(part->partitionMin()), checkDouble(part->partitionMax()));
        fprintf(fout, "PINS ");
    ...
    case defrPinPropCbkType:
      ...
      if (pprop->numProps() > 0) {
        for (i = 0; i < pprop->numProps(); i++) {
          fprintf(fout, "PINPROP PIN %s PROP %s %s\n", pprop->pinName(), pprop->propName(i), pprop->propValue(i));
        }
      }
      --numObjs;
      break;
    ...
    case defrSlotCbkType:
      ...
      for (i = 0; i < slot->numRectangles(); i++) {
        ...
        fprintf(fout, " RECT %d %d %d %d\n", slot->xl(i), slot->yl(i), slot->xh(i), slot->yh(i));
      }
    ...
    case defrFillCbkType:
      ...
      for (i = 0; i < fill->numRectangles(); i++) {
        ...
        if (fill->hasLayerOpc())
          fprintf(fout, " OPC");
        fprintf(fout, " RECT %d %d %d %d\n", fill->xl(i), fill->yl(i), fill->xh(i), fill->yh(i));
      }
      ...
      if (fill->hasVia()) {
        ...
        if (fill->viaTopMask() || fill->viaCutMask() || fill->viaBottomMask()) {
          fprintf(fout, " MASK %d%d%d", fill->viaTopMask(), fill->viaCutMask(), fill->viaBottomMask());
        }
        if (fill->hasViaOpc())
          fprintf(fout, " OPC\n");

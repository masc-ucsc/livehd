 ...
 **********************************************************/
defiComponentMaskShiftLayer::defiComponentMaskShiftLayer(defrData* data) : defData(data) { Init(); }

defiComponentMaskShiftLayer::~defiComponentMaskShiftLayer() { Destroy(); }
...
int defiComponentMaskShiftLayer::numMaskShiftLayers() const { return numLayers_; }
 ...
 * class defiComponent
 **********************************************************/
defiComponent::defiComponent(defrData* data) : defData(data) { Init(); }
...
defiComponent::~defiComponent() { Destroy(); }
...
const char* defiComponent::source() const { return source_; }

int defiComponent::weight() const { return weight_; }
...
void defiComponent::setPlacementStatus(int n) { status_ = n; }
...
// 5.7
void defiComponent::setHaloSoft() { hasHaloSoft_ = 1; }

// 5.7
void defiComponent::setRouteHalo(int haloDist, const char* minLayer, const char* maxLayer) {
  int len;
...
const char* defiComponent::id() const { return id_; }

const char* defiComponent::name() const { return name_; }

int defiComponent::placementStatus() const { return status_; }

int defiComponent::placementX() const { return x_; }

int defiComponent::placementY() const { return y_; }

int defiComponent::placementOrient() const { return orient_; }

const char* defiComponent::placementOrientStr() const { return (defiOrientStr(orient_)); }

const char* defiComponent::regionName() const { return regionName_; }

const char* defiComponent::EEQ() const { return EEQ_; }

const char* defiComponent::generateName() const { return generateName_; }

const char* defiComponent::macroName() const { return macroName_; }

void defiComponent::regionBounds(int* size, int** xl, int** yl, int** xh, int** yh) const {
  *size = numRects_;
  *xl   = rectXl_;
  *yl   = rectYl_;
  *xh   = rectXh_;
...
int defiComponent::isUnplaced() const { return status_ == DEFI_COMPONENT_UNPLACED ? 1 : 0; }

int defiComponent::isPlaced() const { return status_ == DEFI_COMPONENT_PLACED ? 1 : 0; }

int defiComponent::isFixed() const { return status_ == DEFI_COMPONENT_FIXED ? 1 : 0; }

int defiComponent::isCover() const { return status_ == DEFI_COMPONENT_COVER ? 1 : 0; }

void defiComponent::print(FILE* fout) const {
  fprintf(fout, "Component id '%s' name '%s'", id(), name());
  if (isPlaced()) {
    fprintf(fout, " Placed at %d,%d orient %s", placementX(), placementY(), placementOrientStr());
  }
  if (isFixed()) {
    fprintf(fout, " Fixed at %d,%d orient %s", placementX(), placementY(), placementOrientStr());
  }
  if (isCover()) {
    fprintf(fout, " Cover at %d,%d orient %s", placementX(), placementY(), placementOrientStr());
  }
  fprintf(fout, "\n");
  ...
  if (hasRegionBounds()) {
    ...
    for (j = 0; j < size; j++) fprintf(fout, "  Region bounds %d,%d %d,%d\n", xl[j], yl[j], xh[j], yh[j]);
  }
  if (hasNets()) {
    int i;
...
int defiComponent::hasRegionName() const { return (int)(hasRegionName_); }

int defiComponent::hasGenerate() const { return (int)(hasGenerate_); }

int defiComponent::hasWeight() const { return (int)(hasWeight_); }

int defiComponent::maskShiftSize() const { return maskShiftSize_; }

int defiComponent::hasSource() const { return source_ ? 1 : 0; }

int defiComponent::hasRegionBounds() const { return numRects_ ? 1 : 0; }

int defiComponent::hasEEQ() const { return (int)(hasEEQ_); }

int defiComponent::hasNets() const { return numNets_ ? 1 : 0; }

int defiComponent::numNets() const { return numNets_; }

// 5.6
int defiComponent::hasHalo() const { return hasHalo_; }

// 5.7
int defiComponent::hasHaloSoft() const { return hasHaloSoft_; }

// 5.7
int defiComponent::hasRouteHalo() const { return haloDist_; }

// 5.7
int defiComponent::haloDist() const { return haloDist_; }

// 5.7
const char* defiComponent::minLayer() const { return minLayer_; }

// 5.7
const char* defiComponent::maxLayer() const { return maxLayer_; }
...
int defiComponent::numProps() const { return numProps_; }

void defiComponent::addProperty(const char* name, const char* value, const char type) {
  int len = strlen(name) + 1;
...
void defiComponent::addNumProperty(const char* name, const double d, const char* value, const char type) {
  int len = strlen(name) + 1;
  if (numProps_ == propsAllocated_) {
    int     i;
    char**  nn;
...
int defiComponent::hasForeignName() const { return (int)(hasForeignName_); }

const char* defiComponent::foreignName() const { return foreignName_; }

int defiComponent::foreignX() const { return Fx_; }

int defiComponent::foreignY() const { return Fy_; }

int defiComponent::hasFori() const { return (int)(hasFori_); }
...
int defiComponent::foreignOrient() const { return Fori_; }

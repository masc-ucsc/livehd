...
defiRegion::defiRegion(defrData* data) : defData(data) { Init(); }
...
void defiRegion::addProperty(const char* name, const char* value, const char type) {
  int len;
...
void defiRegion::addNumProperty(const char* name, const double d, const char* value, const char type) {
  int len;
  if (numProps_ == propsAllocated_) {
    int     i;
    char**  nn;
...
int defiRegion::hasType() const { return type_ ? 1 : 0; }

const char* defiRegion::type() const { return type_; }

int defiRegion::numRectangles() const { return numRectangles_; }

int defiRegion::numProps() const { return numProps_; }

const char* defiRegion::propName(int index) const {
  char msg[160];
  if (index < 0 || index >= numProps_) {
    sprintf(msg,
            "ERROR (DEFPARS-6130): The index number %d specified for the REGION PROPERTY is invalide.\nValid index number is from "
            "0 to %d. Specify a valid index number and then try again.",
            index,
            numProps_);
...
const char* defiRegion::name() const { return name_; }

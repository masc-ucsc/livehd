...
const char *typeToString(defrCallbackType_e num) {
  switch ((int)num) {
    ...
    case defrComponentMaskShiftLayerCbkType: return "ComponentMaskShiftLayer";
...
// *****************************************************************
int defrInit() { return defrInitSession(0); }
...
void defrPrintUnusedCallbacks(FILE *log) {
  ...
  for (i = 0; i < CBMAX; i++) {
    if (defContext.settings->UnusedCallbacks[i]) {
      if (first) {
        fprintf(log, "WARNING (DEFPARS-5001): DEF statement found in the def file with no callback set.\n");
        first = 0;
      }
...
// obsoleted
int defrReleaseNResetMemory() { return 0; }
...
int defrRead(FILE *f, const char *fName, defiUserData uData, int case_sensitive) {
  ...
  defData->NeedPathData = (((defData->callbacks->NetCbk || defData->callbacks->SNetCbk) && defData->settings->AddPathToNet)
...
defiUserData defrGetUserData() { return defContext.session->UserData; }

...
defiVia::defiVia(defrData* data) : defData(data) { Init(); }
...
void defiVia::addViaRule(char* viaRuleName, int xSize, int ySize, char* botLayer, char* cutLayer, char* topLayer, int xSpacing,
                         int ySpacing, int xBotEnc, int yBotEnc, int xTopEnc, int yTopEnc) {
  int len;
...
int defiVia::hasPattern() const { return hasPattern_; }

const char* defiVia::pattern() const { return pattern_; }

const char* defiVia::name() const { return name_; }

int defiVia::numLayers() const { return numLayers_; }
...
int defiVia::numPolygons() const { return numPolys_; }

const char* defiVia::polygonName(int index) const {
  char msg[160];
  if (index < 0 || index > numPolys_) {
    sprintf(msg,
            "ERROR (DEFPARS-6180): The index number %d specified for the VIA POLYGON is invalid.\nValid index is from 0 to %d. "
...
void defiVia::viaRule(char** viaRuleName, int* xSize, int* ySize, char** botLayer, char** cutLayer, char** topLayer,
                      int* xCutSpacing, int* yCutSpacing, int* xBotEnc, int* yBotEnc, int* xTopEnc, int* yTopEnc) const {
...
int defiVia::hasCutPattern() const { return hasCutPattern_; }
...
const char* defiVia::cutPattern() const { return cutPattern_; }

...
defiOrdered::~defiOrdered() { Destroy(); }
...
void defiOrdered::setOrderedBits(int bits) { bits_[num_ - 1] = bits; }

int defiOrdered::num() const { return num_; }

char** defiOrdered::inst() const { return inst_; }

char** defiOrdered::in() const { return in_; }

char** defiOrdered::out() const { return out_; }

int* defiOrdered::bits() const { return bits_; }

defiScanchain::defiScanchain(defrData* data) : defData(data) { Init(); }
...
defiScanchain::~defiScanchain() { Destroy(); }
...
const char* defiScanchain::name() const { return name_; }
...
void defiScanchain::setFloatingBits(int bits) { floatBits_[numFloating_ - 1] = bits; }
...
int defiScanchain::hasStart() const { return (int)(hasStart_); }

int defiScanchain::hasStop() const { return (int)(hasStop_); }

int defiScanchain::hasFloating() const { return numFloating_ ? 1 : 0; }

int defiScanchain::hasOrdered() const { return numOrdered_ ? 1 : 0; }

// 5.4.1
int defiScanchain::hasPartition() const { return hasPartition_; }

// 5.4.1
int defiScanchain::hasPartitionMaxBits() const { return (maxBits_ != -1) ? 1 : 0; }

// 5.4.1
const char* defiScanchain::partitionName() const { return partName_; }
...
void defiScanchain::ordered(int index, int* size, char*** inst, char*** inPin, char*** outPin, int** bits) const {
  defiOrdered* o;
...
void defiScanchain::floating(int* size, char*** inst, char*** inPin, char*** outPin, int** bits) const {
  *size   = numFloating_;
  *inst   = floatInst_;
  *inPin  = floatIn_;
  *outPin = floatOut_;
...
int defiScanchain::hasCommonInPin() const { return commonInPin_ ? 1 : 0; }

int defiScanchain::hasCommonOutPin() const { return commonOutPin_ ? 1 : 0; }

const char* defiScanchain::commonInPin() const { return commonInPin_; }

const char* defiScanchain::commonOutPin() const { return commonOutPin_; }
...
void defiScanchain::print(FILE* f) const {
  ...
  fprintf(f, "Scan chain '%s'  %d things\n", name(), hasStart() + hasStop() + numFloating_ + numOrderedLists());

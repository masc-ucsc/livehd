...
void defiPath::initTraverse() const { *(pointer_) = -1; }

void defiPath::initTraverseBackwards() const { *(pointer_) = numUsed_; }
...
void defiPath::print(FILE* fout) const {
  ...
  for (i = 0; i < numUsed_; i++) {
    if (keys_[i] == 'L') {
      ...
      fprintf(fout, " point %d,%d\n", ((struct defiPnt*)(data_[i]))->x, ((struct defiPnt*)(data_[i]))->y);
    } else if (keys_[i] == 'F') {
      fprintf(fout,
              " flushpoint %d,%d,%d\n",

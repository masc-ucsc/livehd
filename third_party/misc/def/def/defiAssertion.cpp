...
defiAssertion::defiAssertion(defrData* data) : defData(data) { Init(); }

defiAssertion::~defiAssertion() { Destroy(); }
...
void defiAssertion::setConstraintMode() { isAssertion_ = 0; }

void defiAssertion::setAssertionMode() { isAssertion_ = 1; }

void defiAssertion::setWiredlogicMode() { isWiredlogic_ = 1; }
...
void defiAssertion::setWiredlogic(const char* name, double dist) {
  ...
  if (isDelay_)
    defiError(0,
              6201,
              "ERROR (DEFPARS-6201): Unable to process the DEF file. Both WIREDLOGIC and DELAY statements are defined in "
...
void defiAssertion::setSum() {
  if (isDiff_)
    defiError(0,
              6202,
              "ERROR (DEPPARS-6202): Unable to process the DEF file. Both SUM and DIFF statements are defined in "
              "constraint/assertion.\nUpdate the DEF file to define either a SUM or DIFF statement only.",
              defData);
  isSum_ = 1;
}

void defiAssertion::unsetSum() { isSum_ = 0; }
...
const char* defiAssertion::netName() const { return netName_; }
...
int defiAssertion::isDelay() const { return isDelay_ ? 1 : 0; }

int defiAssertion::isAssertion() const { return isAssertion_ ? 1 : 0; }

int defiAssertion::isConstraint() const { return isAssertion_ ? 0 : 1; }

int defiAssertion::isSum() const { return isSum_; }

int defiAssertion::isDiff() const { return isDiff_; }

int defiAssertion::isWiredlogic() const { return isWiredlogic_; }

int defiAssertion::hasRiseMin() const { return hasRiseMin_; }

int defiAssertion::hasRiseMax() const { return hasRiseMax_; }

int defiAssertion::hasFallMin() const { return hasFallMin_; }

int defiAssertion::hasFallMax() const { return hasFallMax_; }

double defiAssertion::distance() const {
  return fallMax_;  // distance is stored here
}

double defiAssertion::riseMin() const { return riseMin_; }

double defiAssertion::riseMax() const { return riseMax_; }

double defiAssertion::fallMin() const { return fallMin_; }

double defiAssertion::fallMax() const { return fallMax_; }
...
int defiAssertion::numItems() const { return numItems_; }
...
void defiAssertion::path(int index, char** fromInst, char** fromPin, char** toInst, char** toPin) const {
...
void defiAssertion::addPath(const char* fromInst, const char* fromPin, const char* toInst, const char* toPin) {
  int                    i;
  struct defiAssertPath* s;
...
void defiAssertion::print(FILE* f) const { fprintf(f, "Assertion %s\n", netName()); }

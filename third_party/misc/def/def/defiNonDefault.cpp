...
defiNonDefault::defiNonDefault(defrData* data) : defData(data) { Init(); }
...
defiNonDefault::~defiNonDefault() { Destroy(); }
...
void defiNonDefault::clear() {
  ...
  numProps_ = 0;
  for (i = 0; i < numLayers_; i++) free(layerName_[i]);
  numLayers_ = 0;
  for (i = 0; i < numVias_; i++) free((char*)(viaNames_[i]));
  numVias_ = 0;
  for (i = 0; i < numViaRules_; i++) free((char*)(viaRuleNames_[i]));
  numViaRules_ = 0;
  for (i = 0; i < numMinCuts_; i++) free((char*)(cutLayerName_[i]));
  numMinCuts_ = 0;
}
...
void defiNonDefault::setHardspacing() { hardSpacing_ = 1; }
...
void defiNonDefault::addWidth(double num) { width_[numLayers_ - 1] = num; }
...
const char* defiNonDefault::name() const { return name_; }

int defiNonDefault::hasHardspacing() const { return hardSpacing_; }

int defiNonDefault::numLayers() const { return numLayers_; }
...
int defiNonDefault::numVias() const { return numVias_; }
...
int defiNonDefault::numViaRules() const { return numViaRules_; }
...
int defiNonDefault::numMinCuts() const { return numMinCuts_; }
...
int defiNonDefault::numProps() const { return numProps_; }

void defiNonDefault::addProperty(const char* name, const char* value, const char type) {
  int len = strlen(name) + 1;
...
void defiNonDefault::addNumProperty(const char* name, const double d, const char* value, const char type) {
  int len = strlen(name) + 1;
  if (numProps_ == propsAllocated_) {
    int     i;
    int     max;

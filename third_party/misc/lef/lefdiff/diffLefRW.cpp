...
void lefVia(lefiVia* via, char* inName) {
  ...
  if (via->hasForeign()) {
    ...
    if (via->hasForeignPnt()) {
      fprintf(fout, " ( %g %g )", chkNum(via->foreignX()), chkNum(via->foreignY()));
  ...
  if (via->hasViaRule()) {
    ...
    if (via->hasOffset())
      fprintf(fout, " %g %g %g %g", via->xBotOffset(), via->yBotOffset(), via->xTopOffset(), via->yTopOffset());
    if (via->hasCutPattern())
      fprintf(fout, " %s", via->cutPattern());
    fprintf(fout, "\n");
  }
  if (via->hasResistance())
    fprintf(fout, "%s %s RESISTANCE %g\n", inName, via->name(), chkNum(via->resistance()));
  if (via->numLayers() > 0) {
    for (i = 0; i < via->numLayers(); i++) {
      ...
      for (j = 0; j < via->numPolygons(i); j++) {
        ...
        if (via->polyColorMask(i, j)) {
          fprintf(fout, "%s %s POLYGON MASK %d", inName, via->name(), via->polyColorMask(i, j));
        } else {
...
void lefSpacing(lefiSpacing* spacing, char* inName) {
  fprintf(fout, "%s SAMENET %s %s %g ", inName, spacing->name1(), spacing->name2(), chkNum(spacing->distance()));
  if (spacing->hasStack())
    fprintf(fout, " STACK");
  fprintf(fout, "\n");
  return;
...
void lefViaRuleLayer(lefiViaRuleLayer* vLayer, char* viaName) {
  ...
  if (vLayer->hasEnclosure())
    fprintf(fout, " ENCLOSURE %g %g\n", chkNum(vLayer->enclosureOverhang1()), chkNum(vLayer->enclosureOverhang2()));
  if (vLayer->hasWidth())
    fprintf(fout, " WIDTH %g TO %g", chkNum(vLayer->widthMin()), chkNum(vLayer->widthMax()));
  ...
  if (vLayer->hasSpacing())
    fprintf(fout, " SPACING %g BY %g", chkNum(vLayer->spacingStepX()), chkNum(vLayer->spacingStepY()));
  if (vLayer->hasRect())
    fprintf(fout,
            " RECT ( %g %g ) ( %g %g )",
            chkNum(vLayer->xl()),
...
void prtGeometry(lefiGeometries* geometry, char* inName) {
  ...
  for (i = 0; i < numItems; i++) {
    switch (geometry->itemType(i)) {
      case lefiGeomClassE: fprintf(fout, "%s CLASS %s\n", inName, geometry->getClass(i)); break;
      case lefiGeomLayerE: fprintf(fout, "%s LAYER %s\n", inName, geometry->getLayer(i)); break;
      case lefiGeomLayerMinSpacingE: fprintf(fout, "%s SPACING %g\n", inName, chkNum(geometry->getLayerMinSpacing(i))); break;
      case lefiGeomLayerExceptPgNetE: fprintf(fout, "%s EXCEPTPGNET\n", inName); break;
      case lefiGeomLayerRuleWidthE: fprintf(fout, "%s DESIGNRULEWIDTH %g\n", inName, chkNum(geometry->getLayerRuleWidth(i))); break;
      case lefiGeomWidthE: fprintf(fout, "%s WIDTH %g\n", inName, chkNum(geometry->getWidth(i))); break;
      case lefiGeomPathE:
      ...
      case lefiGeomPathIterE:
        ...
        if (pathIter->colorMask != 0) {
          fprintf(fout, "%s PATH MASK %d ITERATED ", inName, pathIter->colorMask);
        } else {
          fprintf(fout, "%s PATH ITERATED ", inName);
        }
        for (j = 0; j < pathIter->numPoints; j++) fprintf(fout, " ( %g %g )", chkNum(pathIter->x[j]), chkNum(pathIter->y[j]));
        fprintf(fout,
                " DO %g BY %g STEP %g %g\n",
      ...
      case lefiGeomPolygonE:
        ...
        for (j = 0; j < polygon->numPoints; j++) {
          if (j + 1 == polygon->numPoints)  // last one on the list
            fprintf(fout, " ( %g %g )\n", chkNum(polygon->x[j]), chkNum(polygon->y[j]));
          else
            fprintf(fout, " ( %g %g )", chkNum(polygon->x[j]), chkNum(polygon->y[j]));
        }
        break;
      case lefiGeomPolygonIterE:
        polygonIter = geometry->getPolygonIter(i);
        if (polygonIter->colorMask != 0) {
          fprintf(fout, "%s POLYGON MASK %d ITERATE", inName, polygonIter->colorMask);
        } else {
          fprintf(fout, "%s POLYGON ITERATE", inName);
        }
        for (j = 0; j < polygonIter->numPoints; j++)
          fprintf(fout, " ( %g %g )", chkNum(polygonIter->x[j]), chkNum(polygonIter->y[j]));
        fprintf(fout,
                " DO %g BY %g STEP %g %g\n",
      ...
      case lefiGeomViaE:
        ...
        if (via->topMaskNum != 0 || via->bottomMaskNum != 0 || via->cutMaskNum != 0) {
          ...
          fprintf(fout, "%s VIA ( %g %g ) %s\n", inName, chkNum(via->x), chkNum(via->y), via->name);
        }
        break;
      case lefiGeomViaIterE:
        ...
        if (viaIter->topMaskNum != 0 || viaIter->cutMaskNum != 0 || viaIter->bottomMaskNum != 0) {
          ...
          fprintf(fout, "%s VIA ITERATE ( %g %g ) %s", inName, chkNum(viaIter->x), chkNum(viaIter->y), viaIter->name);
        }
        fprintf(fout,
                " DO %g BY %g STEP %g %g\n",
...
int antennaCB(lefrCallbackType_e c, double value, lefiUserData ud) {
  ...
  switch (c) {
    case lefrAntennaInputCbkType: fprintf(fout, "ANTINPUTGATEAREA %g\n", chkNum(value)); break;
    case lefrAntennaInoutCbkType: fprintf(fout, "ANTINOUTDIFFAREA %g\n", chkNum(value)); break;
    case lefrAntennaOutputCbkType: fprintf(fout, "ANTOUTPUTDIFFAREA %g\n", chkNum(value)); break;
    ...
    for (i = 0; i < a->numTrack(); i++) {
      ...
      if (track->numLayers() == 0) {
        ...
        fprintf(fout,
                ...
                track->numTracks(),
                chkNum(track->space()));
        for (j = 0; j < track->numLayers(); j++) fprintf(fout, "%s ", track->layerName(j));
        fprintf(fout, ";\n");
      }
    }
  }
...
int correctTableCB(lefrCallbackType_e c, lefiCorrectionTable* table, lefiUserData ud) {
  int                       i, j, k, l;
...
int irdropCB(lefrCallbackType_e c, lefiIRDrop* irdrop, lefiUserData ud) {
  ...
  for (i = 0; i < irdrop->numValues(); i++) fprintf(fout, "%g %g ", chkNum(irdrop->value1(i)), chkNum(irdrop->value2(i)));
  fprintf(fout, "\n");
  return 0;
}
...
int layerCB(lefrCallbackType_e c, lefiLayer* layer, lefiUserData ud) {
  ...
  if (layer->hasDensityCheckStep())
    fprintf(fout, "LAYER %s DENSITYCHECKSTEP %g\n", layer->name(), chkNum(layer->densityCheckStep()));
  if (layer->hasFillActiveSpacing())
    fprintf(fout, "LAYER %s FILLACTIVESPACING %g\n", layer->name(), chkNum(layer->fillActiveSpacing()));
  numMinCut = layer->numMinimumcut();  // 5.4.1
  if (numMinCut > 0) {
    for (i = 0; i < numMinCut; i++) {
      fprintf(fout, "LAYER %s MINIMUMCUT %d WIDTH %g", layer->name(), layer->minimumcut(i), layer->minimumcutWidth(i));
      ...
      if (layer->hasMinimumcutNumCuts(i))
        fprintf(fout, " LENGTH %g WITHIN %g", layer->minimumcutLength(i), layer->minimumcutDistance(i));
      fprintf(fout, "\n");
    }
  }
  ...
  if (numMinenclosed > 0) {
    for (i = 0; i < numMinenclosed; i++) {  // 5.5, made it multiples
      fprintf(fout, "LAYER %s MINENCLOSEDAREA %g", layer->name(), layer->minenclosedarea(i));
      if (layer->hasMinenclosedareaWidth(i))
        fprintf(fout, " MINENCLOSEDAREAWIDTH %g", layer->minenclosedareaWidth(i));
      fprintf(fout, "\n");
    }
  }
  ...
  if (layer->hasSpacingNumber()) {
    for (i = 0; i < layer->numSpacing(); i++) {
      fprintf(fout, "LAYER %s SPACING %g", layer->name(), chkNum(layer->spacing(i)));
      ...
      if (layer->hasSpacingArea(i))
        fprintf(fout, "LAYER %s AREA %g\n", layer->name(), chkNum(layer->spacingArea(i)));
      if (layer->hasSpacingRange(i)) {
        fprintf(fout, "LAYER %s RANGE %g %g", layer->name(), chkNum(layer->spacingRangeMin(i)), chkNum(layer->spacingRangeMax(i)));
        if (layer->hasSpacingRangeUseLengthThreshold(i))
          ...
          if (layer->hasSpacingRangeInfluenceRange(i))
            fprintf(fout, "RANGE %g %g ", layer->spacingRangeInfluenceMin(i), layer->spacingRangeInfluenceMax(i));
        } else if (layer->hasSpacingRangeRange(i))
          fprintf(fout, "RANGE %g %g ", layer->spacingRangeRangeMin(i), layer->spacingRangeRangeMax(i));
        fprintf(fout, "\n");
      } else if (layer->hasSpacingLengthThreshold(i)) {
        fprintf(fout, "LAYER %s LENGTHTHRESHOLD %g\n", layer->name(), chkNum(layer->spacingLengthThreshold(i)));
        if (layer->hasSpacingLengthThresholdRange(i))
          fprintf(fout, "RANGE %g %g", layer->spacingLengthThresholdRangeMin(i), layer->spacingLengthThresholdRangeMax(i));
        fprintf(fout, "\n");
      } else if (layer->hasSpacingNotchLength(i)) {
        fprintf(fout, "LAYER %s NOTCHLENGTH %g\n", layer->name(), chkNum(layer->spacingNotchLength(i)));
      ...
      if (layer->hasSpacingEndOfLine(i)) {
        ...
        if (layer->hasSpacingParellelEdge(i)) {
          fprintf(fout, "PARALLELEDGE %g WITHIN %g ", chkNum(layer->spacingParSpace(i)), chkNum(layer->spacingParWithin(i)));
  ...
  if (layer->numEnclosure() > 0) {
    ...
    for (i = 0; i < layer->numEnclosure(); i++) {
      ...
      fprintf(fout, " %g %g", layer->enclosureOverhang1(i), layer->enclosureOverhang2(i));
  ...
  if (layer->numPreferEnclosure() > 0) {
    ...
    for (i = 0; i < layer->numPreferEnclosure(); i++) {
      ...
      fprintf(fout, "%g %g ", layer->preferEnclosureOverhang1(i), layer->preferEnclosureOverhang2(i));
  ...
  for (j = 0; j < layer->numAntennaModel(); j++) {  // 5.5
    ...
    if (aModel->hasAntennaDiffAreaRatio())
      ...
      fprintf(fout, "LAYER %s %s ANTDIFFAREARATIO PWL", layer->name(), aModel->antennaOxide());
      for (i = 0; i < pwl->numPWL(); i++) fprintf(fout, " %g %g", chkNum(pwl->PWLdiffusion(i)), chkNum(pwl->PWLratio(i)));
      fprintf(fout, "\n");
    }
    ...
    if (aModel->hasAntennaCumDiffAreaRatio())
      ...
      fprintf(fout, "LAYER %s %s ANTCUMDIFFAREARATIO PWL", layer->name(), aModel->antennaOxide());
      for (i = 0; i < pwl->numPWL(); i++) fprintf(fout, " %g %g", chkNum(pwl->PWLdiffusion(i)), chkNum(pwl->PWLratio(i)));
      fprintf(fout, "\n");
    }
    if (aModel->hasAntennaAreaFactor()) {
      fprintf(fout, "LAYER %s %s ANTAREAFACTOR %g", layer->name(), aModel->antennaOxide(), chkNum(aModel->antennaAreaFactor()));
    ...
    if (aModel->hasAntennaDiffSideAreaRatio())
      ...
      fprintf(fout, "LAYER %s %s ANTDIFFSIDEAREARATIO PWL", layer->name(), aModel->antennaOxide());
      for (i = 0; i < pwl->numPWL(); i++) fprintf(fout, " %g %g", chkNum(pwl->PWLdiffusion(i)), chkNum(pwl->PWLratio(i)));
      fprintf(fout, "\n");
    }
    ...
    if (aModel->hasAntennaCumDiffSideAreaRatio())
      ...
      fprintf(fout, "LAYER %s %s ANTCUMDIFFSIDEAREARATIO PWL", layer->name(), aModel->antennaOxide());
      for (i = 0; i < pwl->numPWL(); i++) fprintf(fout, "%g %g ", chkNum(pwl->PWLdiffusion(i)), chkNum(pwl->PWLratio(i)));
      fprintf(fout, "\n");
    }
    if (aModel->hasAntennaSideAreaFactor()) {
      fprintf(fout, "  ANTSIDEAREAFACTOR %g ", chkNum(aModel->antennaSideAreaFactor()));
  ...
  if (layer->numAccurrentDensity()) {
    for (i = 0; i < layer->numAccurrentDensity(); i++) {
      ...
      fprintf(fout, "LAYER %s ACCURRENTDENSITY %s", layer->name(), density->type());
      needHeading = 0;
      if (density->hasOneEntry())
        ...
        if (density->numFrequency()) {
          ...
          */
          ...
          for (j = 0; j < density->numFrequency(); j++) fprintf(fout, " %g", chkNum(density->frequency(j)));
          fprintf(fout, "\n");
          needHeading = 1;
        }
        if (density->numWidths()) {
          ...
          fprintf(fout, " WIDTH");
          for (j = 0; j < density->numWidths(); j++) fprintf(fout, " %g", chkNum(density->width(j)));
          fprintf(fout, "\n");
          needHeading = 1;
        }
  ...
  if (layer->numDccurrentDensity()) {
    for (i = 0; i < layer->numDccurrentDensity(); i++) {
      ...
      fprintf(fout, "LAYER %s DCCURRENTDENSITY %s", layer->name(), density->type());
      needHeading = 0;
      if (density->hasOneEntry())
        fprintf(fout, " %g\n", chkNum(density->oneEntry()));
      else {
        if (density->numCutareas()) {
          ...
          */
          ...
          for (j = 0; j < density->numCutareas(); j++) fprintf(fout, " %g", chkNum(density->cutArea(j)));
          fprintf(fout, "\n");
          needHeading = 1;
        }
        if (density->numWidths()) {
        ...
        if (density->numTableEntries()) {
          ...
          fprintf(fout, " TABLEENTRIES");
          for (j = 0; j < density->numTableEntries(); j++) fprintf(fout, " %g", chkNum(density->tableEntry(j)));
          fprintf(fout, "\n");
          needHeading = 1;
        }
      }
  ...
  for (i = 0; i < layer->numSpacingTable(); i++) {
    ...
    if (spTable->isInfluence()) {
      ...
      for (j = 0; j < parallel->numWidth(); j++) {
        fprintf(fout, "LAYER %s SPACINGTABLE PARALLELRUNLENGTH WIDTH %g", layer->name(), parallel->width(j));
      ...
      for (j = 0; j < twoWidths->numWidth(); j++) {
        ...
        for (k = 0; k < twoWidths->numWidthSpacing(j); k++) fprintf(fout, " %g", twoWidths->widthSpacing(j, k));
        fprintf(fout, "\n");
      }
    }
  }
...
int macroCB(lefrCallbackType_e c, lefiMacro* macro, lefiUserData ud) {
  ...
  if (macro->hasSize())
    fprintf(fout, "MACRO %s SIZE %g BY %g\n", macro->name(), chkNum(macro->sizeX()), chkNum(macro->sizeY()));
  if (macro->hasForeign()) {
    for (i = 0; i < macro->numForeigns(); i++) {
      fprintf(fout, "MACRO %s FOREIGN %s", macro->name(), macro->foreignName(i));
      if (macro->hasForeignPoint(i)) {
        fprintf(fout, " ( %g %g )", chkNum(macro->foreignX(i)), chkNum(macro->foreignY(i)));
  ...
  if (macro->hasOrigin())
    fprintf(fout, "MACRO %s ORIGIN ( %g %g )\n", macro->name(), chkNum(macro->originX()), chkNum(macro->originY()));
  if (macro->hasPower())
...
// Maxviastack
int maxStackViaCB(lefrCallbackType_e c, lefiMaxStackVia* maxStack, lefiUserData ud) {
  checkType(c);
  if (ud != userData)
    dataError();
  fprintf(fout, "MAXVIASTACK %d", maxStack->maxStackVia());
  if (maxStack->hasMaxStackViaRange())
    fprintf(fout, " RANGE %s %s", maxStack->maxStackViaBottomLayer(), maxStack->maxStackViaTopLayer());
  fprintf(fout, "\n");
  return 0;
}
...
int noiseMarginCB(lefrCallbackType_e c, lefiNoiseMargin* margin, lefiUserData ud) {
  checkType(c);
  if (ud != userData)
    dataError();
  fprintf(fout, "UNIVERSALNOISEMARGIN %g %g\n", chkNum(margin->high), chkNum(margin->low));
  return 0;
}
        ...
        for (j = 0; j < edge->numResistances(); j++) {
          ...
          for (k = 0; k < resist->numVictims(); k++) {
            ...
            for (l = 0; l < victim->numNoises(); l++)
              fprintf(fout,
                      "CROSSTALK NOISETABLE %d RESISTANCE %g VICTIMLENGT %g NOISEFACTOR %g\n",
                      table->num(),
                      chkNum(resist->num(j)),
                      chkNum(victim->length()),
...
int nonDefaultCB(lefrCallbackType_e c, lefiNonDefault* def, lefiUserData ud) {
  ...
  for (i = 0; i < def->numLayers(); i++) {
    ...
    if (def->hasLayerCapacitance(i))
      fprintf(fout, " CAPACITANCE CPERSQDIST %g", chkNum(def->layerCapacitance(i)));
  ...
  if (def->numUseVia() > 0) {
    ...
    // handle spacing in nondefaultrule
    for (i = 0; i < def->numUseVia(); i++) fprintf(fout, " %s", def->viaName(i));
    fprintf(fout, "\n");
  }
  ...
  if (def->numUseViaRule() > 0) {
    ...
    // handle spacing in nondefaultrule
    for (i = 0; i < def->numUseViaRule(); i++) fprintf(fout, " %s", def->viaRuleName(i));
    fprintf(fout, "\n");
  }
  ...
  if (def->numMinCuts() > 0) {
    ...
    // handle spacing in nondefaultrule
    for (i = 0; i < def->numMinCuts(); i++) fprintf(fout, " %s %d", def->cutLayerName(i), def->numCuts(i));
    fprintf(fout, "\n");
  }
...
int pinCB(lefrCallbackType_e c, lefiPin* pin, lefiUserData ud) {
  ...
  if (pin->hasOutMargin())
    fprintf(fout, "PIN %s OUTPUTNOISEMARGIN %g %g\n", pin->name(), chkNum(pin->outMarginHigh()), chkNum(pin->outMarginLow()));
  ...
  if (pin->hasInMargin())
    fprintf(fout, "PIN %s INPUTNOISEMARGIN %g %g\n", pin->name(), chkNum(pin->inMarginHigh()), chkNum(pin->inMarginLow()));
  if (pin->hasPower())
  ...
  if (pin->hasAntennaMetalArea()) {
    for (i = 0; i < pin->numAntennaMetalArea(); i++) {
      fprintf(fout, "PIN %s ANTMETALAREA %g", pin->name(), chkNum(pin->antennaMetalArea(i)));
  ...
  if (pin->hasAntennaMetalLength()) {
    for (i = 0; i < pin->numAntennaMetalLength(); i++) {
      fprintf(fout, "PIN %s ANTMETALLENGTH %g", pin->name(), chkNum(pin->antennaMetalLength(i)));
  ...
  if (pin->hasAntennaPartialMetalArea()) {
    for (i = 0; i < pin->numAntennaPartialMetalArea(); i++) {
      fprintf(fout, "PIN %s ANTPARTIALMETALAREA %g", pin->name(), chkNum(pin->antennaPartialMetalArea(i)));
  ...
  if (pin->hasAntennaPartialMetalSideArea()) {
    for (i = 0; i < pin->numAntennaPartialMetalSideArea(); i++) {
      fprintf(fout, "PIN %s ANTPARTIALMETALSIDEAREA %g", pin->name(), chkNum(pin->antennaPartialMetalSideArea(i)));
  ...
  if (pin->hasAntennaPartialCutArea()) {
    for (i = 0; i < pin->numAntennaPartialCutArea(); i++) {
      fprintf(fout, "PIN %s ANTPARTIALCUTAREA %g", pin->name(), chkNum(pin->antennaPartialCutArea(i)));
  ...
  if (pin->hasAntennaDiffArea()) {
    for (i = 0; i < pin->numAntennaDiffArea(); i++) {
      fprintf(fout, "PIN %s ANTDIFFAREA %g", pin->name(), chkNum(pin->antennaDiffArea(i)));
  ...
  for (j = 0; j < pin->numAntennaModel(); j++) {
    ...
    if (aModel->hasAntennaGateArea()) {
      for (i = 0; i < aModel->numAntennaGateArea(); i++) {
        fprintf(fout, "PIN %s %s ANTGATEAREA %g", pin->name(), aModel->antennaOxide(), chkNum(aModel->antennaGateArea(i)));
    ...
    if (aModel->hasAntennaMaxAreaCar()) {
      for (i = 0; i < aModel->numAntennaMaxAreaCar(); i++) {
        fprintf(fout, "PIN %s %s ANTMAXAREACAR %g", pin->name(), aModel->antennaOxide(), chkNum(aModel->antennaMaxAreaCar(i)));
    ...
    if (aModel->hasAntennaMaxSideAreaCar()) {
      for (i = 0; i < aModel->numAntennaMaxSideAreaCar(); i++) {
        fprintf(fout, "PIN %s %s ANTMAXAREACAR %g", pin->name(), aModel->antennaOxide(), chkNum(aModel->antennaMaxSideAreaCar(i)));
    ...
    if (aModel->hasAntennaMaxCutCar()) {
      for (i = 0; i < aModel->numAntennaMaxCutCar(); i++) {
        fprintf(fout, "PIN %s %s ANTMAXCUTCAR %g", pin->name(), aModel->antennaOxide(), chkNum(aModel->antennaMaxCutCar(i)));
...
int propDefCB(lefrCallbackType_e c, lefiProp* prop, lefiUserData ud) {
  ...
  switch (prop->dataType()) {
    case 'I': fprintf(fout, " TYPE I"); break;
    case 'R': fprintf(fout, " TYPE R"); break;
    case 'S': fprintf(fout, " TYPE S"); break;
    case 'Q': fprintf(fout, " TYPE Q"); break;
    case 'N': fprintf(fout, " TYPE N"); break;
  }
  fprintf(fout, "\n");
  return 0;
}
...
int timingCB(lefrCallbackType_e c, lefiTiming* timing, lefiUserData ud) {
  ...
  if (ud != userData)
    dataError();
  for (i = 0; i < timing->numFromPins(); i++) fprintf(fout, "TIMING FROMPIN %s\n", timing->fromPin(i));
  for (i = 0; i < timing->numToPins(); i++) fprintf(fout, "TIMING TOPIN %s\n", timing->toPin(i));
  fprintf(fout,
  ...
  if (timing->hasRiseIntrinsic()) {
    fprintf(fout, "TIMING RISE INTRINSIC %g %g\n", chkNum(timing->riseIntrinsicOne()), chkNum(timing->riseIntrinsicTwo()));
    fprintf(fout, "TIMING RISE VARIABLE %g %g\n", chkNum(timing->riseIntrinsicThree()), chkNum(timing->riseIntrinsicFour()));
  }
  if (timing->hasFallIntrinsic()) {
    fprintf(fout, "TIMING FALL INTRINSIC %g %g\n", chkNum(timing->fallIntrinsicOne()), chkNum(timing->fallIntrinsicTwo()));
    fprintf(fout, "TIMING RISE VARIABLE %g %g\n", chkNum(timing->fallIntrinsicThree()), chkNum(timing->fallIntrinsicFour()));
  }
  if (timing->hasRiseRS())
  ...
  if (timing->hasRiseAtt1())
    fprintf(fout, "TIMING RISESATT1 %g %g\n", chkNum(timing->riseAtt1One()), chkNum(timing->riseAtt1Two()));
  if (timing->hasFallAtt1())
    fprintf(fout, "TIMING FALLSATT1 %g %g\n", chkNum(timing->fallAtt1One()), chkNum(timing->fallAtt1Two()));
  if (timing->hasRiseTo())
    fprintf(fout, "TIMING RISET0 %g %g\n", chkNum(timing->riseToOne()), chkNum(timing->riseToTwo()));
  if (timing->hasFallTo())
    fprintf(fout, "TIMING FALLT0 %g %g\n", chkNum(timing->fallToOne()), chkNum(timing->fallToTwo()));
  return 0;
}
...
int unitsCB(lefrCallbackType_e c, lefiUnits* unit, lefiUserData ud) {
  ...
  if (ud != userData)
    dataError();
  if (unit->hasDatabase())
    fprintf(fout, "UNITS DATABASE %s %g\n", unit->databaseName(), chkNum(unit->databaseNumber()));
...
// UseMinSpacing
int useMinSpacingCB(lefrCallbackType_e c, lefiUseMinSpacing* spacing, lefiUserData ud) {
  checkType(c);
  if (ud != userData)
    dataError();
...
int viaRuleCB(lefrCallbackType_e c, lefiViaRule* viaRule, lefiUserData ud) {
  ...
  if (numLayers == 2) {  // should have vianames
    ...
    if (numVias == 0)
      ...
      for (i = 0; i < numVias; i++) fprintf(fout, "VIARULE %s  VIA %s ;\n", viaRule->name(), viaRule->viaName(i));
    }
  }
  if (viaRule->numProps() > 0) {
    for (i = 0; i < viaRule->numProps(); i++) {
      fprintf(fout, "VIARULE %s PROP %s ", viaRule->name(), viaRule->propName(i));

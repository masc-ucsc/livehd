...
inline static void print_nlines(int lineNum) {
  ...
  if (lefSettings->LineNumberFunction && (lineNum % lefSettings->DeltaNumberLines) == 0)
...
int lefsublex() {
  ...
  if (isalpha(fc) || fc == '&' || fc == '_') {
    ...
    for (ch = lefData->current_token, uch = lefData->uc_token; *ch != '\0'; ch++) *uch++ = toupper(*ch);
    *uch = '\0';
...
void lefInfo(int msgNum, const char *s) {
  ...
  if (lefSettings->MsgLimit[msgNum] > 0) {
    if (lefData->msgLimit[0][msgNum] >= lefSettings->MsgLimit[msgNum]) {  // over the limit
      ...
      sprintf(msgStr, "Message (LEFPARS-%d) has exceeded the message display limit of %d", msgNum, lefSettings->MsgLimit[msgNum]);

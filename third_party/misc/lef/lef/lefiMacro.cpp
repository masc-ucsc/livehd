...
lefiObstruction::lefiObstruction() : geometries_(NULL) { Init(); }

void lefiObstruction::Init() { geometries_ = 0; }

lefiObstruction::~lefiObstruction() { Destroy(); }

void lefiObstruction::Destroy() { clear(); }
...
lefiGeometries *lefiObstruction::geometries() const { return geometries_; }
...
lefiDensity::~lefiDensity() { Destroy(); }

void lefiDensity::Destroy() { clear(); }
...
void lefiDensity::addRect(double x1, double y1, double x2, double y2, double value) {
  if (numRects_[numLayers_ - 1] == rectsAllocated_[numLayers_ - 1]) {
    lefiGeomRect *rs, *ors;
    double *      dv, *odv;
    int           i;

    rectsAllocated_[numLayers_ - 1] = (rectsAllocated_[numLayers_ - 1] == 0) ? 2 : rectsAllocated_[numLayers_ - 1] * 2;

    rs = (lefiGeomRect *)lefMalloc(sizeof(lefiGeomRect) * rectsAllocated_[numLayers_ - 1]);
...
int lefiDensity::numLayer() const { return numLayers_; }

char *lefiDensity::layerName(int index) const { return layerName_[index]; }

int lefiDensity::numRects(int index) const { return numRects_[index]; }
...
void lefiDensity::print(FILE *f) const {
  ...
  for (i = 0; i < numLayers_; i++) {
    ...
    for (j = 0; j < numRects_[i]; j++) {
      fprintf(f, "      RECT %g %g %g %g ", rects_[i][j].xl, rects_[i][j].yl, rects_[i][j].xh, rects_[i][j].yh);
      fprintf(f, "%g\n", densityValue_[i][j]);
    }
  }
}
...
lefiPinAntennaModel::~lefiPinAntennaModel() { Destroy(); }

void lefiPinAntennaModel::Destroy() { clear(); }
...
void lefiPinAntennaModel::addAntennaMaxSideAreaCar(double val, const char *layer) {
  ...
  if (layer) {  // layer can be null, since it is optional
    len                                                    = strlen(layer) + 1;
    antennaMaxSideAreaCarLayer_[numAntennaMaxSideAreaCar_] = (char *)lefMalloc(len);
...
int lefiPinAntennaModel::hasAntennaGateArea() const { return numAntennaGateArea_ ? 1 : 0; }

int lefiPinAntennaModel::hasAntennaMaxAreaCar() const { return numAntennaMaxAreaCar_ ? 1 : 0; }

int lefiPinAntennaModel::hasAntennaMaxSideAreaCar() const { return numAntennaMaxSideAreaCar_ ? 1 : 0; }

int lefiPinAntennaModel::hasAntennaMaxCutCar() const { return numAntennaMaxCutCar_ ? 1 : 0; }

// 5.5
char *lefiPinAntennaModel::antennaOxide() const { return oxide_; }

const char *lefiPinAntennaModel::antennaGateAreaLayer(int i) const { return antennaGateAreaLayer_[i]; }

const char *lefiPinAntennaModel::antennaMaxAreaCarLayer(int i) const { return antennaMaxAreaCarLayer_[i]; }

const char *lefiPinAntennaModel::antennaMaxSideAreaCarLayer(int i) const { return antennaMaxSideAreaCarLayer_[i]; }

const char *lefiPinAntennaModel::antennaMaxCutCarLayer(int i) const { return antennaMaxCutCarLayer_[i]; }

int lefiPinAntennaModel::numAntennaGateArea() const { return numAntennaGateArea_; }

int lefiPinAntennaModel::numAntennaMaxAreaCar() const { return numAntennaMaxAreaCar_; }

int lefiPinAntennaModel::numAntennaMaxSideAreaCar() const { return numAntennaMaxSideAreaCar_; }

int lefiPinAntennaModel::numAntennaMaxCutCar() const { return numAntennaMaxCutCar_; }

double lefiPinAntennaModel::antennaGateArea(int i) const { return antennaGateArea_[i]; }

double lefiPinAntennaModel::antennaMaxAreaCar(int i) const { return antennaMaxAreaCar_[i]; }

double lefiPinAntennaModel::antennaMaxSideAreaCar(int i) const { return antennaMaxSideAreaCar_[i]; }

double lefiPinAntennaModel::antennaMaxCutCar(int i) const { return antennaMaxCutCar_[i]; }

int lefiPinAntennaModel::hasReturn() const { return hasReturn_; }
...
void lefiPin::addPort(lefiGeometries *g) {
  if (numPorts_ == portsAllocated_) {
    ...
    for (i = 0; i < numPorts_; i++) ng[i] = ports_[i];
    lefFree((char *)(ports_));
    ports_ = ng;
  }
  ports_[numPorts_++] = g;
...
void lefiPin::addForeign(const char *name, int hasPnt, double x, double y, int orient) {
  int     i;
  int *   hfo;
  int *   hfp;
  int *   fo;
...
void lefiPin::setNetExpr(const char *name) { netEpxr_ = strdup(name); }

void lefiPin::setSupplySensitivity(const char *pinName) { ssPinName_ = strdup(pinName); }

void lefiPin::setGroundSensitivity(const char *pinName) { gsPinName_ = strdup(pinName); }
...
void lefiPin::setNumProperty(const char *name, double d, const char *value, const char type) {
  int len;
...
int lefiPin::hasForeign() const { return (numForeigns_) ? 1 : 0; }

int lefiPin::hasForeignOrient(int index) const { return (hasForeignOrient_[index] == -1) ? 0 : 1; }

int lefiPin::hasForeignPoint(int index) const { return hasForeignPoint_[index]; }

int lefiPin::hasLEQ() const { return hasLEQ_; }

int lefiPin::hasDirection() const { return hasDirection_; }

int lefiPin::hasUse() const { return hasUse_; }

int lefiPin::hasShape() const { return hasShape_; }

int lefiPin::hasMustjoin() const { return hasMustjoin_; }

int lefiPin::hasOutMargin() const { return hasOutMargin_; }

int lefiPin::hasOutResistance() const { return hasOutResistance_; }

int lefiPin::hasInMargin() const { return hasInMargin_; }

int lefiPin::hasPower() const { return hasPower_; }

int lefiPin::hasLeakage() const { return hasLeakage_; }

int lefiPin::hasMaxload() const { return hasMaxload_; }

int lefiPin::hasMaxdelay() const { return hasMaxdelay_; }

int lefiPin::hasCapacitance() const { return hasCapacitance_; }

int lefiPin::hasResistance() const { return hasResistance_; }

int lefiPin::hasPulldownres() const { return hasPulldownres_; }

int lefiPin::hasTieoffr() const { return hasTieoffr_; }

int lefiPin::hasVHI() const { return hasVHI_; }

int lefiPin::hasVLO() const { return hasVLO_; }

int lefiPin::hasFallSlewLimit() const { return hasFallSlewLimit_; }

int lefiPin::hasRiseSlewLimit() const { return hasRiseSlewLimit_; }

int lefiPin::hasRiseVoltage() const { return hasRiseVoltage_; }

int lefiPin::hasFallVoltage() const { return hasFallVoltage_; }

int lefiPin::hasRiseThresh() const { return hasRiseThresh_; }

int lefiPin::hasFallThresh() const { return hasFallThresh_; }

int lefiPin::hasRiseSatcur() const { return hasRiseSatcur_; }

int lefiPin::hasFallSatcur() const { return hasFallSatcur_; }

int lefiPin::hasCurrentSource() const { return hasCurrentSource_; }

int lefiPin::hasTables() const { return hasTables_; }

int lefiPin::hasAntennaSize() const { return numAntennaSize_ ? 1 : 0; }

int lefiPin::hasAntennaMetalLength() const { return numAntennaMetalLength_ ? 1 : 0; }

int lefiPin::hasAntennaMetalArea() const { return numAntennaMetalArea_ ? 1 : 0; }

int lefiPin::hasAntennaPartialMetalArea() const { return numAntennaPartialMetalArea_ ? 1 : 0; }

int lefiPin::hasAntennaPartialMetalSideArea() const { return numAntennaPartialMetalSideArea_ ? 1 : 0; }

int lefiPin::hasAntennaPartialCutArea() const { return numAntennaPartialCutArea_ ? 1 : 0; }

int lefiPin::hasAntennaDiffArea() const { return numAntennaDiffArea_ ? 1 : 0; }

int lefiPin::hasAntennaModel() const { return antennaModel_ ? 1 : 0; }

int lefiPin::hasTaperRule() const { return taperRule_ ? 1 : 0; }

int lefiPin::hasNetExpr() const { return netEpxr_ ? 1 : 0; }

int lefiPin::hasSupplySensitivity() const { return ssPinName_ ? 1 : 0; }

int lefiPin::hasGroundSensitivity() const { return gsPinName_ ? 1 : 0; }

const char *lefiPin::name() const { return name_; }

const char *lefiPin::taperRule() const { return taperRule_; }

const char *lefiPin::netExpr() const { return netEpxr_; }

const char *lefiPin::supplySensitivity() const { return ssPinName_; }

const char *lefiPin::groundSensitivity() const { return gsPinName_; }

int lefiPin::numPorts() const { return numPorts_; }
...
int lefiPin::numForeigns() const { return numForeigns_; }

const char *lefiPin::foreignName(int index) const { return foreign_[index]; }

int lefiPin::foreignOrient(int index) const { return foreignOrient_[index]; }

const char *lefiPin::foreignOrientStr(int index) const { return (lefiOrientStr(foreignOrient_[index])); }

double lefiPin::foreignX(int index) const { return foreignX_[index]; }

double lefiPin::foreignY(int index) const { return foreignY_[index]; }

const char *lefiPin::LEQ() const { return LEQ_; }

const char *lefiPin::direction() const { return direction_; }

const char *lefiPin::use() const { return use_; }

const char *lefiPin::shape() const { return shape_; }

const char *lefiPin::mustjoin() const { return mustjoin_; }

double lefiPin::outMarginHigh() const { return outMarginH_; }

double lefiPin::outMarginLow() const { return outMarginL_; }

double lefiPin::outResistanceHigh() const { return outResistanceH_; }

double lefiPin::outResistanceLow() const { return outResistanceL_; }

double lefiPin::inMarginHigh() const { return inMarginH_; }

double lefiPin::inMarginLow() const { return inMarginL_; }

double lefiPin::power() const { return power_; }

double lefiPin::leakage() const { return leakage_; }

double lefiPin::maxload() const { return maxload_; }

double lefiPin::maxdelay() const { return maxdelay_; }

double lefiPin::capacitance() const { return capacitance_; }

double lefiPin::resistance() const { return resistance_; }

double lefiPin::pulldownres() const { return pulldownres_; }

double lefiPin::tieoffr() const { return tieoffr_; }

double lefiPin::VHI() const { return VHI_; }

double lefiPin::VLO() const { return VLO_; }

double lefiPin::fallSlewLimit() const { return fallSlewLimit_; }

double lefiPin::riseSlewLimit() const { return riseSlewLimit_; }

double lefiPin::riseVoltage() const { return riseVoltage_; }

double lefiPin::fallVoltage() const { return fallVoltage_; }

double lefiPin::riseThresh() const { return riseThresh_; }

double lefiPin::fallThresh() const { return fallThresh_; }

double lefiPin::riseSatcur() const { return riseSatcur_; }

double lefiPin::fallSatcur() const { return fallSatcur_; }

const char *lefiPin::currentSource() const { return currentSource_; }

const char *lefiPin::tableHighName() const { return highTable_; }

const char *lefiPin::tableLowName() const { return lowTable_; }

const char *lefiPin::antennaSizeLayer(int i) const { return antennaSizeLayer_[i]; }

const char *lefiPin::antennaMetalAreaLayer(int i) const { return antennaMetalAreaLayer_[i]; }

const char *lefiPin::antennaMetalLengthLayer(int i) const { return antennaMetalLengthLayer_[i]; }

const char *lefiPin::antennaPartialMetalAreaLayer(int i) const { return antennaPartialMetalAreaLayer_[i]; }

const char *lefiPin::antennaPartialMetalSideAreaLayer(int i) const { return antennaPartialMetalSideAreaLayer_[i]; }

const char *lefiPin::antennaPartialCutAreaLayer(int i) const { return antennaPartialCutAreaLayer_[i]; }

const char *lefiPin::antennaDiffAreaLayer(int i) const { return antennaDiffAreaLayer_[i]; }

int lefiPin::numAntennaSize() const { return numAntennaSize_; }

int lefiPin::numAntennaMetalArea() const { return numAntennaMetalArea_; }

int lefiPin::numAntennaMetalLength() const { return numAntennaMetalLength_; }

int lefiPin::numAntennaPartialMetalArea() const { return numAntennaPartialMetalArea_; }

int lefiPin::numAntennaPartialMetalSideArea() const { return numAntennaPartialMetalSideArea_; }

int lefiPin::numAntennaPartialCutArea() const { return numAntennaPartialCutArea_; }

int lefiPin::numAntennaDiffArea() const { return numAntennaDiffArea_; }

double lefiPin::antennaSize(int i) const { return antennaSize_[i]; }

double lefiPin::antennaMetalArea(int i) const { return antennaMetalArea_[i]; }

double lefiPin::antennaMetalLength(int i) const { return antennaMetalLength_[i]; }

double lefiPin::antennaPartialMetalArea(int i) const { return antennaPartialMetalArea_[i]; }

double lefiPin::antennaPartialMetalSideArea(int i) const { return antennaPartialMetalSideArea_[i]; }

double lefiPin::antennaPartialCutArea(int i) const { return antennaPartialCutArea_[i]; }

double lefiPin::antennaDiffArea(int i) const { return antennaDiffArea_[i]; }
...
void lefiPin::addAntennaModel(int oxide) {
  ...
  if (numAntennaModel_ == 0) {  // does not have antennaModel
    antennaModel_          = (lefiPinAntennaModel **)lefMalloc(sizeof(lefiPinAntennaModel *) * 4);
    antennaModelAllocated_ = 4;
    for (i = 0; i < 4; i++) {
      antennaModel_[i] = (lefiPinAntennaModel *)lefMalloc(sizeof(lefiPinAntennaModel));
...
// 5.5
int lefiPin::numAntennaModel() const { return numAntennaModel_; }
...
lefiPinAntennaModel *lefiPin::antennaModel(int index) const {
  ...
  if (index == 0) {  // reset all the return flags to 0, beginning of the loop
    int i;
    for (i = 0; i < 4; i++) antennaModel_[i]->setAntennaReturnFlag(0);
  }
  while (j < 4) {
    amo = antennaModel_[j];
...
void lefiPin::addAntennaPartialMetalArea(double val, const char *layer) {
  ...
  if (layer) {  // layer can be null, since it is optional
    len                                                        = strlen(layer) + 1;
    antennaPartialMetalAreaLayer_[numAntennaPartialMetalArea_] = (char *)lefMalloc(len);
...
void lefiPin::addAntennaPartialMetalSideArea(double val, const char *layer) {
  ...
  if (layer) {  // layer can be null, since it is optional
    len                                                                = strlen(layer) + 1;
    antennaPartialMetalSideAreaLayer_[numAntennaPartialMetalSideArea_] = (char *)lefMalloc(len);
    strcpy(antennaPartialMetalSideAreaLayer_[numAntennaPartialMetalSideArea_], layer);
  } else
...
void lefiPin::addAntennaPartialCutArea(double val, const char *layer) {
  ...
  if (layer) {  // layer can be null, since it is optional
    len                                                    = strlen(layer) + 1;
    antennaPartialCutAreaLayer_[numAntennaPartialCutArea_] = (char *)lefMalloc(len);
...
int lefiPin::numProperties() const { return numProperties_; }
...
void lefiMacro::setInverter() { isInverter_ = 1; }

void lefiMacro::setBuffer() { isBuffer_ = 1; }
...
void lefiMacro::setProperty(const char *name, const char *value, const char type) {
  int len;
  if (numProperties_ == propertiesAllocated_)
    bumpProps();
  len                        = strlen(name) + 1;
...
void lefiMacro::setNumProperty(const char *name, double d, const char *value, const char type) {
  int len;
...
void lefiMacro::setFixedMask(int isFixedMask) { isFixedMask_ = isFixedMask; }

int lefiMacro::isFixedMask() const { return isFixedMask_; }

void lefiMacro::addForeign(const char *name, int hasPnt, double x, double y, int orient) {
  int     i;
  int *   hfo;
  int *   hfp;
  int *   fo;
...
void lefiMacro::setXSymmetry() { hasSymmetry_ |= 1; }

void lefiMacro::setYSymmetry() { hasSymmetry_ |= 2; }

void lefiMacro::set90Symmetry() { hasSymmetry_ |= 4; }
...
void lefiMacro::setSitePattern(lefiSitePattern *p) {
  if (numSites_ == sitesAllocated_) {
    ...
    if (sitesAllocated_ == 0) {
      ...
      np              = (lefiSitePattern **)lefMalloc(sizeof(lefiSitePattern *) * lim);
      lim /= 2;
      for (i = 0; i < lim; i++) np[i] = pattern_[i];
      lefFree((char *)(pattern_));
    }
    pattern_ = np;
  }
...
int lefiMacro::hasClass() const { return hasClass_; }

int lefiMacro::hasSiteName() const { return hasSiteName_; }

int lefiMacro::hasGenerator() const { return hasGenerator_; }

int lefiMacro::hasGenerate() const { return hasGenerate_; }

int lefiMacro::hasPower() const { return hasPower_; }

int lefiMacro::hasOrigin() const { return hasOrigin_; }

int lefiMacro::hasSource() const { return hasSource_; }

int lefiMacro::hasEEQ() const { return hasEEQ_; }

int lefiMacro::hasLEQ() const { return hasLEQ_; }

int lefiMacro::hasXSymmetry() const { return (hasSymmetry_ & 1) ? 1 : 0; }

int lefiMacro::hasYSymmetry() const { return (hasSymmetry_ & 2) ? 1 : 0; }

int lefiMacro::has90Symmetry() const { return (hasSymmetry_ & 4) ? 1 : 0; }

int lefiMacro::hasSitePattern() const { return (pattern_) ? 1 : 0; }

int lefiMacro::hasSize() const { return hasSize_; }

int lefiMacro::hasForeign() const { return (numForeigns_) ? 1 : 0; }

int lefiMacro::hasForeignOrigin(int index) const { return hasForeignOrigin_[index]; }

int lefiMacro::hasForeignOrient(int index) const { return (foreignOrient_[index] == -1) ? 0 : 1; }

int lefiMacro::hasForeignPoint(int index) const { return hasForeignPoint_[index]; }

int lefiMacro::hasClockType() const { return hasClockType_; }

int lefiMacro::numSitePattern() const { return numSites_; }

int lefiMacro::numProperties() const { return numProperties_; }
...
const char *lefiMacro::name() const { return name_; }

const char *lefiMacro::macroClass() const { return macroClass_; }

const char *lefiMacro::generator() const { return generator_; }

const char *lefiMacro::EEQ() const { return EEQ_; }

const char *lefiMacro::LEQ() const { return LEQ_; }

const char *lefiMacro::source() const { return source_; }

double lefiMacro::originX() const { return originX_; }

double lefiMacro::originY() const { return originY_; }

double lefiMacro::power() const { return power_; }
...
lefiSitePattern *lefiMacro::sitePattern(int index) const { return pattern_[index]; }

const char *lefiMacro::siteName() const { return siteName_; }

double lefiMacro::sizeX() const { return sizeX_; }

double lefiMacro::sizeY() const { return sizeY_; }

int lefiMacro::numForeigns() const { return numForeigns_; }

int lefiMacro::foreignOrient(int index) const { return foreignOrient_[index]; }

const char *lefiMacro::foreignOrientStr(int index) const { return (lefiOrientStr(foreignOrient_[index])); }

double lefiMacro::foreignX(int index) const { return foreignX_[index]; }

double lefiMacro::foreignY(int index) const { return foreignY_[index]; }

const char *lefiMacro::foreignName(int index) const { return foreign_[index]; }

const char *lefiMacro::clockType() const { return clockType_; }

int lefiMacro::isBuffer() const { return isBuffer_; }

int lefiMacro::isInverter() const { return isInverter_; }
...
void lefiTiming::addTableAxisNumber(double one) {
  if (numOfAxisNumbers_ == axisNumbersAllocated_) {
    ...
    for (i = 0; i < lim; i++) n[i] = axisNumbers_[i];
...
void lefiTiming::addRiseFallSlew(double one, double two, double three, double four) {
  if (nowRise_) {
    hasRiseSlew_   = 1;
    riseSlewOne_   = one;
    riseSlewTwo_   = two;
...
void lefiTiming::addDelay(const char *risefall, const char *unateness, double one, double two, double three) {
  hasDelay_        = 1;
...
void lefiTiming::addTransition(const char *risefall, const char *unateness, double one, double two, double three) {
  hasTransition_        = 1;
...
void lefiTiming::addSDF1Pin(const char *trigType, double one, double two, double three) {
...
void lefiTiming::addSDF2Pins(const char *trigType, const char *fromTrig, const char *toTrig, double one, double two, double three) {
...
void lefiTiming::setSDFcondStart(const char *condStart) { SDFcondStart_ = (char *)condStart; }

void lefiTiming::setSDFcondEnd(const char *condEnd) { SDFcondEnd_ = (char *)condEnd; }

void lefiTiming::setSDFcond(const char *cond) { SDFcond_ = (char *)cond; }

int lefiTiming::hasData() { return ((numFrom_) ? 1 : 0); }
...
int lefiTiming::numFromPins() { return numFrom_; }

const char *lefiTiming::fromPin(int index) { return from_[index]; }

int lefiTiming::numToPins() { return numTo_; }

const char *lefiTiming::toPin(int index) { return to_[index]; }

int lefiTiming::hasTransition() { return hasTransition_; }

int lefiTiming::hasDelay() { return hasDelay_; }

int lefiTiming::hasRiseSlew() { return hasRiseSlew_; }

int lefiTiming::hasRiseSlew2() { return hasRiseSlew2_; }

int lefiTiming::hasFallSlew() { return hasFallSlew_; }

int lefiTiming::hasFallSlew2() { return hasFallSlew2_; }

int lefiTiming::hasRiseIntrinsic() { return hasRiseIntrinsic_; }

int lefiTiming::hasFallIntrinsic() { return hasFallIntrinsic_; }

int lefiTiming::hasSDFonePinTrigger() { return hasSDFonePinTrigger_; }

int lefiTiming::hasSDFtwoPinTrigger() { return hasSDFtwoPinTrigger_; }

int lefiTiming::hasSDFcondStart() { return hasSDFcondStart_; }

int lefiTiming::hasSDFcondEnd() { return hasSDFcondEnd_; }

int lefiTiming::hasSDFcond() { return hasSDFcond_; }

int lefiTiming::numOfAxisNumbers() { return numOfAxisNumbers_; }

double *lefiTiming::axisNumbers() { return axisNumbers_; }

int lefiTiming::numOfTableEntries() { return numOfTableEntries_; }
...
const char *lefiTiming::delayRiseOrFall() { return delayRiseOrFall_; }

const char *lefiTiming::delayUnateness() { return delayUnateness_; }

double lefiTiming::delayTableOne() { return delayTableOne_; }

double lefiTiming::delayTableTwo() { return delayTableTwo_; }

double lefiTiming::delayTableThree() { return delayTableThree_; }

const char *lefiTiming::transitionRiseOrFall() { return transitionRiseOrFall_; }

const char *lefiTiming::transitionUnateness() { return transitionUnateness_; }

double lefiTiming::transitionTableOne() { return transitionTableOne_; }

double lefiTiming::transitionTableTwo() { return transitionTableTwo_; }

double lefiTiming::transitionTableThree() { return transitionTableThree_; }

double lefiTiming::riseIntrinsicOne() { return riseIntrinsicOne_; }

double lefiTiming::riseIntrinsicTwo() { return riseIntrinsicTwo_; }

double lefiTiming::riseIntrinsicThree() { return riseIntrinsicThree_; }

double lefiTiming::riseIntrinsicFour() { return riseIntrinsicFour_; }

double lefiTiming::fallIntrinsicOne() { return fallIntrinsicOne_; }

double lefiTiming::fallIntrinsicTwo() { return fallIntrinsicTwo_; }

double lefiTiming::fallIntrinsicThree() { return fallIntrinsicThree_; }

double lefiTiming::fallIntrinsicFour() { return fallIntrinsicFour_; }

double lefiTiming::riseSlewOne() { return riseSlewOne_; }

double lefiTiming::riseSlewTwo() { return riseSlewTwo_; }

double lefiTiming::riseSlewThree() { return riseSlewThree_; }

double lefiTiming::riseSlewFour() { return riseSlewFour_; }

double lefiTiming::riseSlewFive() { return riseSlewFive_; }

double lefiTiming::riseSlewSix() { return riseSlewSix_; }

double lefiTiming::riseSlewSeven() { return riseSlewSeven_; }

double lefiTiming::fallSlewOne() { return fallSlewOne_; }

double lefiTiming::fallSlewTwo() { return fallSlewTwo_; }

double lefiTiming::fallSlewThree() { return fallSlewThree_; }

double lefiTiming::fallSlewFour() { return fallSlewFour_; }

double lefiTiming::fallSlewFive() { return fallSlewFive_; }

double lefiTiming::fallSlewSix() { return fallSlewSix_; }

double lefiTiming::fallSlewSeven() { return fallSlewSeven_; }

int lefiTiming::hasRiseRS() { return hasRiseRS_; }

double lefiTiming::riseRSOne() { return riseRSOne_; }

double lefiTiming::riseRSTwo() { return riseRSTwo_; }

int lefiTiming::hasRiseCS() { return hasRiseCS_; }

double lefiTiming::riseCSOne() { return riseCSOne_; }

double lefiTiming::riseCSTwo() { return riseCSTwo_; }

int lefiTiming::hasFallRS() { return hasFallRS_; }

double lefiTiming::fallRSOne() { return fallRSOne_; }

double lefiTiming::fallRSTwo() { return fallRSTwo_; }

int lefiTiming::hasFallCS() { return hasFallCS_; }

double lefiTiming::fallCSOne() { return fallCSOne_; }

double lefiTiming::fallCSTwo() { return fallCSTwo_; }

int lefiTiming::hasUnateness() { return hasUnateness_; }

const char *lefiTiming::unateness() { return unateness_; }

int lefiTiming::hasFallAtt1() { return hasFallAtt1_; }

double lefiTiming::fallAtt1One() { return fallAtt1One_; }

double lefiTiming::fallAtt1Two() { return fallAtt1Two_; }

int lefiTiming::hasRiseAtt1() { return hasRiseAtt1_; }

double lefiTiming::riseAtt1One() { return riseAtt1One_; }

double lefiTiming::riseAtt1Two() { return riseAtt1Two_; }

int lefiTiming::hasFallTo() { return hasFallTo_; }

double lefiTiming::fallToOne() { return fallToOne_; }

double lefiTiming::fallToTwo() { return fallToTwo_; }

int lefiTiming::hasRiseTo() { return hasRiseTo_; }

double lefiTiming::riseToOne() { return riseToOne_; }

double lefiTiming::riseToTwo() { return riseToTwo_; }

int lefiTiming::hasStableTiming() { return hasStableTiming_; }

double lefiTiming::stableSetup() { return stableSetup_; }

double lefiTiming::stableHold() { return stableHold_; }

const char *lefiTiming::stableRiseFall() { return stableRiseFall_; }

const char *lefiTiming::SDFonePinTriggerType() { return SDFtriggerType_; }

const char *lefiTiming::SDFtwoPinTriggerType() { return SDFtriggerType_; }

const char *lefiTiming::SDFfromTrigger() { return SDFfromTrigger_; }

const char *lefiTiming::SDFtoTrigger() { return SDFtoTrigger_; }

double lefiTiming::SDFtriggerOne() { return SDFtriggerTableOne_; }

double lefiTiming::SDFtriggerTwo() { return SDFtriggerTableTwo_; }

double lefiTiming::SDFtriggerThree() { return SDFtriggerTableThree_; }

const char *lefiTiming::SDFcondStart() { return SDFcondStart_; }

const char *lefiTiming::SDFcondEnd() { return SDFcondEnd_; }

const char *lefiTiming::SDFcond() { return SDFcond_; }

lefiMacroSite::lefiMacroSite(const char *name, const lefiSitePattern *pattern) : siteName_(name), sitePattern_(pattern) {}

const char *lefiMacroSite::siteName() const { return siteName_; }

const lefiSitePattern *lefiMacroSite::sitePattern() const { return sitePattern_; }

lefiMacroForeign::lefiMacroForeign(const char *name, int hasPts, double x, double y, int hasOrient, int orient)
    : cellName_(name), cellHasPts_(hasPts), py_(y), px_(x), cellHasOrient_(hasOrient), cellOrient_(orient) {}

const char *lefiMacroForeign::cellName() const { return cellName_; }

int lefiMacroForeign::cellHasPts() const { return cellHasPts_; }

double lefiMacroForeign::px() const { return px_; }

double lefiMacroForeign::py() const { return py_; }

int lefiMacroForeign::cellHasOrient() const { return cellHasOrient_; }

int lefiMacroForeign::cellOrient() const { return cellOrient_; }

...
int lefrInit() { return lefrInitSession(0); }

int lefrInitSession(int startSession) {
  if (startSession) {
    if (init_call_func != NULL) {
      fprintf(stderr,
              "ERROR: Attempt to call configuration function '%s' in LEF parser before lefrInit() call in session-based mode.\n",
              init_call_func);
      return 1;
    }
...
int lefrRead(FILE *f, const char *fName, lefiUserData uData) {
  ...
  lefData->versionNum = (lefSettings->VersionNum == 0.0) ? CURRENT_VERSION : lefData->versionNum = lefSettings->VersionNum;
...
void lefrPrintUnusedCallbacks(FILE *f) {
  ...
  for (i = 0; i < NOCBK; i++) {
    if (lefrUnusedCount[i]) {
      ...
      if (firstCB && (lefrCallbackType_e)i != lefrMacroClassTypeCbkType && (lefrCallbackType_e)i != lefrMacroOriginCbkType
          && (lefrCallbackType_e)i != lefrMacroSiteCbkType && (lefrCallbackType_e)i != lefrMacroForeignCbkType
          && (lefrCallbackType_e)i != lefrMacroSizeCbkType && (lefrCallbackType_e)i != lefrMacroFixedMaskCbkType
...
void lefrUnsetCallbacks() { lefrCallbacks::reset(); }

...
int lefwInit(FILE *f) {
  ...
  */
  ...
  if (lefwHasInitCbk == 1) {  // lefwInitCbk has already called, issue an error
    fprintf(lefwFile, "ERROR (LEFWRIT-4000): lefwInitCbk has already called, cannot call lefwInit again.\n");
  ...
  for (i = 0; i < MAXSYN; i++) lefwSynArray[i] = 0;
...
int lefwStartLayer(const char *layerName, const char *type) {
  ...
  if (strcmp(type, "CUT") == 0)
    ...
    lefwIsImplant = 1;
  else if (strcmp(type, "MASTERSLICE") && strcmp(type, "OVERLAP") && strcmp(type, "IMPLANT"))  // IMPLANT is 5.5 syntax
...
int lefwLayerMask(int colorMask) {
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYER_START && lefwState != LEFW_LAYER && lefwState != LEFW_LAYERROUTING_START)
    return LEFW_BAD_ORDER;
...
int lefwLayerCutSpacingTableOrtho(int numSpacing, double *cutWithins, double *orthoSpacings) {
  ...
  if (numSpacing) {
    if (lefwWriteEncrypt) {
      ...
      lefwLines++;
      for (i = 0; i < numSpacing; i++) {
        fprintf(lefwFile, "\n     WITHIN %.11g SPACING %.11g", *cutWithins++, *orthoSpacings++);
        lefwLines++;
      }
      fprintf(lefwFile, " ;\n");
    }
...
int lefwLayerArraySpacing(int longArray, double viaWidth, double cutSpacing, int numArrayCut, int *arrayCuts,
  ...
  if (cutSpacing) {
    if (lefwWriteEncrypt) {
      ...
      for (i = 0; i < numArrayCut; i++) {
        fprintf(lefwFile, "ARRAYCUTS %d SPACING %.11g ", *arrayCuts++, *arraySpacings++);
      }
      fprintf(lefwFile, " ;\n");
    }
    lefwLines++;
...
int lefwLayerPreferEnclosure(const char *location, double overhang1, double overhang2,
  ...
  if (lefwWriteEncrypt) {
    ...
    if (location && (*location != 0)) {
      if (strcmp(location, "ABOVE") && strcmp(location, "BELOW")) {
        fprintf(lefwFile, "PREFERENCLOSURE in Layer(Cut) can only be ABOVE, BELOW.\n");
        return LEFW_BAD_DATA;
      }
      fprintf(lefwFile, "%s ", location);
    }
...
int lefwLayerRouting(const char *direction, double width) {
  ...
  if (lefwWriteEncrypt) {
    ...
    if (strcmp(direction, "HORIZONTAL") && strcmp(direction, "VERTICAL") && strcmp(direction, "DIAG45")
        && strcmp(direction, "DIAG135")) {
      fprintf(lefwFile, "DIRECTION in Layer(Routing) can only be HORIZONTAL, VERTICAL, DIAG45, DIAG135.\n");
      return LEFW_BAD_DATA;  // direction is either HORIZONTAL, VERTICAL, DIAG45, DIAG135 only
    }
...
int lefwLayerRoutingMinsize(int numRect, double *minWidth, double *minLength) {
  ...
  if (lefwWriteEncrypt) {
    ...
    for (i = 0; i < numRect; i++) encPrint(lefwFile, (char *)"%.11g %.11g ", minWidth[i], minLength[i]);
    encPrint(lefwFile, (char *)";\n");
  } else {
    fprintf(lefwFile, "   MINSIZE ");
    for (i = 0; i < numRect; i++) fprintf(lefwFile, "%.11g %.11g ", minWidth[i], minLength[i]);
    fprintf(lefwFile, ";\n");
  }
  lefwLines++;
  return LEFW_OK;
...
int lefwLayerRoutingSpacingRangeUseLengthThreshold() {
  ...
  if (!lefwHasLayerRoutingRange) {
    ...
    fprintf(lefwFile, "ERROR (LEFWRIT-4003): lefwLayerRoutingSpacingUseLengthThreshold cannot be called if\n");
...
int lefwLayerRoutingSpacingRangeInfluence(double infValue, double subMinWidth, double subMaxWidth) {
  ...
  if (!lefwHasLayerRoutingRange) {
    // This routine can be called only if range are non zero
    fprintf(lefwFile, "ERROR (LEFWRIT-4004): lefwLayerRoutingSpacingInfluence cannot be called if\n");
...
int lefwLayerRoutingSpacingRangeRange(double minWidth, double maxWidth) {
  ...
  if (!lefwHasLayerRoutingRange) {
    // This routine can be called only if range are non zero
    fprintf(lefwFile, "ERROR (LEFWRIT-4005): lefwLayerRoutingSpacingInfluence cannot be called if\n");
    fprintf(lefwFile, "       lefRange and rightRange in lefwLayerRoutingSpacing are both zero.\n");
    return LEFW_BAD_DATA;
  }
  if (lefwWriteEncrypt) {
...
int lefwLayerRoutingStartSpacingtableParallel(int numLength, double *length) {
  ...
  if (lefwWriteEncrypt) {
    ...
    for (i = 0; i < numLength; i++) encPrint(lefwFile, (char *)" %.11g", length[i]);
  } else {
    fprintf(lefwFile, "   SPACINGTABLE\n");
    fprintf(lefwFile, "     PARALLELRUNLENGTH");
    for (i = 0; i < numLength; i++) fprintf(lefwFile, " %.11g", length[i]);
  }
...
int lefwLayerRoutingSpacingtableParallelWidth(double width, int numSpacing, double *spacing) {
  ...
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_SP_START && lefwState != LEFW_LAYERROUTINGWIDTH)
    return LEFW_BAD_ORDER;
  if (lefwWriteEncrypt) {
    encPrint(lefwFile, (char *)"\n       WIDTH %.11g", width);
    for (i = 0; i < numSpacing; i++) encPrint(lefwFile, (char *)" %.11g", spacing[i]);
  } else {
    fprintf(lefwFile, "\n       WIDTH %.11g", width);
    for (i = 0; i < numSpacing; i++) fprintf(lefwFile, " %.11g", spacing[i]);
  }
...
int lefwLayerACCurrentDensity(const char *type, double value) {
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START && lefwState != LEFW_LAYERROUTING && lefwState != LEFW_LAYER_START
      && lefwState != LEFW_LAYER)
    return LEFW_BAD_ORDER;
...
int lefwLayerDCWidth(int numWidths, double *widths) {
  ...
  if (lefwOldState != LEFW_LAYER_START && lefwOldState != LEFW_LAYER && lefwOldState != LEFW_LAYERROUTING_START
...
int lefwLayerAntennaDiffAreaRatioPwl(int numPwls, double *diffusions, double *ratios) {
  ...
  if (lefwWriteEncrypt) {
    ...
    for (i = 0; i < numPwls; i++) fprintf(lefwFile, "( %.11g %.11g ) ", *diffusions++, *ratios++);
    fprintf(lefwFile, ") ;\n");
  }
  lefwLines++;
  return LEFW_OK;
...
int lefwLayerAntennaAreaFactor(double value, const char *diffUseOnly) {
  ...
  if (!lefwIsRouting && !lefwIsCut)
    return LEFW_BAD_DATA;
  if ((versionNum < 5.4) && (diffUseOnly && strcmp(diffUseOnly, "DIFFUSEONLY") == 0))
    return LEFW_WRONG_VERSION;
  if (lefwWriteEncrypt) {
...
int lefwLayerAntennaAreaDiffReducePwl(int numPwls, double *diffAreas, double *metalDiffFactors) {
  ...
  */
  ...
  if (lefwWriteEncrypt) {
    ...
    for (i = 0; i < numPwls; i++) encPrint(lefwFile, (char *)"( %.11g %.11g ) ", *diffAreas++, *metalDiffFactors++);
    encPrint(lefwFile, (char *)") ;\n");
  } else {
    fprintf(lefwFile, "   ANTENNAAREADIFFREDUCEPWL ( ");
    for (i = 0; i < numPwls; i++) fprintf(lefwFile, "( %.11g %.11g ) ", *diffAreas++, *metalDiffFactors++);
    fprintf(lefwFile, ") ;\n");
  }
  lefwLines++;
  return LEFW_OK;
...
int lefwIntPropDef(const char *objType, const char *propName, double leftRange,
  ...
  */
  ...
  if (strcmp(objType, "LIBRARY") && strcmp(objType, "VIA") && strcmp(objType, "MACRO") && strcmp(objType, "PIN")
      && strcmp(objType, "LAYER") && strcmp(objType, "NONDEFAULTRULE") && strcmp(objType, "VIARULE"))
    return LEFW_BAD_DATA;
...
int lefwStartVia(const char *viaName,
  ...
  if (lefwState == LEFW_VIA_START || lefwState == LEFW_VIA)
    return LEFW_BAD_ORDER;
  if (lefwState != LEFW_INIT && lefwState < LEFW_END && (!lefwIsNonDefaultRule))  // via defined in nondefaultrule
    return LEFW_BAD_ORDER;                                                        // not of the ENDs
...
int lefwStringProperty(const char *propName, const char *propValue) {
  if ((lefwState != LEFW_VIA) && (lefwState != LEFW_LAYER) && (lefwState != LEFW_VIARULE) && (lefwState != LEFW_MACRO_START)
      ...
      && (lefwState != LEFW_LAYERROUTING_START) && (lefwState != LEFW_LAYERROUTING))
    return LEFW_BAD_ORDER;
...
int lefwEndVia(const char *viaName) {
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_VIA_START && lefwState != LEFW_VIA && lefwState != LEFW_VIAVIARULE)
    return LEFW_BAD_ORDER;
  if (lefwWriteEncrypt)
    encPrint(lefwFile, (char *)"END %s\n\n", viaName);
  else
...
int lefwViaRulePrtLayer(const char *layerName, const char *direction, double minWidth, double maxWidth, double overhang,
  ...
  if (lefwWriteEncrypt)
    ...
    fprintf(lefwFile, "   LAYER %s ;\n", layerName);
  if (direction && strcmp(direction, "HORIZONTAL") && strcmp(direction, "VERTICAL"))
    return LEFW_BAD_DATA;
...
int lefwViaRuleLayer(const char *layerName, const char *direction, double minWidth,
  ...
  if (lefwNumViaRuleLayers >= 2)
    return LEFW_BAD_ORDER;
  if ((status = lefwViaRulePrtLayer(layerName, direction, minWidth, maxWidth, overhang, metalOverhang)) != LEFW_OK)
    return status;
...
int lefwNonDefaultRuleStartVia(const char *viaName, const char *isDefault) {
  ...
  if (!lefwDidInit)
    return LEFW_BAD_ORDER;
  if (lefwState == LEFW_VIA_START || lefwState == LEFW_VIA
      || (lefwState != LEFW_NONDEFAULTRULE_START && lefwState != LEFW_NONDEFAULTRULE))
    return LEFW_BAD_ORDER;
...
int lefwEndNonDefaultRule(const char *ruleName) {
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_NONDEFAULTRULE_START && lefwState != LEFW_NONDEFAULTRULE && lefwState != LEFW_VIA_END
...
int lefwStartSpacing() {
  ...
  if (lefwState == LEFW_SPACING_START || lefwState == LEFW_SPACING)
    return LEFW_BAD_ORDER;
  if (lefwState != LEFW_INIT && lefwState < LEFW_END && (!lefwIsNonDefaultRule))  // spacing defined in nondefaultrule
    return LEFW_BAD_ORDER;                                                        // not of the ENDs
  if (lefwWriteEncrypt)
...
int lefwMacroClass(const char *value1, const char *value2) {
  ...
  if (lefwWriteEncrypt) {
    if (strcmp(value1, "RING") == 0)
      ...
      if (value2) {
        if (strcmp(value2, "INPUT") && strcmp(value2, "OUTPUT") && strcmp(value2, "INOUT") && strcmp(value2, "POWER")
      ...
      if (value2) {
        if (strcmp(value2, "FEEDTHRU") && strcmp(value2, "TIEHIGH") && strcmp(value2, "TIELOW") && strcmp(value2, "SPACER")
      ...
      if (value2) {
        if (strcmp(value2, "PRE") && strcmp(value2, "POST") && strcmp(value2, "TOPLEFT") && strcmp(value2, "TOPRIGHT")
...
int lefwMacroSitePattern(const char *name, double origX, double origY, int orient, int numX, int numY, double spaceX,
  ...
  if (lefwWriteEncrypt) {
    ...
    if (origX || origY || (orient != 1)) {
      ...
      if (numX || numY || spaceX || spaceY)
        fprintf(lefwFile, "DO %d BY %d STEP %.11g %.11g ", numX, numY, spaceX, spaceY);
    }
    fprintf(lefwFile, ";\n");
  }
...
int lefwStartMacroPinPort(const char *classType) {  // optional
  ...
  if (lefwWriteEncrypt) {
    ...
    if (classType) {
      if (strcmp(classType, "NONE") && strcmp(classType, "CORE") && strcmp(classType, "BUMP"))
        return LEFW_BAD_DATA;
...
int lefwMacroPinPortLayerRect(double xl1, double yl1, double xl2, double yl2, int numX,
                              int    numY,  // optional
                              double spaceX,
                              double spaceY,  // optional
                              int    mask) {     // optional
  ...
  if (lefwWriteEncrypt) {
    ...
    if (numX || numY || spaceX || spaceY) {
      if (mask) {
        fprintf(lefwFile, "            RECT MASK %d ITERATE %.11g %.11g %.11g %.11g ", mask, xl1, yl1, xl2, yl2);
      } else {
        fprintf(lefwFile, "            RECT ITERATE %.11g %.11g %.11g %.11g ", xl1, yl1, xl2, yl2);
      }
      fprintf(lefwFile, "DO %d BY %d STEP %.11g %.11g ", numX, numY, spaceX, spaceY);
    } else {
      if (mask) {
        fprintf(lefwFile, "            RECT MASK %d %.11g %.11g %.11g %.11g ", mask, xl1, yl1, xl2, yl2);
      } else {
        fprintf(lefwFile, "            RECT %.11g %.11g %.11g %.11g ", xl1, yl1, xl2, yl2);
      }
    }
    fprintf(lefwFile, ";\n");
  }
...
int lefwMacroObsLayerRect(double xl1, double yl1, double xl2, double yl2, int numX,
                          int    numY,  // optional
                          double spaceX,
                          double spaceY,  // optional
                          int    mask) {     // optional
...
int lefwMacroTimingIntrinsic(const char *riseFall, double min, double max, double slewT1,
  ...
  if (lefwWriteEncrypt) {
    ...
    if (slewT1 || slewT1Min || slewT1Max || slewT2) {
      fprintf(lefwFile, "%.11g %.11g %.11g %.11g ", slewT1, slewT1Min, slewT1Max, slewT2);
      if (slewT2Min || slewT2Max || slewT3)
...
int lefwMacroTimingUnateness(const char *unateness) {
  ...
  if (!lefwIsMacroTiming)
    return LEFW_BAD_ORDER;
  if (strcmp(unateness, "INVERT") && strcmp(unateness, "NONINVERT") && strcmp(unateness, "NONUNATE"))
    return LEFW_BAD_DATA;
  if (lefwWriteEncrypt)
...
int lefwAntenna(const char *type, double value) {
  ...
  if (!type || type == 0 || *type == 0)
    return LEFW_BAD_DATA;
  if (strcmp(type, "INPUTPINANTENNASIZE") && strcmp(type, "OUTPUTPINANTENNASIZE") && strcmp(type, "INOUTPINANTENNASIZE"))
    return LEFW_BAD_DATA;
  if (lefwWriteEncrypt)
...
int lefwCurrentLineNumber() { return lefwLines; }

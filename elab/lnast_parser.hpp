...
protected:
  ...
  void       process_assign_like_op            (const Tree_index& tree_idx_opr);
  void       process_label_op                  (const Tree_index& tree_idx_opr);
  void       process_binary_op                 (const Tree_index& tree_idx_opr);
  void       process_func_call_op              (const Tree_index& tree_idx_opr);
  void       process_func_def_op               (const Tree_index& tree_idx_opr);
  void       process_if_op                     (const Tree_index& tree_idx_opr);
  Tree_index process_operator_node             (const Tree_index& opr_parent_sts, Token tk, Lnast_ntype_id type, uint32_t knum1, uint32_t knum2);
  void       add_operator_subtree              (const Tree_index& tree_idx_opr);

...
#
# Contract for `formal.partitions` / `formal.split` — parallel input-space case-split.
...
run() { OUT=$("$LHD" lec "${@}" --top m --set formal.lec.semdiff=none 2>&1); RC=$?; }
...
# 3) formal.partitions=1 DISABLES the case-split -> single monolithic ind query.
run --ref "$WORK/good.sv" --impl "$WORK/good.sv" --set formal.partitions=1
...
# 4) explicit formal.split=sel forces the same signal.
run --ref "$WORK/good.sv" --impl "$WORK/good.sv" --set formal.split=sel

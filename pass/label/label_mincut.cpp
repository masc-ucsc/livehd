 ...
 * * * * * * * * */
Label_mincut::Label_mincut(bool _v, bool _h, int _i, int _s, std::string_view _a) : verbose(_v), hier(_h), iters(_i), seed(_s), alg(_a) { 
  node_id = 0; 
  num_nodes = 0; 
  num_edges = 0;
...
void Label_mincut::viecut_cut(std::string inp_metis_path, std::string out_path) {
  ...
  if (cfg->find_lowest_conductance) cfg->find_most_balanced_cut = true;
  ...
  if (numthreads.empty()) numthreads.emplace_back(1); 

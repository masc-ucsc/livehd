...
uint64_t hfile(const std::string& path) {
  ...
  if (!ifs.is_open()) {
    return fnv1a64("\x01missing");
  }
  uint64_t          h = hash_util::kFnv1a64_offset;
  std::vector<char> buf(1U << 16U);
  while (ifs.read(buf.data(), static_cast<std::streamsize>(buf.size())) || ifs.gcount() != 0) {
    h = fnv1a64(std::string_view{buf.data(), static_cast<size_t>(ifs.gcount())}, h);
  }
  return h;
}

std::string jesc(std::string_view text) { return json_util::escape(text); }
...
uint64_t Sta_cache::env_hash(const std::vector<std::string>& files, std::string_view top, std::string_view hier, int margin,
  ...
  uint64_t h = fnv1a64("sta-env-v1");
  for (const auto& f : files) {
    h = combine64(h, combine64(fnv1a64(fs::path(f).filename().string()), hfile(f)));
  }
  h = combine64(h, fnv1a64(top));
  h = combine64(h, fnv1a64(hier));
  h = combine64(h, static_cast<uint64_t>(margin));
  ...
  return combine64(h, stats ? 1U : 0U);
}

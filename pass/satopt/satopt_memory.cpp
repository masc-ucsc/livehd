...
struct Port {
  ...
  int              block = -1;  // the memory's pin block (shared profile: edits in place)
};
struct Build {
  hhds::Graph& g;
  Node         original;
...
// Each answer stands alone, so a run the budget cut short keeps (and caches)
// the answers it got; a query it could not afford is answered `not proven`
// and never cached. `options` is the proof-relevant budget (Budget::proof_key).
// A reused answer charges what it cost, in the cold run's order, so warm and
// cold runs leave the same budget to what follows.
constexpr int kMemoryCacheVersion = 3;
...
struct Memory_queries {
  ...
  std::map<std::string, Memory_answer> answers;
  Meter*                               meter = nullptr;
  uint64_t                             asked = 0, proven = 0, reused = 0, budget_skips = 0;
  template <class Query>
  bool ask(const std::string& key, const formal::Prover& prover, const Query& query) {
    if (auto it = answers.find(key); it != answers.end()) {
      // The cold run's charges, in its order: the query gate, then the work
      // after the fact (which may exhaust the stage just as it did then).
      if (meter && !meter->query()) {
        ++budget_skips;
        return false;
      }
      if (meter) {
        meter->work(it->second.work);
      }
      ++reused;
    ...
    const auto work   = prover.work() - before;
    if (meter) {
      meter->work(work);
    }
    proven += result;
    answers.emplace(key, Memory_answer{result, work});
    return result;
  }
};
std::map<std::string, Memory_queries> query_cache;
std::string                           queries_path(std::string_view dir, std::string_view name, bool shared) {
  if (dir.empty()) {
    return {};
  }
  ...
  return std::format("{}/{:016x}-memory{}.json", dir, hash, shared ? "-shared" : "");
}
void read_queries(const std::string& path, Memory_queries& queries, const std::string& source, const std::string& options) {
  queries = {source, options, {}};
  ...
  for (const auto& member : doc["answers"].GetObject()) {
    ...
    if (v.IsArray() && v.Size() == 2 && v[0].IsBool() && v[1].IsUint64()) {
      queries.answers[member.name.GetString()] = {v[0].GetBool(), v[1].GetUint64()};
    }
  }
}
void write_queries(const std::string& path, const Memory_queries& queries) {
  ...
  out << std::format("{{\"version\":{},\"source\":\"{}\",\"options\":\"{}\",\"answers\":{{",
                     kMemoryCacheVersion,
                     json_util::escape(queries.source),
                     json_util::escape(queries.options));
  bool comma = false;
  for (const auto& [key, answer] : queries.answers) {
    out << std::format("{}\"{}\":[{},{}]", comma ? "," : "", json_util::escape(key), answer.value ? "true" : "false", answer.work);
    comma = true;
  }
  out << "}}\n";
  out.close();
...
struct Proofs {
  ...
  std::string    prefix;  // shared-profile answers (unstamped widths rejected) keep their own keys
  explicit Proofs(hhds::Graph* graph, int depth, Memory_queries& queries, bool shared = false)
      : cache(queries)
      , address_bits(std::max(1, static_cast<int>(std::bit_width(static_cast<unsigned>(depth - 1)))))
      , seeds(queries.meter ? queries.meter->budget().samples : Budget{}.samples)
      , prover(graph, detail::prove_options(queries.meter ? queries.meter->budget() : Budget{}, false, shared))
  ...
  bool dead(Pin en) {
    ...
    return cache.ask(prefix + "dead:" + identity(en), prover, [&] { return prover.is_false(en).verdict == Verdict::Proven; });
  }
  ...
  bool relation(const Port& a, const Port& b, bool equal) {
    ...
    if (aa && ba && ae && be) {
      for (size_t i = 0; i < std::min({aa->size(), ba->size(), ae->size(), be->size()}); ++i) {
...
void optimize(hhds::Graph& g, Node mem, Memory_satopt& stats, Memory_queries& queries, Profile profile) {
  ...
  for (auto& [block, fields] : blocks) {
    ...
    Port p{fields[addr], fields[din], fields[enable], fields[clock], {static_cast<int>((read ? reads : writes).size())}, block};
    if (p.clk.is_invalid()) {
      p.clk = shared_clock;
    }
    if (p.en.is_invalid()) {
  ...
  if (profile == Profile::shared) {
    // Only exact, in-place edits: a rebuilt Memory changes its identity and
    // LEC's occurrence pairing, dead-read and read-merge rewrites refine a
    // disabled read (sim still reads it), and collision clearing can leave a
    // non-prefix ordering row sim cannot model. A write enable proven never
    // active is tied to 0 on the port itself.
    Proofs proof(&g, depth, queries, true);
    for (const auto& p : writes) {
      if (p.en.is_const() || !proof.dead(p.en)) {
        continue;
      }
      const auto sink = mem.create_sink_pin(static_cast<hhds::Port_id>(p.block * stride + enable));
      gu::drop_drivers(sink);
      B.konst(0).connect_sink(sink);
      ++stats.dead_ports;
    }
    return;
  }
...
Memory_satopt optimize_memories(const std::vector<std::shared_ptr<hhds::Graph>>& graphs, std::string_view cache_dir, Profile profile,
  ...
  for (const auto& graph : graphs) {
    ...
    const auto path    = queries_path(cache_dir, graph->get_name(), shared);
    auto&      queries = query_cache[std::string(graph->get_name()) + (shared ? "\x01shared" : "")];
    if (queries.source != source || queries.options != options) {
      read_queries(path, queries, source, options);
    }
    queries.meter        = &m;
    queries.asked        = 0;
    queries.proven       = 0;
    queries.reused       = 0;
    queries.budget_skips = 0;
    for (const auto& node : memories) {
      optimize(*graph, node, result, queries, profile);
    }
    queries.meter        = nullptr;
    result.queries      += queries.asked;
    result.proven       += queries.proven;
    result.reused       += queries.reused;
    result.budget_skips += queries.budget_skips;

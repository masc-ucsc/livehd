...
public:
  ...
  std::vector<int> sat_operator_with_Pick_Op(int node_B_ID, ezMiniSAT &sat, int operand_type, LGraph *g, int frombits,
                                             int pick_bits, std::vector<int> vec_pic);
...
void Destination::increment_and_add_edges_with_operation(int node_B_ID, std::vector<int> input_1, ezMiniSAT &sat, int operand_type,
                                                         LGraph *g) {
...
std::vector<int> Destination::sat_operator_with_Pick_Op(int node_B_ID, ezMiniSAT &sat, int operand_type, LGraph *g, int frombits,
                                                        int pick_bits, std::vector<int> vec_pick) {
       ...
       if((int)c.get_inp_pin().get_pid()==0) {
          std::cout<<" \nPiD is:"<<(int)c.get_inp_pin().get_pid();
           input_vector = operands_node[(int)c.get_inp_pin().get_pid()] ;
                     std::cout << "INPUT 1 is for nodeID " << node_B_ID << " is : " << sat.to_string(input_vector[0]).c_str() <<
     std::endl;
       }

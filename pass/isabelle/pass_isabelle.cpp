...
//    Stateful:      Flop plus a conservative first Memory slice in the
//                   executable model. Memory emission supports preserved
//                   async/array memories with at most one read and one write
//                   port, represented as function-valued Isabelle state.
//                   Sync-read and multi-port memories remain fail-fast until
//                   their stateful read-data/collision policy is connected.
//    Rejected:      Latch, Fflop, Sub, LUT, AttrGet/AttrSet, Tposs.
...
const std::unordered_set<std::string> kIsabelleReserved = {
    "type",  "record",  "let",   "if",       "then",  "else",    "case", "end",    "fun",  "definition", "primrec",
    "where", "imports", "begin", "datatype", "lemma", "theorem", "by",   "assume", "show", "obtain",     "fix",
    "do",    "in",      "of",    "and",      "or",    "not",     "True", "False",  "for",
};
...
std::string lit_const_pos(int64_t v, uint32_t w) { return "(" + std::to_string(v) + " :: " + std::to_string(w) + " word)"; }
std::string lit_const_neg(int64_t v, uint32_t w) {
  return "((word_of_int (" + std::to_string(v) + ")) :: " + std::to_string(w) + " word)";
}

std::string isabelle_int_literal(std::string_view decimal) {
  if (!decimal.empty() && decimal.front() == '-') {
    return "(- " + std::string(decimal.substr(1)) + ")";
  }
...
bool is_simple_cert_op_tag(const std::string& tag) {
  static const std::unordered_set<std::string> simple_ops = {"Op_Const",
                                                             "Op_Sum",
                                                             "Op_And",
                                                             "Op_Or",
                                                             "Op_Xor",
...
std::vector<size_t> needed_id_chunks(const std::vector<uint32_t>& deps, const std::vector<uint32_t>& all_ids, size_t chunk_size,
                                     size_t chunk_count) {
...
[[noreturn]] void fatal(const Ctx& /*ctx*/, const std::string& msg) { throw Emit_error("[ERROR] pass.isabelle: " + msg); }
...
std::string lit_const_at(const Ctx& ctx, const Node& node, const Dlop& v, uint32_t w) {
  if (w == 0 || static_cast<size_t>(w) > ctx.max_width) {
    ...
    return undefined_at(1, "Const node n_" + std::to_string(node_id(node)) + " has invalid width " + std::to_string(w));
  }
  if (v.has_unknowns()) {
    if (ctx.strict) {
      fatal(ctx, "Const node n_" + std::to_string(node_id(node)) + " has X/Z bits; v1 strict mode rejects four-valued logic.");
    }
  ...
  if (v.same_repr(*Dlop::create_integer(-1))) {
    return "((word_of_int (-1)) :: " + std::to_string(w) + " word)";
  }
  if (v.is_just_i64()) {
    const int64_t iv = v.to_just_i64();
    if (iv < 0) {
      return lit_const_neg(iv, w);
    }
...
std::string int_of_const(const Ctx& ctx, const Node& node, const Dlop& v, uint32_t w) {
  if (v.has_unknowns()) {
    fatal(ctx,
          "Const node n_" + std::to_string(node_id(node)) + " has X/Z bits; certificate strict mode rejects four-valued logic.");
  }
  if (v.is_known_zero()) {
    return "0";
  }
...
std::string bv_of_word_expr(const std::string& expr, uint32_t w) { return "(BV " + std::to_string(w) + " (uint (" + expr + ")))"; }

std::string word_of_bv_expr(const std::string& rho, uint32_t id, uint32_t w) {
  return "((word_of_int (bv_uint (" + rho + " " + std::to_string(id) + "))) :: " + std::to_string(w) + " word)";
}
...
uint32_t cert_dep_id(const Ctx& ctx, Cert_build& build, const Node_pin& pin, uint32_t expected_w) {
  ...
  if (pin_is_const(pin)) {
    if (expected_w == 0) {
      fatal(ctx, "Const node n_" + std::to_string(id) + " reached by certificate with zero expected width.");
    }
...
std::string cert_const_pin_expr(const Ctx& ctx, const Node_pin& pin, uint32_t synth_id, uint32_t w) {
  ...
  std::ostringstream oss;
  oss << "\\<lparr>nid = " << synth_id << ", op = Op_Const " << int_of_const(ctx, node, pin_const_value(pin), w)
      << ", width = " << w << ", deps = []\\<rparr>";
  return oss.str();
}
...
std::string cert_node_expr(const Ctx& ctx, Cert_build& build, const Node& node, const std::string& forced_op = "",
  ...
  if (op_expr.empty()) {
    switch (node_op(node)) {
      ...
      case Ntype_op::Ror:
      case Ntype_op::EQ : {
        if (node_op(node) == Ntype_op::And) {
          op_expr = "Op_And";
        } else if (node_op(node) == Ntype_op::Or) {
          op_expr = "Op_Or";
        } else if (node_op(node) == Ntype_op::Xor) {
          op_expr = "Op_Xor";
        } else if (node_op(node) == Ntype_op::Ror) {
          op_expr = "Op_Ror";
        } else {
          op_expr = "Op_EQ";
        }
      ...
      case Ntype_op::SHL: {
        ...
        if (!have_a || bs.empty()) {
          fatal(ctx, "SHL node n_" + std::to_string(node_id(node)) + " is malformed.");
        }
        op_expr = "Op_SHL";
      ...
      case Ntype_op::SRA: {
        ...
        if (!have_a && have_b) {
          ...
          deps = {cert_dep_id(ctx, build, a, pin_width(ctx, a, node)), cert_dep_id(ctx, build, b, shift_w)};
        } else {
      ...
      case Ntype_op::Mux: {
        ...
        for (const auto& kv : options) {
          deps.push_back(cert_dep_id(ctx, build, kv.second, w));
        }
        break;
      }
...
void emit_memory_cert_stub_theory(const Ctx &ctx,
  ...
  ofs << "definition " << ctx.top_name << "_node_count :: nat where\n";
  ofs << "  \"" << ctx.top_name << "_node_count = " << topo.size() << "\"\n\n";
  ofs << "definition " << ctx.top_name << "_flop_count :: nat where\n";
  ofs << "  \"" << ctx.top_name << "_flop_count = " << flop_nodes.size() << "\"\n\n";
  ofs << "definition " << ctx.top_name << "_memory_count :: nat where\n";
  ...
  ofs << "  generated model represents them as function-valued Isabelle state fields\n";
  ofs << "  using SemanticPrimitives.mem_read/mem_write/mem_write_be.  The current\n";
  ofs << "  graph certificate evaluator is still bit-vector-only, so pass.isabelle\n";
  ofs << "  intentionally does not emit graph_cert_wf or eval_graph bridge theorems\n";
  ofs << "  for memory designs yet.  Add memory-valued certificate sources and memory\n";
  ofs << "  read/write evaluator operators before enabling certificate proofs here.\n";
  ofs << "\\<close>\n\n";
...
void emit_cert_theory(const Ctx &ctx,
  ...
  for (const auto& kv : build.const_pins) {
    ...
    if (id_it == build.const_ids.end()) {
      fatal(ctx,
            "internal: missing synthetic certificate id for const node n_" + std::to_string(static_cast<uint64_t>(kv.first.first)));
    }
  ...
  std::cerr << "pass.isabelle cert chunks for " << ctx.top_name << ": total_nodes=" << all_exprs.size()
            << " chunk_size=" << chunk_size << " emitted_chunks=" << chunk_count << " full_chunks=" << full_chunk_count << "\n";
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    std::cerr << "  chunk " << chunk << ": nodes=" << (end - begin)
              << " class=" << (const_only_chunk ? "const-only" : (simple_chunk ? "simple-mixed" : "unsupported-mixed")) << " ops={"
  ...
  ofs << "lemma " << ctx.top_name << "_certificate_counts:\n";
  ofs << "  \"" << node_count_name << " = " << topo.size() << " \\<and> " << flop_count_name << " = " << flop_nodes.size()
      << "\"\n";
  ofs << "  by (simp add: " << node_count_name << "_def " << flop_count_name << "_def)\n\n";
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    ofs << "  \"" << certs_name << "_ids_" << chunk << " = map nid " << certs_name << "_" << chunk << "\"\n\n";
  }
  ...
  for (const auto id : source_ids) {
    ...
    if (expr_it == build.source_exprs.end()) {
      continue;
    }
    ofs << "     " << (first_src ? "(" : "else ") << "if n = " << id << " then " << expr_it->second << "\n";
    first_src = false;
  }
  ofs << "      else BV 1 0";
  if (!first_src) {
  ...
  ofs << "definition " << outputs_name << " :: \"(nat \\<Rightarrow> bv) \\<Rightarrow> " << ctx.top_name << "_out\" where\n";
  ...
  for (const auto& kv : ctx.output_field) {
    ...
    if (id_it == out_driver_ids.end()) {
      ...
      ofs << kv.second << " = " << word_of_bv_expr("rho", id_it->second, ctx.output_width.at(out_name));
    }
  }
  ...
  if (sequential) {
    ofs << "definition " << next_name << " :: \"(nat \\<Rightarrow> bv) \\<Rightarrow> " << ctx.top_name << "_state\" where\n";
    ...
    for (const auto& fn : flop_nodes) {
      ...
      const auto din     = flop_din_ids.count(fid) ? word_of_bv_expr("rho", flop_din_ids.at(fid), fw) : lit_zero(fw);
      const auto reset = flop_reset_ids.count(fid) ? "(bv_nonzero (rho " + std::to_string(flop_reset_ids.at(fid)) + "))" : "False";
      const auto enable
          = flop_enable_ids.count(fid) ? "(bv_nonzero (rho " + std::to_string(flop_enable_ids.at(fid)) + "))" : "True";
      ofs << fld << " = flop_next " << reset << " (0 :: " << fw << " word) " << enable << " " << din << " " << current;
    }
    ofs << "\\<rparr>\"\n\n";

    ofs << "definition " << cert_step_name << " :: \"" << ctx.top_name << "_in \\<Rightarrow> " << ctx.top_name
        << "_state \\<Rightarrow> " << ctx.top_name << "_state \\<times> " << ctx.top_name << "_out\" where\n";
    ofs << "  \"" << cert_step_name << " i s =\n";
    ofs << "     (let rho = eval_graph (topo " << graph_name << ") " << graph_name << " (" << source_env_name << " i s)\n";
    ofs << "      in (" << next_name << " rho, " << outputs_name << " rho))\"\n\n";
  }

  ofs << "definition " << wf_bool_name << " :: bool where\n";
  ofs << "  \"" << wf_bool_name << " = graph_cert_wf_bool " << certs_name << " " << sources_name << "\"\n\n";
  ...
  if (ctx.cert_wf == CertWFMode::Chunked) {
    for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
      ...
      ofs << "  \"map nid " << certs_name << "_" << chunk << " = " << certs_name << "_ids_" << chunk << "\"\n";
      ...
      if (const_only_chunk) {
        ...
        ofs << "  by (simp add: " << certs_name << "_" << chunk << "_def const_node_cert_wf_bool_def)\n\n";
        ...
        ofs << "  \"set (map nid " << certs_name << "_" << chunk << ") \\<subseteq> set " << all_ids_name << "\"\n";
        ofs << "  by (auto simp add: " << all_ids_name << "_def " << id_chunks_name << "_def " << certs_name << "_ids_" << chunk
            << "_def)\n\n";

        ofs << "lemma " << ctx.top_name << "_cert_chunk_" << chunk << "_ids_disjoint:\n";
        ofs << "  \"set (map nid " << certs_name << "_" << chunk << ") \\<inter> set " << sources_name << " = {}\"\n";
        ofs << "  by (simp add: " << certs_name << "_" << chunk << "_def " << sources_name << "_def)\n\n";

        ofs << "lemma " << ctx.top_name << "_cert_chunk_" << chunk << "_ok:\n";
        ofs << "  \"node_cert_chunk_wf_bool " << all_ids_name << " " << sources_name << " " << certs_name << "_" << chunk << "\"\n";
        ...
        ofs << "  \"list_all simple_node_cert_shape_wf_bool " << certs_name << "_" << chunk << "\"\n";
        ofs << "  by (simp add: " << certs_name << "_" << chunk << "_def "
            << "simple_node_cert_shape_wf_bool_def simple_op_cert_wf_bool_def)\n\n";

        ofs << "lemma " << ctx.top_name << "_cert_chunk_" << chunk << "_deps:\n";
        ofs << "  \"set (node_cert_deps " << certs_name << "_" << chunk << ") \\<subseteq> set " << deps_name << "\"\n";
        ofs << "  by (simp add: " << certs_name << "_" << chunk << "_def " << deps_name << "_def node_cert_deps_def)\n\n";
        ...
        ofs << "  \"set (node_cert_deps " << certs_name << "_" << chunk << ") \\<subseteq> set " << all_ids_name
            << " \\<union> set " << sources_name << "\"\n";
        ...
        ofs << "lemma " << ctx.top_name << "_cert_chunk_" << chunk << "_ok:\n";
        ofs << "  \"node_cert_chunk_wf_bool " << all_ids_name << " " << sources_name << " " << certs_name << "_" << chunk << "\"\n";
        ofs << "  by (rule simple_node_cert_chunk_wf_bool_sound'[OF " << ctx.top_name << "_cert_chunk_" << chunk << "_shape "
            << ctx.top_name << "_cert_chunk_" << chunk << "_deps_subset " << ctx.top_name << "_cert_chunk_" << chunk
            << "_ids_distinct " << ctx.top_name << "_cert_chunk_" << chunk << "_ids_subset " << ctx.top_name << "_cert_chunk_"
            << chunk << "_ids_disjoint])\n\n";
      } else {
        if (ctx.cert_wf_fallback == CertWFFallback::Fail) {
          fatal(ctx,
                "unsupported mixed certificate chunk " + std::to_string(chunk) + " for " + ctx.top_name
                    + " under cert_wf_fallback:fail; ops={" + chunk_op_summary(all_op_tags, begin, end) + "}");
        }
        ofs << "lemma " << ctx.top_name << "_cert_chunk_" << chunk << "_ok:\n";
        ofs << "  \"node_cert_chunk_wf_bool " << all_ids_name << " " << sources_name << " " << certs_name << "_" << chunk << "\"\n";
        if (ctx.cert_wf_fallback == CertWFFallback::Sorry) {
          ofs << "  sorry (* placeholder: unsupported mixed certificate chunk; ops={" << chunk_op_summary(all_op_tags, begin, end)
              << "} *)\n\n";
        } else {
          ofs << "  by eval\n\n";
        }
    ...
    ofs << "  \"list_all (node_cert_chunk_wf_bool " << all_ids_name << " " << sources_name << ") " << cert_chunks_name << "\"\n";
    ...
    if (chunk_limited) {
      ofs << "text \\<open>\n";
      ofs << "  cert_chunk_limit emitted only " << chunk_count << " of " << full_chunk_count
          << " chunks.  This theory is for proof-shape\n";
      ...
      ofs << "  unfolding " << graph_name << "_def " << topo_name << "_def " << nodes_name << "_def " << certs_name << "_def\n";
      ofs << "  by (rule graph_cert_wf_from_chunk_checks[where chunks=" << cert_chunks_name << " and id_chunks=" << id_chunks_name
          << "],\n";
      ofs << "      simp_all add: " << ctx.top_name << "_cert_chunks_ids " << ctx.top_name << "_cert_all_ids_distinct "
          << ctx.top_name << "_cert_sources_distinct " << ctx.top_name << "_cert_chunks_ok " << all_ids_name << "_def)\n\n";
    }
  ...
  std::cout << "pass.isabelle: " << ctx.base_name << " -> " << path << " (" << topo.size() << " internal nodes, "
            << build.const_pins.size() << " const nodes, " << build.source_ids.size() << " source nodes, " << flop_nodes.size()
            << " flops)\n";
}
  ...
  switch (op) {
    ...
    case Ntype_op::Sum: {
      ...
      if (a_terms.empty() && b_terms.empty()) {
        fatal(ctx, "Sum node n_" + std::to_string(node_id(node)) + " has 0 inputs - pass.cprop should have folded it.");
      }
      if (b_terms.empty()) {
    ...
    case Ntype_op::Div: {
      ...
      return "(sem_udiv " + ucast_pin_at(ctx, a, w) + " " + ucast_pin_at(ctx, b, w) + ")";
    }
    ...
    case Ntype_op::SHL: {
      ...
      for (auto& bd : bs) {
        ...
        shifts.push_back("(push_bit (unat (" + shift_amount_expr_at(ctx, bd, bw) + ")) " + a_cast + ")");
      }
    ...
    case Ntype_op::Mux: {
      ...
      if (!have_sel || options.size() < 2) {
        fatal(ctx, "Mux node n_" + std::to_string(node_id(node)) + " is malformed (need selector + >=2 options).");
      }
      ...
      if (options.size() == 2 && sel_w == 1) {
        ...
        return "(if (" + sel_e + ") \\<noteq> 0 then " + ucast_pin_at(ctx, true_drv, w) + " else " + ucast_pin_at(ctx, false_drv, w)
               + ")";
      }
      ...
      size_t      idx = 0;
      for (auto& kv : options) {
        out += "if unat (" + sel_e + ") = " + std::to_string(idx) + " then " + ucast_pin_at(ctx, kv.second, w) + " else ";
        ++idx;
      }
      out += lit_zero(w) + ")";
      return out;
    ...
    default: throw Emit_error("internal: unhandled Ntype_op in emit_node_expr");
  }
}
...
void Pass_isabelle::emit_for_graph(const std::shared_ptr<hhds::Graph>& graph) const {
  ...
  for (auto node : g->fast_class()) {
    if (node_is_memory(node)) {
      ...
      memory_nids.insert(node_id(node));
    }
  }
  ...
  if (!ofs) {
    livehd::diag::warn("pass.isabelle", "write-failed", "io").msg("could not write {}", theory_path).emit();
    return;
  }
  ...
  try {
    ...
    // <top>_comb signature.
    if (sequential) {
      ofs << "definition " << ctx.top_name << "_comb :: \"" << ctx.top_name << "_in \\<Rightarrow> " << ctx.top_name
          << "_state \\<Rightarrow> " << ctx.top_name << "_out\" where\n";
    ...
    if (sequential) {
      ofs << "definition " << ctx.top_name << "_next :: \"" << ctx.top_name << "_in \\<Rightarrow> " << ctx.top_name
          << "_state \\<Rightarrow> " << ctx.top_name << "_state\" where\n";
      ...
      ofs << "definition " << ctx.top_name << "_step :: \"" << ctx.top_name << "_in \\<Rightarrow> " << ctx.top_name
          << "_state \\<Rightarrow> " << ctx.top_name << "_state \\<times> " << ctx.top_name << "_out\" where\n";
      ofs << "  \"" << ctx.top_name << "_step i s = (" << ctx.top_name << "_next i s, " << ctx.top_name << "_comb i s)\"\n\n";
    }
    ...
    if (ctx.strict) {
      ...
      livehd::diag::err("pass.isabelle", "isabelle-error", "internal").msg("{}", err.what()).fatal();
    }
    return;
  }

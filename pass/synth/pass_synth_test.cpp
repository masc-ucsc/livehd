...
protected:
  std::string root;
  void        SetUp() override {
    root = (fs::temp_directory_path() / "livehd-synth-lifetime-XXXXXX").string();
...
TEST_F(PassSynth, RepeatedRunsMapUnateRegionsAndReuseTheCache) {
  ...
  for (unsigned i = 0; i < 3; ++i) {
    ...
    const auto text = read(report + ".synth.json");
    EXPECT_NE(text.find("\"kind\":\"synth\""), std::string::npos) << text;
    if (i == 0) {
      EXPECT_NE(text.find("\"status\":\"unate\""), std::string::npos) << text;
    ...
    EXPECT_TRUE(fs::exists(report + ".witness.jsonl"));
    EXPECT_FALSE(read(root + "/cache_templates.bin").empty());
  }
  // Without a cache the region is searched again, with identical decisions.
  var.dict.erase("cache_dir");
  run();
  EXPECT_NE(read(report + ".synth.json").find("\"reused\":0"), std::string::npos);

...
// A function of at most 6 inputs in one word, replicated over 8 variables.
Table replicate(uint64_t t, uint32_t n) {
  t = low_bits(t, n);
  for (uint32_t v = n; v < 6; ++v) {
    t |= t << (1U << v);
  }
  return {t, t, t, t};
}
...
Cover_result lut_cover(const Logic_network& source, const Search_options& options) {
  Cover_result result;
  if (!source.valid() || options.recipes.empty()) {
    result.status = Status::invalid;
    result.reason = "invalid source network";
    return result;
  }
  const auto&    recipe = options.recipes.front();
  const uint32_t k      = recipe.support;
  const auto     n      = static_cast<Id>(source.nodes.size());
  ...
  for (Id v = 0; v < n; ++v) {
    const auto& node = source.nodes[v];
    kind[v]          = node.source ? kSource : node.inputs.empty() ? kConstant : kInternal;
    if (node.inputs.size() > k) {
      result.status = Status::unsupported;
  ...
  const auto duplication_free = [&](Id v, const Id* u, uint32_t us) {
    ...
    cone_stack.assign(source.nodes[v].inputs.begin(), source.nodes[v].inputs.end());
    while (!cone_stack.empty()) {
      ...
      cone.push_back(w);
      cone_stack.insert(cone_stack.end(), source.nodes[w].inputs.begin(), source.nodes[w].inputs.end());
    }
    for (auto w : cone) {
      if (kind[w] == kConstant) {
  ...
  for (Id v = 0; v < n; ++v) {
    ...
    if (kind[v] == kConstant) {
      Cut c;
      c.table = node.table.get(0) ? kOnes : Table{};
      c.fn    = store.get(c.table, 0);
      cuts[v] = {c};
      continue;
    }
    const auto m = static_cast<uint32_t>(node.inputs.size());
    for (uint32_t j = 0; j < m; ++j) {
      const auto in = node.inputs[j];
  ...
  for (uint32_t w = 0; w < 4; ++w) {
    for (Id v = 0; v < n; ++v) {
      ...
      for (size_t j = 0; j < node.inputs.size(); ++j) {
        in[j] = sv[node.inputs[j]];
      }
      sv[v] = eval_table(node.table.words.data(), static_cast<uint32_t>(node.inputs.size()), in.data());
    }
    for (const auto& lut : result.luts) {
...
Cover_reference classify_luts(const std::vector<std::pair<uint64_t, uint32_t>>& tables, const Recipe& recipe, const Cover_cost& cost) {
  Cover_reference result;
  Store           store(recipe, cost);
  for (const auto& [table, inputs] : tables) {
    if (inputs > 6) {
      continue;
    }
    Cut c;
    c.size  = static_cast<uint8_t>(inputs);
    c.table = replicate(table, inputs);
    reduce(c);
    const auto& f = store.fns[store.get(c.table, c.size)];
    ++result.luts;
    result.cost += f.cost;
    if (f.constant || f.alias) {
      ++result.aliases;
    } else {
      ++(f.domino ? result.domino : result.nonunate);
    }
  }
  return result;
}

...
// One LUT of the cover: `root` of the source network as a function of
// `leaves` (sources or other LUT roots), in ascending leaf order.
struct Cover_lut {
  Id              root = 0;
...
// Cut-based cover of the whole region: priority cuts with composed truth
// tables, a depth-optimal round, then area-flow and exact-area rounds that
// minimize the summed transistor proxy under the domino_levels required times.
// The cover is checked by simulation against the source network.
Cover_result lut_cover(const Logic_network& source, const Search_options& options);

// The ABC `&if` reference, classified by the same costs (insight only).
struct Cover_reference {
  uint64_t luts = 0, domino = 0, nonunate = 0, aliases = 0, cost = 0;
};
// Tables of at most 6 inputs (bit x of the word is f(x)).

...
TEST_F(Provenance, CapturesStableInputBytesWithSha256AndIndependentSnapshots) {
  ...
  const auto          summary = livehd::synth::archive_provenance(root / "archive", context(root / "input"), "mapper-v1");
  ...
  EXPECT_THROW(livehd::synth::archive_provenance(root / "archive", context(root / "input"), "mapper-v1"), std::runtime_error);
}
TEST_F(Provenance, LimitsAndUnavailableInputsRemainExplicitlyIncomplete) {
  put(root / "large", "abcd");
  livehd::synth::archive_provenance(root / "limited", context(root / "large"), "v1", {3, 5});
  ...
  livehd::synth::archive_provenance(root / "missing", context(root / "absent"), "v1");
  EXPECT_FALSE(manifest("missing")["capture_complete"].GetBool());
  livehd::synth::archive_provenance(root / "unknown", "", "v1");
  EXPECT_FALSE(manifest("unknown")["capture_complete"].GetBool());
  put(root / "tree/a", "a");
  put(root / "tree/b", "b");
  livehd::synth::archive_provenance(root / "entries", context(root / "tree"), "v1", {100, 1});
...
TEST_F(Provenance, RefusesRecursiveArchivesAndDirectorySymlinks) {
  ...
  livehd::synth::archive_provenance(root / "links", context(root / "input"), "v1");
  EXPECT_FALSE(manifest("links")["capture_complete"].GetBool());
  livehd::synth::archive_provenance(root / "nested", context(root), "v1");

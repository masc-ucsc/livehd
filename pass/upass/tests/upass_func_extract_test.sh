...
"${LGSHELL}" -q -c "inou.prp files:${PRP_FILE} |> pass.upass constprop:false assert:false verifier:false ssa:false |> lnast.dump" \
  >"${OUT_FILE}" 2>&1
...
"${LGSHELL}" -q -c "inou.prp files:${EMPTY_PRP} |> pass.upass constprop:false assert:false verifier:false ssa:false |> lnast.dump" \
  >"${EMPTY_OUT_FILE}" 2>&1

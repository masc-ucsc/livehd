...
void Bitwidth::process_mux(Node &node, XEdge_iterator &inp_edges) {
  ...
  for (auto e : inp_edges) {
    if (e.sink.get_pid() == 0) {
      ...
      I(n_data_mux); // mux has at least 2 inputs
      Bitwidth_range bw2(-n_data_mux, n_data_mux -1);
...
void Bitwidth::insert_tposs_nodes(Node &node_attr_hier, Bits_t ubits, Fwd_edge_iterator::Fwd_iter &fwd_it) {
  ...
  for (auto &e : node_attr.out_edges()) {
    ...
    if (ntposs.is_invalid()) {
      ...
      ntposs.setup_driver_pin().set_unsign();
    ...
    lg->each_graph_input(
        [this](Node_pin &dpin) {
          if (dpin.get_bits()) {
            ...
            if (dpin.is_unsign()) {
              bw.set_ubits_range(dpin.get_bits());
            }else{

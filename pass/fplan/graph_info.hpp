...
typedef decltype(graph::Bi_adjacency_list().vert_map<int>()) set_map_t;
...
public:
  ...
  Graph_info(
    ...
    set_map_t&& new_sets
  ) : al(std::move(new_list)), names(std::move(new_names)), areas(std::move(new_areas)), weights(std::move(new_weights)), sets(std::move(new_sets)) { }
  ...
  void print() {
    ...
    std::cout << al.dot_format("name"_of_vert = names, "set"_of_vert = sets, "weight"_of_edge = weights) << std::endl;
  }
};

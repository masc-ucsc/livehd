...
public:
  ...
  Graph_info(graph::Bi_adjacency_list&& new_list, name_map_t&& new_names, area_map_t&& new_areas, weight_map_t&& new_weights,
             set_t&& new_sets)
      : al(std::move(new_list))
      , names(std::move(new_names))
      , areas(std::move(new_areas))
      , weights(std::move(new_weights))
      , sets(1, std::move(new_sets)) {}

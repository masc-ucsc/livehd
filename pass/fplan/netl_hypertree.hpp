...
struct Netl_node {
  ...
  typedef std::shared_ptr<Netl_node> pnode; // this typedef is useful both inside and outside Netl_node
...
typedef std::shared_ptr<Netl_node> pnode;

struct cost_matrix_row {
  pnode node; // the actual netl_node we're referring to
...
class Netl_hypertree {
public:
  Netl_hypertree() { }
  
  // construct a new tree from a node, assuming that node represents a hierarchy
  Netl_hypertree(const Netl_node& n) { root = std::make_shared<Netl_node>(n); }
  
  // copies require copying the entire tree and are very expensive.
  Netl_hypertree(const Netl_hypertree& other) = delete;
	Netl_hypertree& operator=(const Netl_hypertree& other) = delete;
  
  // moves defined since copies are deleted
  Netl_hypertree(Netl_hypertree&& other) noexcept : 
    root(other.root), 
    min_area(other.min_area),
    num_components(other.num_components) { other.root.reset(); }

  Netl_hypertree& operator=(Netl_hypertree&& other) noexcept {
...
private:
  ...
  pnode make_hier_tree(pnode t1, pnode t2);

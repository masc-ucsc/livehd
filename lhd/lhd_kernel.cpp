...
// Run inou.cgen.sim (TODO 3d): one <module>.hpp per graph into `odir`. Mirrors
// cgen_into — seed odir, run the step, then assert each header exists.
...
// module of per-module Slop<N> headers over ../hlop: the pass writes <name>.hpp,
// here we add the build scaffold (MODULE.bazel / BUILD / sim_all.cpp) + manifest
// so `cd DIR && bazel build //:sim` compiles the library.
void emit_sim_outputs(Options& opts, Result& res, Eprp_var& var) {
  ...
  auto        names = sim_into(opts, res, var, dir);  // writes <name>.hpp + checks
  const auto  hlop  = sim_hlop_path(opts);

  // sim_all.cpp — includes every module header so the cc_library actually
  // compiles them (a headers-only target would not be type-checked).
  {
    std::ofstream ofs(std::format("{}/sim_all.cpp", dir));
    ofs << "// Generated by inou.cgen.sim. Compile-checks every module header.\n";
    for (const auto& n : names) {
      ofs << std::format("#include \"{}.hpp\"\n", n);
    }
  }
  ...
  // BUILD — one cc_library compiling sim_all.cpp against @hlop//hlop (C++23).
  {
  ...
  std::vector<std::pair<std::string, uint64_t>> manifest;
  for (const auto& n : names) {
    std::ifstream      ifs(std::format("{}/{}.hpp", dir, n));
    std::ostringstream oss;
    oss << ifs.rdbuf();
    manifest.emplace_back(n, hash_bytes(oss.str()));
  }

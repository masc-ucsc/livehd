...
#   sequential: abc_seq mapped UNCOLORED (flat __c0, native Flops kept) ->
...
run pass opentimer --top "${TOP}__c1" lg:"$W/net" "$LIB" --workdir "$W/wt"
...
# 3. sequential, flat (uncolored -> single __c0 region, native Flops kept):
...
"$LHD" pass opentimer --top "${STOP}__c0" lg:"$W/snet" "$LIB" --workdir "$W/wts" \

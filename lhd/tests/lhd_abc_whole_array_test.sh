...
run lec --impl lg:"$W/$design" --ref verilog:"$W/ref.v" --lib lg:"$W/models" --impl-top "$top" --ref-top reference \
  --set formal.bound=2 --set formal.timeout=60 --workdir "$W/$design-default"
...
PY
# Check the actual source clock/enable behavior independently of the native
# reader: a dropped bulk-write clock must not become a combinational loop.
# Icarus does not support the source's unpacked whole-array assignment.
# Express its copy-then-overwrite behavior directly as one enabled entry write.
cat > "$W/array_ref.v" <<'SV'
...
SV
iverilog -g2012 -s array_tb -o "$W/array_sim" "$W/array.v" "$W/models.v" "$W/array_ref.v" "$W/array_tb.v"
vvp "$W/array_sim"

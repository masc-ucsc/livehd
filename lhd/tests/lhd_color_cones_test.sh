...
#
# End-to-end test for `pass.color synth --set pass.color.synth.mode=cones`
...
#   lhd pass color synth --set color.synth.mode=cones --set color.synth.max_gate=<small>
...
# coloring_info (`"mode":"cones"`, `"max_gate"`, and the "packed":true that
...
# multi-region path -- the one that can go wrong -- would never run. For the
# same reason every run keeps colors under the shipped 12-node floor
# (min_color_nodes=0): the fixtures are smaller than one real region.
set -u
LHD=lhd/lhd
...
for entry in "${DESIGNS[@]}"; do
  ...
  run pass color synth --set color.synth.min_color_nodes=0 --top "$TOP" --stats \
       --set color.synth.max_gate=40 \
  ...
  info_has '"mode":"cones"' || fail "$FIX: coloring_info does not record mode=cones"
...
for MODE in false pair; do
  ...
  run pass color synth --set color.synth.min_color_nodes=0 --top hier_seq.top  --set color.synth.max_gate=40 \
      --set "color.synth.forward=$MODE" lg:"$D/lg" --workdir "$D/w3"
...
if "$LHD" pass color synth --set color.synth.min_color_nodes=0 --top hier_seq.top  --set color.synth.forward=maybe \
...
count_colors() {  # $1 = max_gate
  rm -rf "$D/lg_$1"
  cp -R "$D/lg0" "$D/lg_$1"
  "$LHD" pass color synth --set color.synth.min_color_nodes=0 --top hier_seq.top  --set color.synth.max_gate="$1" --stats \
...
"$LHD" synth "inou/prp/tests/pyrope/hier_seq.prp" --top hier_seq.top --workdir "$D/w" \
   --set synth.liberty="$LIB"  --set color.synth.max_gate=40 \
   --set synth.opentimer=false \
   -q --result-json "$D/r.json" || fail "lhd synth with cones -> $(cat "$D/r.json" 2>/dev/null)"
grep -q '"regions":' "$D/w/synth/qor.json" || fail "lhd synth --set color.synth.mode=cones produced no abc regions"
...
EOF
echo "PASS: lhd synth --set color.synth.mode=cones maps regions and records pred_aig"

# Negative control: a typo must be refused, not silently colored with `synth`.
if "$LHD" pass color synth --set color.synth.min_color_nodes=0 --top hier_seq.top --set color.synth.mode=conez lg:"$D/../raw/lg" \
     --workdir "$W/neg" -q --result-json "$W/neg.json" 2>/dev/null; then
  fail "an unknown pass.color.synth.mode was accepted"
fi
echo "PASS: unknown pass.color.synth.mode is refused"
...
run pass color synth --set color.synth.min_color_nodes=0 lg:"$D/lg_merged" --top ctrl_shared  --set color.synth.max_gate=5000 --workdir "$D/w2m"
LC_ALL=C grep -raq '"mux_groups":1,' "$D/lg_merged" || fail 'overlapping mux selects were not merged'
run pass color synth --set color.synth.min_color_nodes=0 lg:"$D/lg" --top ctrl_shared  --set color.synth.max_gate=40 --workdir "$D/w2"
...
cp -R "$D/lg" "$D/lg_off"
run pass color synth --set color.synth.min_color_nodes=0 lg:"$D/lg_off" --top ctrl_shared  --set color.synth.ctrl_cones=false --workdir "$D/off"
...
run synth "$D/ref.v" --top ctrl_shared --workdir "$D/syn" --set synth.liberty="$LIB" --set synth.opentimer=false  --set color.synth.max_gate=40 --emit verilog:"$D/mapped.v"
...
cp -R "$D/lg" "$D/lg_merged"
run pass color synth --set color.synth.min_color_nodes=0 lg:"$D/lg_merged" --top ctrl_families   --set color.synth.max_gate=5000 --workdir "$D/w2m"
...
run pass color synth --set color.synth.min_color_nodes=0 lg:"$D/lg" --top ctrl_families   --set color.synth.max_gate=40 --workdir "$D/w2"
...
run synth "$D/ref.v" --top ctrl_families --workdir "$D/syn" --set synth.liberty="$LIB" --set synth.opentimer=false   --set color.synth.max_gate=40 --emit verilog:"$D/mapped.v"
...
run pass color synth --set color.synth.min_color_nodes=0 lg:"$D/lg" --top ctrl_normalizer  --workdir "$D/w2"

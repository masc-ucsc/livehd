...
#   lhd elaborate --top bar lg:L1 ln:L2 --emit-dir lg:L3
...
"$LHD" elaborate "$W/foo.prp" --emit-dir lg:"$W/L1/" --workdir "$W/w1" -q --result-json "$W/r1.json" \
  || fail "foo elaborate→lg failed: $(cat "$W/r1.json" 2>/dev/null)"
...
"$LHD" elaborate "$W/bar.prp" --emit-dir ln:"$W/L2/" --workdir "$W/w2" -q --result-json "$W/r2.json" \
  || fail "bar elaborate→ln failed: $(cat "$W/r2.json" 2>/dev/null)"
grep -q '"status":"pass"' "$W/r2.json" || fail "bar result not pass"

# ── 3. MERGE: assemble L1 (foo) + L2 (bar) into a new lg: library L3 ──────────
"$LHD" elaborate --top bar lg:"$W/L1/" ln:"$W/L2/" --emit-dir lg:"$W/L3/" \
  --workdir "$W/w3" -q --result-json "$W/r3.json" \
  || fail "merge elaborate failed: $(cat "$W/r3.json" 2>/dev/null)"
...
# ── 4. synth the assembled library → Verilog: both modules, bar instantiates foo
"$LHD" synth lg:"$W/L3/" --emit verilog:"$W/out.v" --workdir "$W/w4" -q --result-json "$W/r4.json" \
  || fail "synth of merged library failed: $(cat "$W/r4.json" 2>/dev/null)"
grep -q '"status":"pass"' "$W/r4.json" || fail "synth result not pass"

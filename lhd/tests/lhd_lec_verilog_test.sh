...
# the same word/bit states. Every proposed pair must prove its transition.
python3 - "$W" <<'PYMEMORY_NAMES'
from pathlib import Path
import sys
w = Path(sys.argv[1])
ports = "input clk, we, input [3:0] wa, ra, input [7:0] d, output [7:0] y"
...
for variant in ("impl", "bad"):
    ...
    for word in range(16):
        ...
        lines += [f"reg [7:0] _mem{word};",
                  f"always @(posedge clk) if (we && wa == 4'd{word}) _mem{word} <= {value};"]
    lines += ["assign y = " + "".join(f"ra == 4'd{i} ? _mem{i} : " for i in range(15)) + "_mem15;",
              "endmodule"]

...
# The 5 sources are shared by the ware/stop combinations below; write them once.
SOURCES = [('arith','a+b+c-d','sum'),('cmp','a<b','lt'),('cmp','a>b','gt'),
           ('shift','a>>s','sra'),('shift','a<<s','shl')]
for kind,expr,macro in SOURCES:
    (w/f'{macro}.v').write_text(
        f'module top(input signed [16:0] a, input [10:0] b, input [8:0] c,d, input [5:0] s, output [{0 if kind=="cmp" else 16}:0] y); assign y={expr}; endmodule\n')

def combo(job):
    kind,macro,ware,stop = job
    src=w/f'{macro}.v'
...
def narrow(job):
    data_bits,count_bits,amount,shift,signed = job
...
# Every case below is independent (its own workdir, its own emitted netlist), so
# they run concurrently; each `lhd` is pinned to synth.threads=1 and the BUILD
# target reserves the fan-out with a cpu: tag.
jobs=[(k,m,wa,st) for k,_,m in SOURCES for wa in ('true','false') for st in ('true','false')]
narrows=[(d,c,a,sh,sg) for d,c,a in ((257,9,'s'),(17,32,'s'),(4,32,"32'd128"))
         for sh in ('>>>','<<') for sg in ('','signed')]
with ThreadPoolExecutor(max_workers=int(os.environ.get('WARE_SMOKE_JOBS','8'))) as pool:
    list(pool.map(combo, jobs))
    src=list(pool.map(narrow, narrows))[-1]

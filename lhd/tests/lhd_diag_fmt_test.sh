...
out=$("$LHD" elaborate "$PRP" --workdir "$W/w1" -q 2>/dev/null)
case "$out" in
  '{"schema_version"'*) ;;
...
out=$("$LHD" elaborate "$PRP" --workdir "$W/w2" -q --diag-fmt pretty 2>/dev/null)
echo "$out" | grep -q 'lhd elaborate pyrope: pass' || fail "pretty pass line missing: $out"
...
"$LHD" elaborate "$W/bad.prp" --workdir "$W/w3" --diag-fmt pretty >"$W/o3" 2>"$W/e3" && fail "bad.prp must fail"
grep -q 'lhd elaborate pyrope: fail' "$W/o3" || fail "pretty fail line missing: $(cat "$W/o3")"
...
"$LHD" elaborate "$W/bad.prp" --workdir "$W/w4" --diag-fmt jsonl >"$W/o4" 2>"$W/e4" && fail "bad.prp must fail"
...
"$LHD" elaborate "$PRP" --workdir "$W/w5" -q --diag-fmt pretty --result-json "$W/r5.json" 2>/dev/null \
  || fail "elaborate with --result-json exited nonzero"
...
"$LHD" elaborate "$PRP" --diag-fmt bogus -q 2>/dev/null >"$W/r6.json"

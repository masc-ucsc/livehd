...
int help_command(const Options& opts) {
  ...
  if (topic == "formal" || topic == "formal verify") {
    std::print(
        ...
        "structured minetimeout core, witness artifact paths) — pass --workdir to keep\n"
        "it; a REFUTED run adds formalfail.prp/.json (+ VCD replay). Knob formal.report.\n"
        "with formal.minetimeout set, a stuck run also MINES invariants (base-proven +\n"
        ...
        "  witness, strict, solver, report, minetimeout, ...); legacy lec.* accepted.\n"
        "\n"
        "examples:\n"
        "  lhd formal verify foo.prp --top foo --set formal.bound=12\n"

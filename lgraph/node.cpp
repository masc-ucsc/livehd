...
std::string Node::debug_name() const {
  ...
  if (conta++ == 0) {
    std::print("WARNING: Node::debug_name should not be called during release (Slowww!)\n");
  }
#endif
...
void Node::dump() const {
  std::print("nid:{} type:{} lgraph:{} ", nid, get_type_name(), current_g->get_name());
  if (has_color()) {
    std::print(" color:{} ", get_color());
  }
  std::print("is_type_loop_last:{} ", is_type_loop_last());
  std::print("node_name:{} ", has_name() ? get_name() : "N.A");
  if (has_loc()) {
    auto [loc1, loc2] = get_loc();
    std::print(" loc:[{},{}] ", loc1, loc2);
  }
